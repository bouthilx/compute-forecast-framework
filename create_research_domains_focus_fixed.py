...
def create_research_domains_focus_chart_fixed():
    ...
    ax1.set_title('Research Domain Proportions Over Time (FIXED)\n(Excluding unclassified papers)', fontsize=14, fontweight='bold')
    ...
    ax2.set_title('Research Domain Counts Over Time (FIXED)', fontsize=14, fontweight='bold')
    ax2.set_xlabel('Year')
    ...
    plt.tight_layout()
    plt.savefig('research_domains_temporal_analysis_FIXED.png', dpi=300, bbox_inches='tight')
    plt.show()
...
def create_comparison_chart():
    ...
    for year in years:
        orig_count = sum(original_data['year_data'][str(year)][cat] for cat in research_categories)
        fixed_count = sum(fixed_data['year_data'][str(year)][cat] for cat in research_categories)
    ...
    bars1 = ax.bar(x - width/2, original_research_counts, width, label='Original (broken)', color='#ff7f7f', alpha=0.8)
    bars2 = ax.bar(x + width/2, fixed_research_counts, width, label='Fixed', color='#2ca02c', alpha=0.8)
    
    ax.set_title('Research Papers with Domain Classification\n(Original vs Fixed)', fontsize=14, fontweight='bold')
    ax.set_xlabel('Year')
    ...
    plt.tight_layout()
    plt.savefig('research_papers_comparison_original_vs_fixed.png', dpi=300, bbox_inches='tight')
    plt.show()
...
def main():
    ...
    print(f"\nFiles generated:")
    print(f"- research_domains_temporal_analysis_FIXED.png")
    print(f"- research_papers_comparison_original_vs_fixed.png")
