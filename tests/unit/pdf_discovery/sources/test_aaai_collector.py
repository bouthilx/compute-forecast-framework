...
class TestAAICollector:
    ...
    def test_make_request_success(self, collector):
        ...
        with patch('requests.get', return_value=mock_response) as mock_get:
            with patch('time.time', side_effect=[101, 102]):  # Time has passed since last request
    ...
    def test_make_request_with_rate_limiting(self, collector):
        ...
        with patch('requests.get', return_value=mock_response) as mock_get:
            with patch('time.time', side_effect=[100.1, 100.6]):  # Only 0.1s passed, need to wait
                with patch('time.sleep') as mock_sleep:
                    response = collector._make_request("http://test.com")
    ...
    def test_make_request_max_retries_exceeded(self, collector):
        ...
        with patch('requests.get', side_effect=requests.RequestException("Persistent error")):
            with patch('time.sleep'):
                with pytest.raises(requests.RequestException):
                    collector._make_request("http://test.com")
    
    def test_search_by_title_success(self, collector, search_response_html, article_response_html):
        ...
        with patch.object(collector, '_make_request') as mock_request:
            ...
            result = collector._search_by_title("Deep Learning for Natural Language Processing", 2024)
    ...
    def test_search_by_authors_success(self, collector, search_response_html, article_response_html):
    ...
    def test_discover_single_success(self, collector, sample_paper, search_response_html, article_response_html):
        ...
        with patch.object(collector, '_make_request') as mock_request:
            ...
            assert pdf_record.pdf_url == "https://ojs.aaai.org/index.php/AAAI/article/download/12345/98765"
    ...
    def test_caching(self, collector, search_response_html, article_response_html):
        ...
        with patch.object(collector, '_make_request') as mock_request:
            ...
            # First search
            result1 = collector._search_by_title("Deep Learning for Natural Language Processing", 2024)
            
            # Second search should use cache
            result2 = collector._search_by_title("Deep Learning for Natural Language Processing", 2024)
