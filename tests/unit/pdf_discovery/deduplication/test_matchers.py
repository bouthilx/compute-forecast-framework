...
class TestIdentifierNormalizer:
    ...
    def test_normalize_doi_valid(self):
        ...
        assert (
            normalizer.normalize_doi("https://doi.org/10.1234/example.doi")
            == "10.1234/example.doi"
        )
        assert (
            normalizer.normalize_doi("http://dx.doi.org/10.1234/example.doi")
            == "10.1234/example.doi"
        )
        assert (
            normalizer.normalize_doi("DOI: 10.1234/example.doi")
            == "10.1234/example.doi"
        )
        assert (
            normalizer.normalize_doi("doi:10.1234/example.doi") == "10.1234/example.doi"
        )
    ...
    def test_normalize_arxiv_id_valid(self):
        ...
        assert (
            normalizer.normalize_arxiv_id("https://arxiv.org/abs/2301.12345")
            == "2301.12345"
        )
        assert (
            normalizer.normalize_arxiv_id("http://arxiv.org/pdf/2301.12345.pdf")
            == "2301.12345"
        )
        ...
        assert (
            normalizer.normalize_arxiv_id("https://arxiv.org/abs/cs.LG/0701001")
            == "cs.LG/0701001"
        )
    ...
    def test_extract_identifiers_from_url(self):
        ...
        # DOI URLs
        result = normalizer.extract_identifiers_from_url(
...
class TestPaperFuzzyMatcher:
    ...
    def test_normalize_title(self):
        ...
        # Basic normalization
        assert (
            matcher.normalize_title("Deep Learning for NLP") == "deep learning for nlp"
        )
        assert matcher.normalize_title("  Deep  Learning  ") == "deep learning"

        # Remove common suffixes
        assert (
            matcher.normalize_title("Deep Learning (Extended Abstract)")
            == "deep learning"
        )
        assert (
            matcher.normalize_title("Deep Learning - Supplementary Material")
            == "deep learning"
        )
...
class TestVenueNormalizationIntegration:
    ...
    @patch("compute_forecast.pipeline.pdf_acquisition.discovery.deduplication.matchers.VenueNormalizer")
    ...
    def test_venue_confidence_threshold(self, mock_venue_normalizer_class):
        ...
        mock_normalizer.normalize_venue.side_effect = [
