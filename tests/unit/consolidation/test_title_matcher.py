...
class TestTitleMatcher:
    ...
    def test_arxiv_conference_variations(self):
        ...
        score, match_type = matcher.calculate_similarity(
            "Neural Networks for NLP (Extended Abstract)", "Neural Networks for NLP"
        )
        assert score >= 0.95
        ...
        score, match_type = matcher.calculate_similarity(
            "Vision Transformers at Scale [v2]", "Vision Transformers at Scale"
        )
        assert score >= 0.95
        assert match_type in ["exact", "high_confidence"]
    ...
    def test_punctuation_variations(self):
        ...
        score, match_type = matcher.calculate_similarity(
            'Learning to "Read" Images', "Learning to 'Read' Images"
        )
        assert score >= 0.90
        assert match_type in [
            "exact",
            "high_confidence",
        ]  # Could be exact after normalization
    ...
    def test_fuzzy_matches(self):
        ...
        score, match_type = matcher.calculate_similarity(
            "A Method for Deep Learning", "Method for Deep Learning"
        )
        assert score >= 0.85
    ...
    def test_year_safety_check(self):
        ...
        score1, match_type1 = matcher.calculate_similarity(
            "Neural Networks", "Neural Networks", year1=2023, year2=2023
        )

        # Same title, different years (should reduce confidence)
        score2, match_type2 = matcher.calculate_similarity(
            "Neural Networks", "Neural Networks", year1=2023, year2=2020
        )
    ...
    def test_is_similar_convenience_method(self):
        ...
        assert matcher.is_similar(
            "Deep Learning", "Deep Learning", min_confidence="high_confidence"
        )
