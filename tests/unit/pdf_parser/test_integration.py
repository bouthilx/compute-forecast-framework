...
class TestPDFParserIntegration:
    ...
    def test_extractor_cost_calculation(self):
        ...
        google_cost = processor._calculate_extraction_cost(
            "google_vision", "affiliation_extraction", mock_result
        )
        claude_cost = processor._calculate_extraction_cost(
            "claude_vision", "affiliation_extraction", mock_result
        )
        pymupdf_cost = processor._calculate_extraction_cost(
            "pymupdf", "full_text", mock_result
        )
    ...
    def test_validation_integration(self):
        ...
        paper_metadata = {"title": "Test Paper", "authors": ["Jane Doe", "John Smith"]}

        is_valid = processor.validator.validate_affiliations(
            good_result, paper_metadata
        )
    ...
    def test_error_handling_and_logging(self, mock_logger):
        ...
        failing_extractor.extract_first_pages.side_effect = Exception(
            "Mock extraction failed"
        )
        failing_extractor.extract_full_text.return_value = (
            ""  # Return empty string for full text
        )
    ...
    def test_processor_configuration(self):
        ...
        config = {"cache_dir": "./test_cache", "max_retries": 3, "timeout": 30}
