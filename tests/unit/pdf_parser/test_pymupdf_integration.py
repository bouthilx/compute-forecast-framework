...
class TestPyMuPDFIntegration:
    ...
    def test_register_and_use_pymupdf_extractor(self, mock_create_doc):
        ...
        with patch.object(
            self.processor.validator, "validate_affiliations", return_value=True
        ):
            # Process PDF
        ...
        assert (
            mock_doc.close.call_count >= 2
        )  # Called for both affiliation and full text
    ...
    def test_pymupdf_fallback_behavior(self, mock_create_doc):
        ...
        self.processor.register_extractor(
            "failing_extractor", failing_extractor, level=0
        )
        ...
        with patch.object(
            ...
            with patch.object(
                self.processor,
                "_extract_full_text",
                return_value="Successful extraction from PyMuPDF",
            ):
                result = self.processor.process_pdf(
                    self.test_pdf_path, self.paper_metadata
                )
    ...
    def test_pymupdf_cost_tracking(self):
        ...
        cost = self.processor._calculate_extraction_cost(
            "pymupdf", "affiliation_extraction", {}
        )
        assert cost == 0.0
