...
class TestEnhancedCollectionOrchestrator:
    ...
    def test_collect_from_all_sources_parallel_success(
        self, orchestrator, sample_query, sample_papers
    ):
    ...
    def test_collect_with_source_failure(self, orchestrator, sample_query):
        ...
        for source_name in ["openalex", "crossref", "google_scholar"]:
            orchestrator.sources[source_name].search_venue_batch = Mock(
    ...
    def test_collect_with_fallback_success(
        self, orchestrator, sample_query, sample_papers
    ):
        ...
        orchestrator.sources["openalex"].search_venue_batch = Mock(
            return_value=mock_response
        )
        ...
        assert (
            len(result.errors) == 0
        )  # No errors since the API returned success=False rather than throwing
    ...
    def test_rate_limiting_enforcement(self, orchestrator, sample_query):
        ...
        orchestrator.sources["semantic_scholar"].search_papers = Mock(
            return_value=mock_response
        )
        ...
        orchestrator.rate_limiter.wait_if_needed.assert_called_once_with(
            "semantic_scholar"
        )
