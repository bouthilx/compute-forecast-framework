...
class TestPDFDownloader:
    ...
    def mock_response(self):
        ...
        response.iter_content = Mock(return_value=[pdf_content[i:i+100] for i in range(0, len(pdf_content), 100)])
    ...
    def test_download_with_progress_callback(self, downloader, mock_response, mock_storage_manager):
    ...
    def test_timeout_error(self, downloader, mock_storage_manager):
        ...
        with patch.object(downloader.session, 'get', side_effect=Timeout("Connection timed out")):
            success, error = downloader.download_pdf(
                "test_paper_123",
    ...
    def test_connection_error(self, downloader, mock_storage_manager):
        ...
        with patch.object(downloader.session, 'get', side_effect=ConnectionError("Connection failed")):
            success, error = downloader.download_pdf(
                "test_paper_123",
    ...
    def test_retry_on_server_error(self, downloader, mock_storage_manager):
        ...
        mock_responses = [
            ...
            Mock(status_code=200, headers={"Content-Type": "application/pdf", "Content-Length": str(len(pdf_content))},
                 iter_content=Mock(return_value=[pdf_content[i:i+100] for i in range(0, len(pdf_content), 100)]))
        ]
    ...
    def test_max_retries_exceeded(self, downloader, mock_storage_manager):
        ...
        with patch.object(downloader.session, 'get', return_value=mock_response):
            with patch('time.sleep'):  # Don't actually sleep in tests
                success, error = downloader.download_pdf(
                    "test_paper_123",
                    "https://example.com/always_fails.pdf"
                )
    ...
    def test_html_content_detection(self, downloader, mock_storage_manager):
        ...
        html_content = b"<!DOCTYPE html><html><body>Error page" + b"x" * 2000 + b"</body></html>"
        mock_response.iter_content = Mock(return_value=[html_content[i:i+100] for i in range(0, len(html_content), 100)])
    ...
    def test_invalid_pdf_header(self, downloader, mock_storage_manager):
        ...
        mock_response = Mock()
        mock_response.status_code = 200
        mock_response.headers = {"Content-Type": "application/pdf", "Content-Length": "2000"}
        mock_response.text = ""
        
        # Return content without PDF header but with sufficient size
        invalid_content = b"This is not a PDF file" + b"\x00" * 2000
        mock_response.iter_content = Mock(return_value=[invalid_content[i:i+100] for i in range(0, len(invalid_content), 100)])
    ...
    def test_metadata_passed_to_storage(self, downloader, mock_storage_manager):
        ...
        pdf_content = b"%PDF-1.4\n%Mock PDF content" + b"\x00" * 2000
        mock_response.iter_content = Mock(return_value=[pdf_content[i:i+100] for i in range(0, len(pdf_content), 100)])
