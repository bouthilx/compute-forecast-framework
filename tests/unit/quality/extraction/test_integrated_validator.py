...
class TestIntegratedExtractionValidator:
    ...
    def test_validate_single_extraction(self):
        ...
        result = self.validator.validate_extraction(
            self.paper, self.extraction, self.paper_group
        )
    ...
    def test_quality_calculation(self):
        ...
        result = self.validator.validate_extraction(
            self.paper, high_quality_extraction, self.paper_group
        )
    ...
    def test_outlier_detection_integration(self):
        ...
        result = self.validator.validate_extraction(
            self.paper, outlier_extraction, large_group
        )

        # Should detect outliers
        assert len(result.outlier_fields) > 0
        assert (
            "gpu_hours" in result.outlier_fields
            or "training_time" in result.outlier_fields
        )

    def test_recommendations_generation(self):
        """Test recommendation generation."""
        # Incomplete extraction
        incomplete_extraction = ComputationalAnalysis(gpu_hours=100)

        result = self.validator.validate_extraction(self.paper, incomplete_extraction)
    ...
    def test_calibration_integration(self):
        ...
        with patch.object(self.validator, "_has_manual_data", return_value=True):
            with patch.object(
                self.validator, "_separate_manual_auto", return_value=({}, {})
            ):
                with patch.object(
