...
class TestInterruptionRecoveryEngine:
    ...
    def test_detect_interruption_type_api_failure(
        self, recovery_engine, state_manager, sample_session
    ):
    ...
    def test_detect_interruption_type_process_termination(
        self, recovery_engine, state_manager
    ):
        """Test detection of process termination interruption"""
        # Create session with old last activity time
        venues = [
            VenueConfig(venue_name="CVPR", target_years=[2023], max_papers_per_year=50)
        ]
        session_id = state_manager.create_session(
            target_venues=venues,
            target_years=[2023],
    ...
    def test_max_recovery_attempts(self, recovery_engine, sample_session):
        ...
        recovery_engine._recovery_attempts[sample_session] = (
            recovery_engine.max_recovery_attempts
        )
        ...
        assert result.success is False
        assert any(
            "Maximum recovery attempts" in error for error in result.resume_errors
        )
    ...
    def test_error_handling_in_recovery(self, recovery_engine, state_manager):
        ...
        with patch.object(
            state_manager, "get_recovery_plan", side_effect=Exception("Test error")
        ):
            result = recovery_engine.resume_interrupted_session(session_id)
            ...
            assert len(result.resume_errors) > 0
            assert any(
                "Recovery engine error" in error for error in result.resume_errors
            )
    ...
    def test_performance_requirement_validation(self, recovery_engine, sample_session):
        ...
        assert not any(
            "exceeded 5-minute requirement" in warning
            for warning in result.resume_warnings
        )
    ...
    def test_multiple_interruption_types(self, recovery_engine, state_manager):
        ...
        for interruption_type in interruption_types:
            ...
            venues = [
                VenueConfig(
                    venue_name="CVPR", target_years=[2023], max_papers_per_year=50
                )
            ]
            session_id = state_manager.create_session(
                target_venues=venues,
