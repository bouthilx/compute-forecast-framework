...
class MockJournalScraperImpl(JournalPublisherScraper):
    ...
    def search_papers(
        self, journal: str, keywords: List[str], year_range
    ) -> List[Paper]:
        papers = []
        for i in range(3):
            papers.append(
...
class TestScrapingResult:
    ...
    def test_success_result(self):
        result = ScrapingResult.success_result(
            papers_count=10, metadata={"venue": "NeurIPS"}
        )
    ...
    def test_failure_result(self):
        ...
        result = ScrapingResult.failure_result(
            errors=errors, metadata={"venue": "ICML"}
        )
...
class TestBaseScraper:
    ...
    def test_scrape_multiple_venues(self):
        ...
        with patch("time.sleep"):  # Skip delays in tests
            results = scraper.scrape_multiple_venues(
                {"NeurIPS": [2023, 2024], "ICML": [2023], "InvalidVenue": [2023]}
            )
    ...
    def test_scrape_multiple_venues_with_error(self):
        ...
        with patch("time.sleep"):
            results = scraper.scrape_multiple_venues({"error_venue": [2023]})
