...
class TestContractValidationIntegration:
    ...
    def test_end_to_end_analysis_validation(
        self, contract_validator, realistic_analyses
    ):
        ...
        assert (
            passed == 7
        )  # The 5 high quality + 2 low confidence (warnings don't fail)
    ...
    def test_pipeline_flow_validation(
        self, pipeline_validator, realistic_papers, realistic_analyses
    ):
        """Test validation through complete pipeline flow."""
        # Stage 1: Collection to Analysis
        collection_report = pipeline_validator.validate_collection_to_analysis(
            realistic_papers
        )
    ...
    def test_contract_test_suite_execution(self, test_suite):
        ...
        assert (
            valid_results["passed"] == valid_results["total_tests"]
        )  # All valid tests pass
    ...
    def test_contract_validation_with_real_world_scenarios(self, pipeline_validator):
        ...
        web_scraped_papers = [
            {
                ...
                "authors": [
                    "John Doe",
                    "Jane Smith",
                ],  # Strings instead of Author objects
        ...
        ml_analyses = [
            ...
            },
            {
                ...
                "resource_metrics": {
                    f"metric_{i}": i for i in range(60)
                },  # Too many metrics
            },
        ]
        ...
        for analysis_data in ml_analyses:
            contract = pipeline_validator.contract_validator.contracts[
            ...
            if analysis_data.get("computational_richness") == "high":
                assert any(
                    v.violation_type == ContractViolationType.INVALID_TYPE
                    for v in violations
                )
    ...
    def test_cross_component_integration(self, contract_validator, pipeline_validator):
        ...
        analysis_validation = contract_validator.validate_computational_analysis(
            analysis, paper
        )
