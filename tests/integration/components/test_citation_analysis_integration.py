...
class TestCitationAnalysisIntegration:
    ...
    def test_end_to_end_citation_analysis(self, venue_configs, realistic_papers):
        ...
        assert (
            len(analysis_report.breakthrough_candidates) >= 4
        ...
        filter_result = analyzer.filter_papers_by_citations(
            realistic_papers, preserve_breakthroughs=True
        )

        assert filter_result.filtered_count < filter_result.original_count
        assert (
            filter_result.breakthrough_preservation_rate > 0.9
        )  # Most breakthroughs preserved

        # Step 3: Validate filtering quality
        quality_report = analyzer.validate_filtering_quality(
            realistic_papers, filter_result.papers_above_threshold
        )

        assert (
            quality_report.venue_coverage_rate > 0.5
        )  # At least half venues preserved
        assert (
            quality_report.impact_preservation_rate > 0.8
        )  # Most high-impact preserved
        assert (
            quality_report.citation_improvement_ratio > 1.0
        )  # Better average citations
    ...
    def test_year_based_threshold_adaptation(self, venue_configs):
        ...
        with patch(
            ...
            with patch(
                "compute_forecast.pipeline.metadata_collection.processors.adaptive_threshold_calculator.datetime"
            ) as mock_datetime:
        ...
        # Calculate thresholds
        threshold_2023 = calculator.calculate_venue_threshold(
            "ICML", 2023, papers_2023, "tier1"
        )
        threshold_2020 = calculator.calculate_venue_threshold(
            "ICML", 2020, papers_2020, "tier1"
        )
