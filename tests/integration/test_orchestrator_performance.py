...
class TestOrchestratorPerformance:
    ...
    def mock_responses(self):
        ...
        return {
            ...
            "crossref": {
                "message": {
                    "items": [
                        {"DOI": "10.1/1", "title": ["Paper 5"], "published-print": {"date-parts": [[2023]]}},
                        {"DOI": "10.1/2", "title": ["Paper 6"], "published-print": {"date-parts": [[2023]]}}
                    ],
                    "total-results": 2
                }
            },
    ...
    @pytest.mark.asyncio
    async def test_parallel_performance_improvement(self, mock_responses, mock_delay_responses):
        """Test that parallel collection is at least 60% faster than sequential."""
        orchestrator = EnhancedOrchestrator()
        
        # Mock each source with delayed responses
        with patch('src.data.sources.semantic_scholar.SemanticScholarSource.search') as mock_ss, \
             ...
             patch('src.data.sources.google_scholar.GoogleScholarSource.search') as mock_gs:
            
            # Configure mocks with delays to simulate real API calls
            mock_ss.return_value = await mock_delay_responses(mock_responses["semantic_scholar"], 0.5)
            mock_oa.return_value = await mock_delay_responses(mock_responses["openalex"], 0.5)
            mock_cr.return_value = await mock_delay_responses(mock_responses["crossref"], 0.5)
            mock_gs.return_value = await mock_delay_responses(mock_responses["google_scholar"], 0.5)
            
            # Test sequential collection
            start_sequential = time.time()
            sequential_results = await orchestrator.collect_sequential("machine learning", max_results=10)
            ...
            parallel_results = await orchestrator.collect_parallel("machine learning", max_results=10)
            ...
            # Assertions
            assert improvement >= 60, f"Expected at least 60% improvement, got {improvement:.1f}%"
            assert len(parallel_results["papers"]) == len(sequential_results["papers"])
            assert parallel_results["total_results"] == sequential_results["total_results"]
    ...
    async def test_rate_limiting_behavior(self, mock_responses):
        ...
        with patch('src.data.sources.google_scholar.GoogleScholarSource.search') as mock_gs:
            # Track call times
            call_times = []
            ...
            for _ in range(3):
                await orchestrator.sources["google_scholar"].search("test query", max_results=5)
            
            # Check that calls are properly spaced (Google Scholar has 2s delay)
            if len(call_times) > 1:
                for i in range(1, len(call_times)):
                    time_diff = call_times[i] - call_times[i-1]
                    assert time_diff >= 1.9, f"Rate limit not enforced: {time_diff}s between calls"
    ...
    async def test_mock_api_responses_validity(self, mock_responses):
        ...
        assert all(key in paper for paper in ss_response["data"] for key in ["paperId", "title", "year"])
        
        # Test OpenAlex format
        oa_response = mock_responses["openalex"]
        assert "results" in oa_response
        assert all(key in paper for paper in oa_response["results"] for key in ["id", "title", "publication_year"])
        
        # Test CrossRef format
        cr_response = mock_responses["crossref"]
        assert "message" in cr_response
        assert "items" in cr_response["message"]
        assert all(key in paper for paper in cr_response["message"]["items"] for key in ["DOI", "title"])
        
        # Test Google Scholar format
        gs_response = mock_responses["google_scholar"]
        assert isinstance(gs_response, list)
        assert all(key in paper for paper in gs_response for key in ["title", "link", "year"])