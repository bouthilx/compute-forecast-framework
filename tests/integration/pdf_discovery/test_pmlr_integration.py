...
class TestPMLRIntegration:
    ...
    def test_framework_discovery_with_pmlr(self, mock_get, framework, sample_papers):
        ...
        aistats_record = next(
            r for r in result.records if r.paper_id == "aistats_2024_bo"
        )
        assert (
            aistats_record.pdf_url == "https://proceedings.mlr.press/v238/jones24b.pdf"
        )
    ...
    def test_pmlr_statistics_through_framework(self, framework, sample_papers):
        ...
        with patch("requests.get") as mock_get:
            ...
            mock_response.status_code = 200
            mock_response.text = (
                '<html><body><a href="/v202/test.html">Test Paper</a></body></html>'
            )
            mock_get.return_value = mock_response
