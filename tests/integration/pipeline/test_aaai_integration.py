...
import pytest
from compute_forecast.data.sources.scrapers.registry import get_registry
...
class TestAAAIIntegration:
    ...
    def test_aaai_scraper_live_collection(self, registry, config):
        ...
        assert len(papers) <= config.batch_size, f"Too many papers: {len(papers)} > {config.batch_size}"
