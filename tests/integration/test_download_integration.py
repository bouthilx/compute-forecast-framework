...
class TestDownloadIntegration:
    ...
    def test_download_command_basic(self, sample_papers, temp_dir, mock_pdf_content):
        ...
        mock_response.text = ""
        mock_response.iter_content = Mock(return_value=[mock_pdf_content[i:i+1000] for i in range(0, len(mock_pdf_content), 1000)])
        
        with patch('requests.Session.get', return_value=mock_response):
            with patch.dict('os.environ', {'LOCAL_CACHE_DIR': str(temp_dir / "cache")}):
                result = runner.invoke(
                    app,
                    ["download", "--papers", str(sample_papers), "--no-progress"]
                )
    ...
    def test_resume_functionality(self, sample_papers, temp_dir, mock_pdf_content):
        ...
        with patch('requests.Session.get', return_value=mock_response) as mock_get:
            with patch.dict('os.environ', {'LOCAL_CACHE_DIR': str(temp_dir / "cache")}):
                with patch('compute_forecast.cli.commands.download.get_download_state_path', return_value=checkpoint_dir / "download_progress.json"):
                    result = runner.invoke(
                        app,
                        ["download", "--papers", str(sample_papers), "--resume", "--no-progress"]
                    )
        ...
        with patch('requests.Session.get', return_value=mock_response) as mock_get:
            with patch.dict('os.environ', {'LOCAL_CACHE_DIR': str(temp_dir / "cache")}):
                with patch('compute_forecast.cli.commands.download.get_download_state_path', return_value=checkpoint_dir / "download_progress.json"):
                    result = runner.invoke(
                        app,
                        ["download", "--papers", str(sample_papers), "--retry-failed", "--no-progress"]
                    )
    ...
    def test_parallel_downloads(self, temp_dir):
            ...
            "papers": [
                {
                    ...
                    "authors": [{"name": f"Author {i}", "affiliations": [], "email": ""}],
                    "venue": "Test Conference",
                    "year": 2024,
                    "urls": [
                        {
        ...
        for paper_data in papers_data["papers"]:
            ...
            paper.processing_flags["selected_pdf_url"] = paper_data["urls"][0]["data"]["url"]
            papers.append(paper)
    ...
    def test_verbosity_levels(self, sample_papers, temp_dir, mock_pdf_content):
        ...
        with patch('requests.Session.get', return_value=mock_response):
            with patch.dict('os.environ', {'LOCAL_CACHE_DIR': str(temp_dir / "cache")}):
                ...
                result = runner.invoke(
                    app,
                    ["download", "--papers", str(sample_papers), "-vv", "--no-progress"]
                )
                
                assert result.exit_code == 0
                # Should see DEBUG level logs
                assert "DEBUG" in result.output or "Making HTTP request" in result.output