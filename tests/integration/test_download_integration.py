...
class TestDownloadIntegration:
    ...
    def test_download_command_basic(self, sample_papers, temp_dir, mock_pdf_content):
        ...
        with patch(
            "compute_forecast.workers.pdf_downloader.PDFDownloader._create_session"
        ) as mock_create:
            mock_session = Mock()
    ...
    def test_resume_functionality(self, sample_papers, temp_dir, mock_pdf_content):
        ...
        assert (paper_subdir / "test_paper_1.pdf").exists(), (
            f"File not created at {paper_subdir / 'test_paper_1.pdf'}"
        )
                    ...
                    for i, line in enumerate(output_lines):
                        if "To Download" in line and i + 2 < len(output_lines):
                            ...
                            if len(numbers) >= 2:
                                assert (
                                    numbers[1] == "1"
                                )  # To Download column should be 1
                                table_found = True
                                break
