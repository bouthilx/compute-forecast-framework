...
class TestCollectionQualityIntegration:
    ...
    def test_completeness_validator_individually(self):
        ...
        with TemporaryDirectory() as tmp_dir:
            ...
            config = QualityConfig(
                stage="collection",
                skip_checks=["consistency", "accuracy", "coverage"]
            )
            report = runner.run_checks("collection", data_file, config)
            ...
            critical_issues = [i for i in completeness_result.issues if i.level.name == "CRITICAL"]
    ...
    def test_consistency_validator_individually(self):
        ...
        with TemporaryDirectory() as tmp_dir:
            ...
            config = QualityConfig(
                stage="collection",
                skip_checks=["completeness", "accuracy", "coverage"]
            )
            report = runner.run_checks("collection", data_file, config)
