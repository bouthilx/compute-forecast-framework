...
class TestPerformanceProfile:
    ...
    def test_add_snapshot(self):
        ...
        snapshot1 = ResourceSnapshot(
            timestamp=time.time(), cpu_percent=30.0, memory_mb=1000.0
        )

        snapshot2 = ResourceSnapshot(
            timestamp=time.time() + 1, cpu_percent=50.0, memory_mb=1200.0
        )
...
class TestPerformanceMonitor:
    ...
    def test_start_phase_monitoring(self, monitor):
        ...
        assert isinstance(
            monitor.profiles[PipelinePhase.COLLECTION], PerformanceProfile
        )
