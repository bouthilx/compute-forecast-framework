...
class TestCitationAnalyzer:
    ...
    def test_analyze_citation_distributions(self, analyzer, sample_papers):
        ...
        with patch('src.data.processors.citation_analyzer.datetime') as mock_datetime:
    ...
    def test_filter_papers_by_citations_with_breakthrough(self, analyzer, sample_papers):
        ...
        with patch.object(analyzer.breakthrough_detector, 'detect_breakthrough_papers', 
                         return_value=[mock_breakthrough]):
            result = analyzer.filter_papers_by_citations(sample_papers, preserve_breakthroughs=True)
    ...
    def test_filter_papers_by_citations_without_breakthrough(self, analyzer, sample_papers):
        """Test citation filtering without breakthrough preservation."""
        result = analyzer.filter_papers_by_citations(sample_papers, preserve_breakthroughs=False)
    ...
    def test_detect_breakthrough_papers(self, analyzer, sample_papers):
        ...
        with patch('src.data.processors.breakthrough_detector.datetime') as mock_datetime:
            mock_datetime.now.return_value = datetime(2024, 1, 1)
            ...
            assert any(bp.paper.paper_id == "breakthrough1" for bp in breakthrough_papers)
    ...
    def test_calculate_adaptive_threshold(self, analyzer, sample_papers):
        ...
        threshold = analyzer.calculate_adaptive_threshold("NeurIPS", 2023, neurips_papers)
    ...
    def test_papers_without_citations(self, analyzer):
        ...
        papers = [
            Paper(paper_id=str(i), title=f"Paper {i}", venue="Test", year=2023, citations=None, authors=[Author(name="Test")])
            for i in range(5)
        ]
    ...
    def test_performance_large_dataset(self, analyzer):
        ...
        for i in range(50000):
            large_papers.append(Paper(
                ...
                normalized_venue="NeurIPS" if i % 3 == 0 else "ICML" if i % 3 == 1 else "CVPR",
                year=2020 + (i % 4),
