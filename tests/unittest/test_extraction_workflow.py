...
class TestTaskQueue:
    ...
    def test_priority_ordering(self):
        ...
        low_task = ExtractionTask("low", "paper1", "content", TaskPriority.LOW, "analyst")
        high_task = ExtractionTask("high", "paper2", "content", TaskPriority.HIGH, "analyst")
        medium_task = ExtractionTask("med", "paper3", "content", TaskPriority.MEDIUM, "analyst")
    ...
    def test_get_tasks_by_status(self):
        ...
        pending_task = ExtractionTask("pending", "paper1", "content", TaskPriority.LOW, "analyst")
        running_task = ExtractionTask("running", "paper2", "content", TaskPriority.LOW, "analyst")
...
class TestValidationGate:
    ...
    def test_gate_validation_pass(self):
        ...
        gate = ValidationGate(
            "test_gate",
            lambda result: result.get("score", 0) > 50
        )
...
class TestWorkflowManager:
    ...
    def test_start_workflow(self, workflow_config, sample_task, mock_extraction_protocol):
...
class TestExtractionWorkflow:
    ...
    async def test_run_batch_extraction(self, extraction_workflow):
        ...
        tasks = [
            ExtractionTask(f"task_{i}", f"paper_{i}", "content", TaskPriority.MEDIUM, "analyst")
            for i in range(3)
        ]
...
class TestWorkflowEdgeCases:
    ...
    def test_concurrent_task_limit(self, workflow_config):
        ...
        # Add tasks
        for i in range(5):
            task = ExtractionTask(f"task_{i}", f"paper_{i}", "content", TaskPriority.MEDIUM, "analyst")
    ...
    async def test_analyzer_failure(self, extraction_workflow, sample_task):
        ...
        assert sample_task.status == TaskStatus.FAILED
        assert "analyzer" in sample_task.error.lower() or "failed" in sample_task.error.lower()