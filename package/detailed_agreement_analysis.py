...
def analyze_domain_specific_agreement():
    ...
    for dataset_domain, research_domain in domain_mapping.items():
        ...
        dataset_papers = {pid for pid, info in dataset_data['dataset_classifications'].items() 
                         if info['domain'] == dataset_domain and pid in overlap_papers}
        
        # Get papers classified as this domain by research classification
        research_papers = {pid for pid, domains in paper_research_domains.items() 
        ...
        if len(dataset_papers | research_papers) > 0:
            overall_agreement = len(both_agree) / len(dataset_papers | research_papers) * 100
        else:
            overall_agreement = 0
        ...
        print(f"  Dataset precision: {dataset_precision:.1f}% (how often dataset is confirmed by research)")
        print(f"  Research precision: {research_precision:.1f}% (how often research is confirmed by dataset)")
...
def analyze_false_positives_negatives():
    ...
    cv_dataset_papers = {pid for pid, info in dataset_data['dataset_classifications'].items() 
                        if info['domain'] == 'Computer Vision & Medical Imaging' and pid in overlap_papers}
    
    cv_research_papers = {pid for pid, domains in paper_research_domains.items() 
    ...
    rl_dataset_papers = {pid for pid, info in dataset_data['dataset_classifications'].items() 
                        if info['domain'] == 'Reinforcement Learning & Robotics' and pid in overlap_papers}
    
    rl_research_papers = {pid for pid, domains in paper_research_domains.items() 
...
def challenge_dataset_bias():
    ...
    print("   - Theoretical/methodological papers less likely to mention specific datasets")
    ...
    avg_research_domains = total_research_domain_assignments / len(paper_research_counts)
    
    print(f"\\n5. MULTI-DOMAIN EVIDENCE:")
    print(f"   - Research classification: {avg_research_domains:.2f} domains per paper on average")
    print(f"   - {papers_with_multiple_research_domains}/{len(paper_research_counts)} papers have multiple research domains")
    print(f"   - Dataset classification: typically 1 primary domain")
    print(f"   - Research domains may capture interdisciplinary nature better")
...
def examine_research_domain_quality():
    ...
    for category, entries in domain_strength_analysis.items():
        if entries:
            avg_match_strength = sum(e['match_strength'] for e in entries) / len(entries)
            avg_justification_length = sum(e['justification_length'] for e in entries) / len(entries)
            ...
            print(f"  Average justification length: {avg_justification_length:.0f} chars")
...
def final_recommendation():
    ...
    research_precisions = [stats['research_precision'] for stats in domain_stats.values()]
    ...
    if avg_dataset_precision > 70 and avg_research_precision < 60:
        ...
        print("   Evidence: Datasets show higher precision when validated against research domains")
    elif avg_research_precision > avg_dataset_precision:
        print("❌ CHALLENGE dataset-based corrections")
        print("   Evidence: Research domains show higher precision when validated against datasets")
    else:
