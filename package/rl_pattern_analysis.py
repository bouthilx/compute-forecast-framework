...
def identify_rl_paper_groups():
                ...
                if isinstance(dataset, dict) and 'name' in dataset:
                    ...
                    if isinstance(name_data, dict):
                        dataset_text = (name_data.get('value', '') + ' ' + 
                                      name_data.get('justification', '')).lower()
            ...
            if not has_rl_data:
                ...
                if isinstance(description, dict):
                    desc_text = (description.get('value', '') + ' ' + 
                               description.get('justification', '')).lower()
                    
                    rl_matches = sum(1 for keyword in rl_keywords if keyword in desc_text)
    ...
    print(f"Total RL papers: {len(rl_with_datasets_envs) + len(rl_without_datasets_envs)}")
    
    return rl_with_datasets_envs, rl_without_datasets_envs, paper_research_domains

def analyze_other_research_domains(rl_with_data, rl_without_data, paper_research_domains):
    ...
    for domain, count in sorted(other_domains_with_data.items(), key=lambda x: x[1], reverse=True):
    ...
    for domain, count in sorted(other_domains_without_data.items(), key=lambda x: x[1], reverse=True):
    ...
    all_other_domains = set(other_domains_with_data.keys()) | set(other_domains_without_data.keys())
    
    for domain in sorted(all_other_domains):
        with_pct = other_domains_with_data.get(domain, 0) / total_with_data * 100
        without_pct = other_domains_without_data.get(domain, 0) / total_without_data * 100
...
def analyze_theoretical_vs_empirical(rl_with_data, rl_without_data):
    ...
    theoretical_indicators = [
        'theoretical', 'theory', 'mathematical', 'proof', 'theorem', 'lemma',
        'analysis', 'convergence', 'bound', 'complexity', 'optimal', 'optimality',
        'regret', 'sample complexity', 'pac', 'probably approximately correct'
    ]
    
    empirical_indicators = [
        'experiment', 'empirical', 'evaluation', 'benchmark', 'performance',
        'results', 'comparison', 'ablation', 'baseline', 'state-of-the-art',
    ...
    def classify_paper_type(paper_id):
        ...
        try:
            ...
            theoretical_matches = sum(1 for indicator in theoretical_indicators if indicator in full_text)
            empirical_matches = sum(1 for indicator in empirical_indicators if indicator in full_text)
    ...
    for paper_type, count in sorted(with_data_types.items(), key=lambda x: x[1], reverse=True):
    ...
    for paper_type, count in sorted(without_data_types.items(), key=lambda x: x[1], reverse=True):
    ...
    for paper_type in sorted(all_types):
        ...
        print(f"{paper_type:<12} {with_pct:<11.1f}% {without_pct:<11.1f}% {diff:+<11.1f}%")
...
def analyze_sample_papers(rl_with_data, rl_without_data, paper_research_domains):
    ...
    def get_paper_info(paper_id):
        ...
        other_domains = [d for d in paper_research_domains.get(paper_id, set()) 
...
def final_pattern_analysis():
    ...
    dl_with_pct = other_domains_with.get('Deep Learning & Neural Architectures', 0) / total_with * 100
    dl_without_pct = other_domains_without.get('Deep Learning & Neural Architectures', 0) / total_without * 100
    
    theory_with_pct = other_domains_with.get('Machine Learning Theory & Methods', 0) / total_with * 100
    theory_without_pct = other_domains_without.get('Machine Learning Theory & Methods', 0) / total_without * 100
    ...
    empirical_without_pct = paper_types_without.get('empirical', 0) / total_without * 100
    
    theoretical_with_pct = paper_types_with.get('theoretical', 0) / total_with * 100
    theoretical_without_pct = paper_types_without.get('theoretical', 0) / total_without * 100
    
    print(f"\n2. THEORETICAL VS EMPIRICAL:")
    print(f"   - RL with data: {empirical_with_pct:.1f}% empirical, {theoretical_with_pct:.1f}% theoretical")
    print(f"   - RL without data: {empirical_without_pct:.1f}% empirical, {theoretical_without_pct:.1f}% theoretical")
    ...
    if theoretical_without_pct > theoretical_with_pct:
        ...
        print("   ❌ NOT CONFIRMED: Pattern doesn't support theoretical bias hypothesis")
    ...
    print(f"   - RL papers with datasets/envs: {len(rl_with_data)} ({len(rl_with_data)/(total_with+total_without)*100:.1f}%)")
    print(f"   - RL papers without datasets/envs: {len(rl_without_data)} ({len(rl_without_data)/(total_with+total_without)*100:.1f}%)")
    print(f"   → For compute projections, focus on the {len(rl_with_data)} papers with actual computational work")
