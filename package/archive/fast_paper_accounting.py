...
def create_fast_paper_accounting():
    ...
    print(f"Papers WITH analysis: {papers_with_analysis:,} ({papers_with_analysis/total_papers*100:.1f}%)")
    print(f"Papers WITHOUT analysis: {papers_without_analysis:,} ({papers_without_analysis/total_papers*100:.1f}%)")
    ...
    # Verify accounting
    accounted = len(research_only) + len(dataset_only) + len(both_methods) + neither_method
    print(f"Verification: {accounted:,} = {papers_with_analysis:,} ✓" if accounted == papers_with_analysis else f"ERROR: {accounted} ≠ {papers_with_analysis}")
    ...
    print(f"Difference:       {(total_research + total_datasets) - len(classified_papers):,} papers (the overlap)")
...
def show_domain_overlap_details():
    ...
    for category in ['Computer Vision & Medical Imaging', 'Natural Language Processing', 
    ...
    print("┌────────────────────────────────────┬─────────┬─────────┬─────────┬─────────┬─────────┐")
    print("│ Domain                             │Research │Dataset  │ Overlap │ Union   │Coverage │")
    print("├────────────────────────────────────┼─────────┼─────────┼─────────┼─────────┼─────────┤")
    ...
    for dataset_domain, research_domain in domain_mapping.items():
        ...
        coverage = len(overlap) / len(research_papers) * 100 if len(research_papers) > 0 else 0
        
        print(f"│ {dataset_domain[:30]:<30} │ {len(research_papers):>7} │ {len(dataset_papers):>7} │ {len(overlap):>7} │ {len(union):>7} │ {coverage:>6.1f}% │")
    
    print("└────────────────────────────────────┴─────────┴─────────┴─────────┴─────────┴─────────┘")
    
    print(f"\nINTERPRETATION:")
    print(f"• Research: Papers classified by research domain analysis")
    print(f"• Dataset: Papers classified by dataset/environment detection")  
    print(f"• Overlap: Papers found by BOTH methods")
    print(f"• Union: Total unique papers found by either method")
    print(f"• Coverage: % of research domain papers captured by dataset method")
