...
def analyze_domain_mappings():
    ...
    for domain_entry in raw_domains:
        ...
        if domain_name not in domain_to_category:
            ...
            if any(kw in domain_lower for kw in cv_keywords):
                ...
                auto_mapped[category].append((domain_name, [kw for kw in cv_keywords if kw in domain_lower]))
            elif any(kw in domain_lower for kw in nlp_keywords):
                category = 'Natural Language Processing'
                domain_to_category[domain_name] = category
                auto_mapped[category].append((domain_name, [kw for kw in nlp_keywords if kw in domain_lower]))
            elif any(kw in domain_lower for kw in rl_keywords):
                category = 'Reinforcement Learning & Robotics'
                domain_to_category[domain_name] = category
                auto_mapped[category].append((domain_name, [kw for kw in rl_keywords if kw in domain_lower]))
            elif any(kw in domain_lower for kw in graph_keywords):
                category = 'Graph Learning & Network Analysis'
                domain_to_category[domain_name] = category
                auto_mapped[category].append((domain_name, [kw for kw in graph_keywords if kw in domain_lower]))
            else:
    ...
    for category, domains_and_keywords in auto_mapped.items():
        ...
        domains_sorted = sorted(domains_and_keywords, 
                              key=lambda x: domain_frequencies[x[0]], reverse=True)
        ...
        for domain_name, matched_keywords in domains_sorted:
            ...
            if category == 'Computer Vision & Medical Imaging':
                ...
                if any(suspicious in domain_name.lower() for suspicious in 
                       ['language', 'text', 'nlp', 'linguistic', 'dialogue', 'speech', 
                        'reinforcement', 'policy', 'agent', 'graph', 'network', 'node']):
                    suspicious_mappings.append((domain_name, category, matched_keywords, freq))
            
            elif category == 'Natural Language Processing':
                # Check for non-NLP domains
                if any(suspicious in domain_name.lower() for suspicious in 
                       ['vision', 'image', 'visual', 'segmentation', 'detection', 
                        'reinforcement', 'policy', 'agent', 'robotics', 'control',
                ...
                if any(suspicious in domain_name.lower() for suspicious in 
                       ['vision', 'image', 'visual', 'language', 'text', 'nlp',
                        'graph', 'network', 'node', 'edge']):
                    suspicious_mappings.append((domain_name, category, matched_keywords, freq))
                ...
                if any(suspicious in domain_name.lower() for suspicious in 
                       ['vision', 'image', 'visual', 'language', 'text', 'nlp',
                        'reinforcement', 'policy', 'agent', 'robotics']):
                    suspicious_mappings.append((domain_name, category, matched_keywords, freq))
