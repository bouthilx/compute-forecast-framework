...
def print_full_corrected_venue_list(corrected_data):
    ...
    sorted_venues = sorted(merged_venues.items(), 
    ...
    for venue, info in sorted_venues:
        ...
        if by_year:
            year_breakdown = ", ".join([f"{y}:{c}" for y, c in sorted(by_year.items()) if c > 0])
    ...
    print(f"Top 20 venues represent: {top_20_papers} papers ({top_20_papers/total_papers:.1%})")
    print(f"Top 50 venues represent: {top_50_papers} papers ({top_50_papers/total_papers:.1%})")
    
    # Merged venue impact
    total_constituent_venues = sum(len(info['merged_from']) for _, info in sorted_venues if info['is_merged'])
    ...
    merged_venues_only = [(venue, info) for venue, info in sorted_venues if info['is_merged']]
    ...
    for i, (venue, info) in enumerate(merged_venues_only[:10], 1):
        ...
        print(f"{i:2d}. {venue:30s} | {papers:3d} papers from {constituent_count} variants")
    ...
    standalone_venues_only = [(venue, info) for venue, info in sorted_venues if not info['is_merged']]
...
def create_venue_mapping_for_worker6(corrected_data):
    ...
    print(f"Worker 6 venue mapping saved to: worker6_venue_mapping.json")
