                                ...
                                if 'datasets' in extractions:
                                    ...
                                    if isinstance(datasets, list) and len(datasets) > 0:
                                        ...
                                        for dataset in datasets:
                                            if isinstance(dataset, dict) and dataset.get('name', {}).get('value'):
                                                dataset_count += 1
                            break
                        except:
                            continue
...
def calculate_empirical_coverage_by_domain(data):
    ...
    for category in main_research_categories + ['Other research domains']:
        ...
        if len(domain_papers_with_analysis) > 0:
            coverage = len(domain_papers_with_datasets) / len(domain_papers_with_analysis)
            domain_coverage[category] = {
                'total_papers': len(domain_papers_with_analysis),
                'empirical_papers': len(domain_papers_with_datasets),
                'theoretical_papers': len(domain_papers_with_analysis) - len(domain_papers_with_datasets),
                'empirical_coverage': coverage
            }
            
            print(f"{category}:")
            print(f"  Total papers: {len(domain_papers_with_analysis)}")
            print(f"  Empirical (with datasets): {len(domain_papers_with_datasets)} ({coverage*100:.1f}%)")
            print(f"  Theoretical (domains only): {len(domain_papers_with_analysis) - len(domain_papers_with_datasets)} ({(1-coverage)*100:.1f}%)")
            print()
...
def calculate_correction_factors(domain_coverage):
    ...
    total_empirical = sum(stats['empirical_papers'] for stats in domain_coverage.values())
    ...
    for category, stats in domain_coverage.items():
        ...
        correction_factors[category] = {
            ...
            'interpretation': 'under-represented' if correction_factor > 1 else 'over-represented' if correction_factor < 1 else 'balanced'
        }
...
def apply_dataset_corrections(data, correction_factors):
    ...
    for year in years:
        ...
        corrected_year_data[year]['WITHOUT AI analysis'] = raw_year_data[year]['WITHOUT AI analysis']
        corrected_year_data[year]['NO domain classification'] = raw_year_data[year]['NO domain classification']
        
        # Apply correction factors to research domains
        for category in all_research_categories:
            raw_count = raw_year_data[year][category]
            correction_factor = correction_factors.get(category, {}).get('correction_factor', 1.0)
            corrected_count = raw_count * correction_factor
            corrected_year_data[year][category] = corrected_count
            
            print(f"{year} {category}: {raw_count:.0f} → {corrected_count:.1f} (factor: {correction_factor:.3f})")
    
    return raw_year_data, corrected_year_data, total_papers_by_year

def create_comparison_visualization(raw_year_data, corrected_year_data, total_papers_by_year):
    ...
    for i, year in enumerate(years):
        total_research_raw = sum(raw_year_data[year][cat] for cat in research_categories)
        total_research_corrected = sum(corrected_year_data[year][cat] for cat in research_categories)
        
        for j, category in enumerate(research_categories):
            raw_count = raw_year_data[year][category]
            corrected_count = corrected_year_data[year][category]
            
            raw_data[j, i] = raw_count / total_research_raw * 100 if total_research_raw > 0 else 0
            corrected_data[j, i] = corrected_count / total_research_corrected * 100 if total_research_corrected > 0 else 0
    ...
    ax1.set_title('Raw Domain Distribution (No Corrections)', fontsize=14, fontweight='bold')
    ax1.set_xlabel('Year')
    ...
    # Corrected data
    ax2.stackplot(years, corrected_data, labels=research_categories, colors=colors, alpha=0.8)
    ax2.set_title('Dataset-Corrected Domain Distribution', fontsize=14, fontweight='bold')
    ax2.set_xlabel('Year')
...
def main():
    ...
    raw_year_data, corrected_year_data, total_papers_by_year = apply_dataset_corrections(data, correction_factors)
    
    # Step 5: Create comparison visualization
    raw_data, corrected_data = create_comparison_visualization(raw_year_data, corrected_year_data, total_papers_by_year)
    ...
    print(f"Results saved to: dataset_based_corrections_results.json")
    print(f"Comparison chart saved to: dataset_corrections_comparison.png")
