...
def extract_all_domains():
    ...
    for i, paper_json in enumerate(papers_data):
        if i % 500 == 0:
            print(f"Processing paper {i}/{len(papers_data)} ({papers_with_analysis} with analysis)")
...
def create_final_taxonomy(all_domains):
    ...
    mila_taxonomy = {
        'Computer Vision & Medical Imaging': {
            'keywords': [
                'computer vision', 'vision', 'image', 'visual', 'medical imaging',
                'segmentation', 'detection', 'classification', 'mri', 'ultrasound',
                'spinal cord', 'medical', 'healthcare', 'radiology', 'tomography',
                'microscopy', 'pathology', 'diagnostic', 'clinical'
            ],
            'description': 'Visual understanding, medical image analysis, diagnostic imaging'
        },
        'Natural Language Processing': {
            'keywords': [
                'nlp', 'natural language', 'language', 'text', 'linguistic',
                'translation', 'sentiment', 'dialogue', 'conversation', 'chatbot',
                'question answering', 'summarization', 'parsing', 'generation',
                'bert', 'transformer', 'gpt', 'llm', 'large language model'
            ],
        ...
        'Reinforcement Learning & Robotics': {
            'keywords': [
                ...
                'robotics', 'robotic', 'manipulation', 'control', 'locomotion',
                'bipedal', 'imitation learning', 'autonomous', 'navigation',
                'tactile', 'sensing', 'actuator', 'motor'
            ],
        ...
        'Deep Learning & Neural Architectures': {
            'keywords': [
                ...
                'transformer', 'attention', 'autoencoder', 'generative',
                'adversarial', 'gan', 'vae', 'diffusion', 'architecture',
        ...
        'Graph Learning & Network Analysis': {
            'keywords': [
                'graph', 'network', 'node', 'edge', 'graph neural network', 'gnn',
                'graph transformer', 'graph signal processing', 'temporal graph',
                'social network', 'knowledge graph', 'molecular graph',
        ...
        'Machine Learning Theory & Methods': {
            'keywords': [
                'machine learning', 'learning theory', 'statistical learning',
                'bayesian', 'probabilistic', 'uncertainty', 'active learning',
                'transfer learning', 'meta learning', 'few shot', 'zero shot',
                'generalization', 'bias', 'fairness', 'interpretability'
            ],
            'description': 'ML foundations, theoretical analysis, learning paradigms'
        },
        'Software Engineering & Systems': {
            'keywords': [
                'software engineering', 'software', 'system', 'distributed',
                'parallel', 'scalability', 'performance', 'testing',
                'debugging', 'reproducibility', 'deployment', 'mlops',
        ...
        'Scientific Computing & Applications': {
            'keywords': [
                'astrophysics', 'astronomy', 'physics', 'chemistry', 'biology',
                'bioinformatics', 'computational biology', 'drug discovery',
                'climate', 'weather', 'materials science', 'quantum',
                'simulation', 'scientific computing', 'numerical methods'
            ],
            'description': 'AI for scientific discovery, computational science applications'
        },
        'Social & Economic Applications': {
            'keywords': [
                'social', 'economic', 'finance', 'trading', 'market',
                'recommendation', 'personalization', 'advertising',
                'social media', 'human behavior', 'psychology',
                'education', 'learning analytics', 'game theory'
            ],
...
def main():
    ...
    for category, paper_count in sorted(category_stats.items(), key=lambda x: x[1], reverse=True):
        print(f"{category}: {paper_count} papers")
        
        # Show top domains in this category
        category_domains = [(name, info) for name, info in classification.items() 
        ...
        if len(category_domains) > 5:
            ...
            print(f"  - ... and {len(category_domains)-5} more domains ({remaining} papers)")
        print()
    ...
    if unclassified:
        ...
        for domain in sorted(unclassified, key=lambda x: x['paper_count'], reverse=True)[:10]:
