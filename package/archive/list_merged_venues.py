...
def print_full_venue_list(final_venues):
    ...
    sorted_venues = sorted(final_venues.items(), 
    ...
    # Merged venue impact
    total_original_venues = sum(len(info['constituent_venues']) for _, info in sorted_venues if info['is_merged'])
    venues_eliminated = total_original_venues - merged_venues
    
    print(f"\nMerging impact:")
    print(f"Original venues before merging: {total_original_venues + standalone_venues}")
    print(f"Venues eliminated through merging: {venues_eliminated}")
    print(f"Reduction: {venues_eliminated/(total_original_venues + standalone_venues):.1%}")
...
def save_venue_list(final_venues):
    ...
    output_data = {
        ...
        'summary': {
            'total_venues': len(sorted_venues),
            'merged_venues': sum(1 for _, info in sorted_venues if info['is_merged']),
            'standalone_venues': sum(1 for _, info in sorted_venues if not info['is_merged']),
            'total_papers': sum(info['total_papers'] for _, info in sorted_venues)
        }
    }
