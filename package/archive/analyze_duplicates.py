...
def analyze_duplicates():
    ...
    # Load the data
    try:
        with open('/home/bouthilx/projects/preliminary_report/package/data/raw_collected_papers.json', 'r') as f:
            papers = json.load(f)
    ...
    exact_duplicates = {title: papers_list for title, papers_list in title_groups.items() if len(papers_list) > 1}
    
    print(f"   Unique normalized titles: {len(title_groups)}")
    print(f"   Titles with duplicates: {len(exact_duplicates)}")
    print(f"   Total duplicate papers: {sum(len(papers_list) - 1 for papers_list in exact_duplicates.values())}")
    ...
    hash_duplicates = {h: papers_list for h, papers_list in hash_groups.items() if len(papers_list) > 1}
    
    print(f"   Unique hashes: {len(hash_groups)}")
    print(f"   Hashes with duplicates: {len(hash_duplicates)}")
    print(f"   Total hash duplicate papers: {sum(len(papers_list) - 1 for papers_list in hash_duplicates.values())}")
    ...
    if similar_pairs:
        ...
        for i, (idx1, idx2, sim, p1, p2) in enumerate(similar_pairs[:3]):
            ...
            print(f"      Sources: {p1.get('source', 'unknown')} vs {p2.get('source', 'unknown')}")
            print()
    ...
    if duplicate_years:
        print(f"   Duplicates by year (top 5):")
    ...
    print(f"Exact title duplicates: {sum(len(papers_list) - 1 for papers_list in exact_duplicates.values()):,}")
    print(f"Hash duplicates: {sum(len(papers_list) - 1 for papers_list in hash_duplicates.values()):,}")
    print(f"Similar title pairs: {len(similar_pairs):,}")
    print(f"Cross-source duplicates: {cross_source_duplicates:,}")
    print(f"Same-source duplicates: {same_source_duplicates:,}")
    
    duplication_rate = (sum(len(papers_list) - 1 for papers_list in exact_duplicates.values()) / len(papers)) * 100
