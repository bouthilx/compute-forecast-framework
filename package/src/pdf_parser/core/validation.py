...
class AffiliationValidator:
    ...
    def __init__(self, min_confidence: float = 0.5, min_affiliations: int = 1, 
    ...
    def validate_affiliations(self, extraction_result: Dict, paper_metadata: Dict) -> bool:
        ...
        try:
            ...
            if confidence < self.min_confidence:
                logger.debug(f"Confidence {confidence} below threshold {self.min_confidence}")
                return False
            
            # Check affiliations count
            affiliations = extraction_result.get('affiliations', [])
            if len(affiliations) < self.min_affiliations:
                logger.debug(f"Only {len(affiliations)} affiliations, need {self.min_affiliations}")
                return False
            
            # Check text quality
            text = extraction_result.get('text', '')
            if len(text) < self.min_text_length:
                logger.debug(f"Text too short: {len(text)} chars, need {self.min_text_length}")
                return False
    ...
    def _validate_author_matching(self, extracted_text: str, author_names: List[str]) -> bool:
    ...
    def get_validation_summary(self, extraction_result: Dict, paper_metadata: Dict) -> Dict[str, Any]:
        ...
        summary['checks']['confidence_check'] = summary['confidence'] >= self.min_confidence
        summary['checks']['affiliation_count_check'] = summary['affiliation_count'] >= self.min_affiliations
        summary['checks']['text_length_check'] = summary['text_length'] >= self.min_text_length
