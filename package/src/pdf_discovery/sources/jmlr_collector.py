...
class JMLRCollector(BasePDFCollector):
    ...
    def _discover_single(self, paper: Paper) -> PDFRecord:
        ...
        is_tmlr = "tmlr" in venue or "transactions on machine learning research" in venue
    ...
    def _search_jmlr_website(self, paper: Paper) -> PDFRecord:
        ...
        try:
                ...
                if self._improved_fuzzy_title_match(paper_title, link_text):
                    ...
                    if href:
                        ...
                        if href.endswith('.pdf'):
                            pdf_url = href if href.startswith('http') else self.jmlr_base_url + href
                            ...
                            if match:
                                volume = match.group(1)
                                paper_id = match.group(2).replace('.html', '')
                                pdf_url = f"{self.jmlr_base_url}v{volume}/{paper_id}.pdf"
            ...
            logger.error(f"Error searching JMLR website for paper {paper.paper_id}: {e}")
            raise ValueError(f"Could not find JMLR paper {paper.paper_id} through website search: {e}")
