...
class ThresholdOptimizer:
    ...
    def optimize_thresholds(self, current_thresholds: QualityThresholds,
                          performance_data: QualityPerformanceMetrics) -> QualityThresholds:
        """
        ...
        self._record_optimization_step(current_thresholds, optimized_thresholds, 
                                     performance_data, gradients)
        
        return optimized_thresholds
    
    def calculate_optimization_gradient(self, performance_data: QualityPerformanceMetrics) -> Dict[str, float]:
        """
        ...
        efficiency_gap = self.config.target_collection_efficiency - performance_data.collection_efficiency
        ...
        if abs(recall_gap) > 0.01:
            gradients['recall_gradient'] = -recall_gap  # Negative because higher recall needs lower thresholds
        ...
        target_f1 = 2 * (self.config.target_precision * self.config.target_recall) / \
    ...
    def _apply_gradient_updates(self, current_thresholds: QualityThresholds,
        ...
        for threshold_name, gradient in threshold_gradients.items():
            ...
            momentum_update = (self.config.momentum * self.momentum_values[threshold_name] +
    ...
    def _calculate_threshold_gradients(self, performance_gradients: Dict[str, float]) -> Dict[str, float]:
        """
    ...
    def _get_adaptive_learning_rate(self) -> float:
        ...
        if len(self.optimization_history) >= 3:
            recent_changes = [step['threshold_changes'] for step in self.optimization_history[-3:]]
            
            # Check for oscillation (sign changes in updates)
            oscillation_detected = False
            for threshold_name in ['min_paper_quality_score', 'min_combined_quality_score']:
    ...
    def _apply_optimization_constraints(self, updates: Dict[str, float]) -> Dict[str, float]:
        """
        ...
        for threshold_name, new_value in updates.items():
            ...
            if threshold_name == 'min_citation_count':
                new_value = max(0, int(new_value))  # Citation count must be non-negative integer
    ...
    def _record_optimization_step(self, old_thresholds: QualityThresholds,
        ...
        step_record = {
            ...
            'threshold_changes': {
                'min_paper_quality_score': new_thresholds.min_paper_quality_score - old_thresholds.min_paper_quality_score,
                'min_combined_quality_score': new_thresholds.min_combined_quality_score - old_thresholds.min_combined_quality_score,
                'min_citation_count': new_thresholds.min_citation_count - old_thresholds.min_citation_count
            },
            'gradients': gradients,
            'learning_rate': self._get_adaptive_learning_rate()
        }
    ...
    def analyze_optimization_performance(self) -> Dict[str, Any]:
        ...
        f1_scores = [step['performance_before']['f1_score'] for step in self.optimization_history]
        ...
        if len(self.optimization_history) >= 5:
            ...
            for step in self.optimization_history[-5:]:
                total_change = sum(abs(change) for change in step['threshold_changes'].values())
                recent_changes.append(total_change)
            
            avg_recent_change = np.mean(recent_changes)
            convergence_status = 'converged' if avg_recent_change < 0.01 else 'converging'
        else:
            convergence_status = 'unknown'
        ...
        return {
            ...
            'average_recent_change': avg_recent_change if 'avg_recent_change' in locals() else 0.0
        }
    
    def suggest_threshold_bounds(self, performance_history: List[QualityPerformanceMetrics]) -> Dict[str, Tuple[float, float]]:
        """
        ...
        if precisions and recalls:
            ...
            if avg_precision < self.config.target_precision:
                suggestions['min_paper_quality_score'] = (0.1, 0.8)  # Need room to increase
            else:
                suggestions['min_paper_quality_score'] = (0.0, 0.6)  # Can be more permissive
