...
class OutlierDetector:
    ...
    def detect_outliers(self, 
                       values: List[float], 
                       method: str = "z_score",
                       field: Optional[str] = None) -> List[int]:
        """
    ...
    def contextualize_outlier(self, 
                            paper: Paper, 
                            field: str, 
    ...
    def _detect_z_score_outliers(self, values: np.ndarray, field: Optional[str] = None) -> List[int]:
    ...
    def _detect_iqr_outliers(self, values: np.ndarray, field: Optional[str] = None) -> List[int]:
        ...
        if field and field in self.field_thresholds:
            multiplier = self.field_thresholds[field].get("iqr_multiplier", self.iqr_multiplier)
    ...
    def _detect_combined_outliers(self, values: np.ndarray, field: Optional[str] = None) -> List[int]:
    ...
    def _check_corroborating_evidence(self, 
                                    paper: Paper, 
                                    extraction: Dict[str, Any],
                                    field: str,
                                    value: float) -> bool:
    ...
    def _check_field_consistency(self, 
                               extraction: Dict[str, Any],
                               field: str,
                               value: float) -> bool:
    ...
    def analyze_outlier_pattern(self, 
        ...
        for field, field_outliers in outliers_by_field.items():
            ...
            if years:
                temporal_patterns[field] = {
                    "min_year": min(years),
                    "max_year": max(years),
                    "concentration": "recent" if np.mean(years) > 2020 else "historical"
                }
