...
class IntegratedExtractionValidator:
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        from src.quality.quality_structures import AdaptationConfig
    ...
    def validate_extraction(self,
                          ...
                          paper_group: Optional[List[Paper]] = None) -> IntegratedValidationResult:
        """
        ...
        extraction_validation = self.extraction_validator.validate_extraction(paper, extraction)
        ...
        domain_check = self.consistency_checker.check_domain_consistency(paper, extraction_dict)
        ...
        if gpu_hours and parameters:
            scaling_check = self.consistency_checker.check_scaling_consistency(gpu_hours, parameters)
        ...
        if paper_group and len(paper_group) > 1:
            for field in ["gpu_hours", "parameters", "training_time"]:
                if field in extraction_dict and extraction_dict[field]:
                    cross_check = self.consistency_checker.check_cross_paper_consistency(
                        paper_group, field
                    )
            ...
            if isinstance(value, (int, float)) and value > 0:
                ...
                if field_values and len(field_values) > 3:
                    ...
                    if value in [field_values[i] for i in outliers]:
                        ...
                        if not self.outlier_detector.verify_outlier(paper, extraction_dict, field, value):
    ...
    def validate_extraction_batch(self, 
                                extractions: List[Tuple[Paper, ComputationalAnalysis]]) -> Dict[str, Any]:
        """
        ...
        return self._generate_batch_report(results, temporal_checks, cross_validation_results)
    ...
    def _calculate_overall_quality(self,
                                 extraction_validation: ExtractionValidation,
                                 consistency_checks: List[ConsistencyCheck],
                                 outlier_fields: List[str]) -> Tuple[str, float]:
        ...
        if consistency_checks:
            ...
            avg_consistency = sum(consistency_scores) / len(consistency_checks) if consistency_checks else 0.5
    ...
    def _generate_recommendations(self,
                                extraction_validation: ExtractionValidation,
                                consistency_checks: List[ConsistencyCheck],
                                outlier_fields: List[str]) -> List[str]:
        ...
        if extraction_validation.confidence < 0.8:
            ...
            if completeness < 0.7:
                recommendations.append("Improve extraction completeness - missing critical fields")
        
        # Consistency recommendations
        failed_checks = [c for c in consistency_checks if not c.passed]
        for check in failed_checks:
            if check.check_type == "domain_specific":
                recommendations.append(f"Review domain-specific values: {check.details}")
            elif check.check_type == "scaling_law":
                recommendations.append("GPU hours and parameters don't follow expected scaling")
            elif check.check_type == "temporal":
                recommendations.append(f"Temporal trend anomaly detected: {check.details}")
        
        # Outlier recommendations
        if outlier_fields:
            recommendations.append(f"Manual review recommended for outlier fields: {', '.join(outlier_fields)}")
    ...
    def _build_batch_cache(self, extractions: List[Tuple[Paper, ComputationalAnalysis]]):
    ...
    def _get_field_values(self, field: str, paper_group: Optional[List[Paper]] = None) -> List[float]:
        ...
        if paper_group:
            ...
            for paper in paper_group:
                if hasattr(paper, 'computational_analysis') and paper.computational_analysis:
    ...
    def _group_papers_by_domain(self, 
                              extractions: List[Tuple[Paper, ComputationalAnalysis]]) -> Dict[str, List[Paper]]:
        """Group papers by domain for consistency checks."""
        domain_groups = {}
    ...
    def _determine_paper_domain(self, paper: Paper) -> str:
        ...
        if any(term in title_lower for term in ["language", "nlp", "transformer", "bert", "gpt"]):
            return "nlp"
        elif any(term in title_lower for term in ["image", "vision", "cnn", "detection"]):
            return "cv"
    ...
    def _has_manual_data(self, extractions: List[Tuple[Paper, ComputationalAnalysis]]) -> bool:
    ...
    def _separate_manual_auto(self, 
                            extractions: List[Tuple[Paper, ComputationalAnalysis]]) -> Tuple[Dict, Dict]:
    ...
    def _generate_batch_report(self,
                             ...
                             cross_validation_results: Optional[Dict[str, Any]]) -> Dict[str, Any]:
