...
class QualityAnalyzer:
    ...
    def calculate_paper_quality_score(self, paper_data: Dict[str, Any]) -> float:
        ...
        venue_impact_factor = paper_data.get('venue_impact_factor', 0.0)
        score += venue_impact_factor * self.scoring_weights.get('venue_impact_factor', 0)
    ...
    def calculate_venue_quality_score(self, venue_data: Dict[str, Any]) -> float:
        ...
        acceptance_score = max(0.0, (0.5 - acceptance_rate) / 0.4) if acceptance_rate > 0 else 0.5
    ...
    def _calculate_combined_quality_score(self, paper_score: float, venue_score: float) -> float:
        ...
        max_possible_paper_score = sum(self.scoring_weights.values()) * 100  # Rough estimate
        normalized_paper_component = min(paper_score / max_possible_paper_score, 1.0) * 0.6
    ...
    def _calculate_confidence_level(self, paper_data: Dict[str, Any]) -> float:
        ...
        citation_count = paper_data.get('citation_count', 0)
        citation_confidence = min(citation_count / 50.0, 1.0)  # Max confidence at 50+ citations
        confidence_factors.append(citation_confidence)
        
        # Venue data completeness
        venue_data_fields = ['venue_impact_factor', 'venue_acceptance_rate', 'venue_h_index']
        venue_completeness = sum(1 for field in venue_data_fields 
                               if paper_data.get(field, 0) > 0) / len(venue_data_fields)
        ...
        paper_completeness = sum(1 for field in paper_fields 
                               if paper_data.get(field, 0) > 0) / len(paper_fields)
    ...
    def _validate_scoring_weights(self) -> None:
        ...
        required_weights = [
            'citation_count', 'venue_impact_factor', 'author_count',
            'page_count', 'reference_count', 'venue_h_index'
        ]
        ...
        if total_weight == 0:
            logger.error("Total scoring weights sum to zero - quality assessment will not work")
        
        logger.info(f"Scoring weights validated. Total weight: {total_weight}")
    
    def get_quality_score_breakdown(self, paper_data: Dict[str, Any]) -> Dict[str, float]:
        """
    ...
    def batch_assess_quality(self, papers: List[Dict[str, Any]]) -> List[QualityMetrics]:
        """
        ...
        for paper_data in papers:
            try:
                ...
                logger.error(f"Error assessing quality for paper {paper_data.get('paper_id', 'unknown')}: {e}")
