...
from ..data.models import Author, AuthorshipAnalysis
...
class AuthorshipClassifier:
    ...
    def __init__(self):
        ...
        self.academic_keywords = {
            'university', 'college', 'institute', 'academia', 'school',
            'department', 'faculty', 'laboratory', 'lab', 'research',
            'professor', 'student', 'postdoc', 'doctoral', 'graduate'
        }
        
        # Specific academic institution names (top CS departments)
        self.known_academic = {
            'mit', 'stanford', 'berkeley', 'cmu', 'caltech', 'harvard',
            'princeton', 'yale', 'columbia', 'cornell', 'upenn', 'nyu',
            'ucla', 'usc', 'umich', 'uiuc', 'georgia tech', 'ut austin',
            'uw', 'ucsd', 'umd', 'wisc', 'purdue', 'brown', 'rice',
            'duke', 'unc', 'virginia', 'washington', 'oxford', 'cambridge',
            'eth', 'epfl', 'imperial', 'ucl', 'edinburgh', 'toronto',
            'waterloo', 'mcgill', 'ubc', 'melbourne', 'sydney', 'anu',
            'tsinghua', 'peking', 'tokyo', 'kyoto', 'seoul national',
            'kaist', 'iit', 'iisc', 'technion', 'tel aviv', 'max planck',
            'inria', 'cnrs', 'ens', 'polytechnique', 'sorbonne'
        }
        ...
        self.industry_keywords = {
            'corporation', 'company', 'incorporated', 'limited', 'labs',
            'laboratory', 'technologies', 'software', 'hardware', 'solutions',
            'products', 'services', 'engineering', 'development', 'r&d'
        }
        
        # Known tech companies and research labs
        self.known_industry = {
            'google', 'microsoft', 'facebook', 'meta', 'apple', 'amazon',
            'ibm', 'intel', 'nvidia', 'amd', 'qualcomm', 'broadcom',
            'cisco', 'oracle', 'sap', 'salesforce', 'adobe', 'vmware',
            'twitter', 'netflix', 'uber', 'airbnb', 'lyft', 'spotify',
            'deepmind', 'openai', 'anthropic', 'cohere', 'hugging face',
            'tesla', 'spacex', 'boston dynamics', 'waymo', 'cruise',
            'baidu', 'alibaba', 'tencent', 'bytedance', 'huawei',
            'samsung', 'lg', 'sony', 'panasonic', 'toshiba', 'fujitsu',
            'siemens', 'bosch', 'philips', 'ericsson', 'nokia',
            'accenture', 'deloitte', 'pwc', 'ey', 'kpmg', 'mckinsey'
        }
        
        logger.info("AuthorshipClassifier initialized with comprehensive affiliation patterns")
    ...
    def classify_authors(self, authors: List[Author]) -> AuthorshipAnalysis:
        ...
        for author in authors:
            ...
            author_details.append({
                ...
                'confidence': self._get_classification_confidence(author.affiliation, classification)
            })
        ...
        category = self._determine_category(academic_count, industry_count, unknown_count)
    ...
    def _classify_single_author(self, author: Author) -> str:
        ...
        if 'research' in affiliation_lower:
            ...
            if any(term in affiliation_lower for term in ['university', 'institute', 'college']):
                academic_score += 3
            elif any(term in affiliation_lower for term in ['labs', 'corporation', 'company']):
                industry_score += 3
    ...
    def _get_classification_confidence(self, affiliation: str, classification: str) -> float:
    ...
    def _determine_category(self, academic_count: int, industry_count: int, 
    ...
    def _calculate_overall_confidence(self, academic_count: int, industry_count: int,
                                    unknown_count: int, author_details: List[Dict]) -> float:
    ...
    def _extract_institution_name(self, affiliation: str) -> str:
        ...
        cleaned = re.sub(r'\b(?:inc|corp|ltd|llc|gmbh|ag|sa|plc)\b\.?', '', affiliation, flags=re.IGNORECASE)
        cleaned = re.sub(r'\b(?:university|college|institute|labs?|research)\b', '', cleaned, flags=re.IGNORECASE)
        cleaned = re.sub(r'[,;].*', '', cleaned)  # Take first part before comma/semicolon
