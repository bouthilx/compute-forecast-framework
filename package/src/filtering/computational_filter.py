...
from ..data.models import Paper, ComputationalAnalysis, AuthorshipAnalysis, VenueAnalysis
...
class ComputationalResearchFilter:
    ...
    def filter_paper(self, paper: Paper) -> FilteringResult:
        ...
        # Perform analyses
        computational_analysis = self.computational_analyzer.analyze_computational_content(paper)
        ...
        comp_passed, comp_reasons = self._check_computational_criteria(computational_analysis)
        ...
        if not passed and not all_reasons:
            all_reasons.append(f"Combined score {combined_score:.2f} below threshold {self.config.min_combined_score}")
    ...
    def _check_computational_criteria(self, analysis: ComputationalAnalysis) -> Tuple[bool, List[str]]:
    ...
    def _check_authorship_criteria(self, analysis: AuthorshipAnalysis) -> Tuple[bool, List[str]]:
        ...
        if self.config.require_academic_eligible:
            if analysis.category != 'academic_eligible':
                passed = False
                reasons.append(f"Paper not academic eligible (category: {analysis.category})")
        
        if not self.config.allow_industry_collaboration:
            if analysis.industry_count > 0:
                passed = False
                reasons.append(f"Industry collaboration not allowed ({analysis.industry_count} industry authors)")
    ...
    def _calculate_combined_score(self, comp: ComputationalAnalysis,
                                auth: AuthorshipAnalysis,
                                venue: VenueAnalysis) -> float:
    ...
    def _calculate_overall_confidence(self, comp: ComputationalAnalysis,
                                    auth: AuthorshipAnalysis,
                                    venue: VenueAnalysis) -> float:
    ...
    def batch_filter(self, papers: List[Paper], 
                    return_all: bool = False) -> List[FilteringResult]:
        """
        ...
        for paper in papers:
            try:
                ...
                if return_all:
                    ...
                    results.append(FilteringResult(
                        paper=paper,
                        passed=False,
                        score=0.0,
                        computational_analysis=ComputationalAnalysis(
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        if stats['total_processed'] > 0:
            ...
            stats['computational_filter_rate'] = stats['computational_filtered'] / stats['total_processed']
            stats['authorship_filter_rate'] = stats['authorship_filtered'] / stats['total_processed']
            stats['venue_filter_rate'] = stats['venue_filtered'] / stats['total_processed']
            stats['combined_filter_rate'] = stats['combined_filtered'] / stats['total_processed']
        else:
            stats['pass_rate'] = 0.0
