...
from ..data.models import Paper, ComputationalAnalysis
...
class ComputationalAnalyzer:
    ...
    def __init__(self):
        ...
        self.computational_keywords = {
            'algorithms': {
                'algorithm', 'optimization', 'heuristic', 'solver', 'search',
                'greedy', 'dynamic programming', 'branch and bound', 'approximation',
                'randomized', 'online algorithm', 'streaming', 'parallel algorithm',
                'distributed algorithm', 'quantum algorithm', 'genetic algorithm',
                'evolutionary', 'metaheuristic', 'local search', 'simulated annealing'
            },
            'complexity': {
                'complexity', 'np-hard', 'np-complete', 'pspace', 'polynomial',
                'exponential', 'logarithmic', 'linear time', 'quadratic', 'cubic',
                'big-o', 'omega', 'theta', 'amortized', 'worst-case', 'average-case',
                'approximation ratio', 'competitive ratio', 'hardness', 'reduction'
            },
            'machine_learning': {
                'neural network', 'deep learning', 'machine learning', 'training',
                'dataset', 'model', 'accuracy', 'precision', 'recall', 'f1-score',
                'loss function', 'gradient', 'backpropagation', 'optimizer', 'epoch',
                'batch', 'validation', 'test set', 'cross-validation', 'overfitting',
                'regularization', 'dropout', 'convolution', 'transformer', 'attention',
                'bert', 'gpt', 'gan', 'vae', 'reinforcement learning', 'supervised',
                'unsupervised', 'semi-supervised', 'transfer learning', 'fine-tuning'
            },
            'systems': {
                'implementation', 'system', 'architecture', 'performance', 'scalability',
                'throughput', 'latency', 'bandwidth', 'memory', 'cache', 'distributed',
                'parallel', 'concurrent', 'thread', 'process', 'synchronization',
                'load balancing', 'fault tolerance', 'reliability', 'availability',
                'consistency', 'partition', 'replication', 'consensus', 'database'
            },
            'experimental': {
                'experiment', 'evaluation', 'benchmark', 'dataset', 'baseline',
                'comparison', 'results', 'performance', 'metrics', 'measurement',
                'empirical', 'simulation', 'prototype', 'implementation', 'case study',
                'ablation', 'analysis', 'statistical significance', 'confidence interval',
                'standard deviation', 'variance', 'correlation', 'hypothesis test'
            },
            'computation_resources': {
                'gpu', 'cpu', 'memory', 'ram', 'storage', 'compute', 'cluster',
                'cloud', 'aws', 'azure', 'gcp', 'hpc', 'supercomputer', 'fpga',
                'asic', 'tpu', 'quantum computer', 'edge device', 'iot', 'embedded'
            },
            'data_structures': {
                'array', 'list', 'tree', 'graph', 'hash', 'map', 'set', 'queue',
                'stack', 'heap', 'trie', 'suffix tree', 'segment tree', 'fenwick tree',
                'union-find', 'bloom filter', 'skip list', 'b-tree', 'red-black tree',
                'avl tree', 'splay tree', 'treap', 'kd-tree', 'quadtree', 'octree'
            }
        }
        
        # Patterns indicating computational resources or experiments
        self.resource_patterns = [
            (r'\d+\s*(?:GB|TB|PB)\s*(?:of\s*)?(?:memory|RAM|storage|data)', 'memory_size'),
            (r'\d+\s*(?:hours?|days?|weeks?)\s*(?:of\s*)?(?:compute|computation|training)', 'compute_time'),
            (r'\d+\s*(?:CPU|GPU|TPU)s?\s*(?:cores?|units?)?', 'compute_units'),
            (r'(?:trained|ran|executed)\s*(?:on|using)\s*\d+\s*(?:nodes?|machines?|servers?)', 'distributed_scale'),
            (r'\d+(?:\.\d+)?\s*(?:million|billion|M|B)\s*(?:parameters|samples|examples)', 'scale_metric'),
            (r'(?:dataset|corpus)\s*(?:of|with|containing)\s*\d+\s*(?:samples|examples|instances)', 'dataset_size')
        ]
        
        # Experimental indicators
        self.experimental_indicators = {
            'strong': {
                'we implement', 'we develop', 'we build', 'we design', 'we propose and implement',
                'our implementation', 'our system', 'our algorithm', 'we evaluate',
                'experimental results', 'empirical evaluation', 'extensive experiments',
                'real-world experiments', 'experimental setup', 'experimental validation'
            },
            'medium': {
                'we test', 'we measure', 'we compare', 'we analyze', 'we benchmark',
                'performance evaluation', 'experimental study', 'case study',
                'simulation results', 'synthetic experiments', 'controlled experiments'
            },
            'weak': {
                'we propose', 'we present', 'we introduce', 'theoretical analysis',
                'we prove', 'we show', 'formal analysis', 'mathematical framework'
            }
        }
    ...
    def _analyze_experimental_indicators(self, text: str) -> Dict[str, Any]:
        ...
        indicators['implementation_mentioned'] = any(
            term in text for term in ['implement', 'implementation', 'prototype', 'system']
        )
        indicators['evaluation_mentioned'] = any(
            term in text for term in ['evaluat', 'experiment', 'benchmark', 'test', 'measur']
        )
        indicators['theoretical_focus'] = any(
            term in text for term in ['theorem', 'proof', 'lemma', 'corollary', 'formal']
        )
        
        return indicators
    
    def _calculate_richness_score(self, keyword_matches: Dict[str, int],
                                 resource_metrics: Dict[str, Any],
                                 experimental_indicators: Dict[str, Any]) -> float:
        """
        ...
        if keyword_matches:
            ...
            total_weighted_matches = sum(
                keyword_matches.get(cat, 0) * weights.get(cat, 1.0)
                for cat in weights
            )
    ...
    def _calculate_confidence_score(self, text: str, keyword_matches: Dict[str, int],
                                  resource_metrics: Dict[str, Any],
                                  experimental_indicators: Dict[str, Any]) -> float:
        """
        ...
        keyword_factor = min(1.0, total_keywords / 20.0)  # 20 keywords = full confidence
        ...
        experimental_evidence = (
            experimental_indicators.get('strong_experimental', 0) +
    ...
    def batch_analyze(self, papers: List[Paper]) -> List[ComputationalAnalysis]:
        ...
        for paper in papers:
            try:
                ...
                results.append(ComputationalAnalysis(
