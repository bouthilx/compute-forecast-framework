...
class StatisticalAnalyzer:
    ...
    def analyze_paper_collection(self, papers: List[Dict[str, Any]]) -> PaperStatistics:
        ...
        authors = [p.get('author_count', 0) for p in papers if p.get('author_count') is not None]
        pages = [p.get('page_count', 0) for p in papers if p.get('page_count') is not None]
        venues = [p.get('venue', 'Unknown') for p in papers if p.get('venue')]
        years = [p.get('year', 0) for p in papers if p.get('year') and p.get('year') > 1900]
        ...
        if venue_counts:
            ...
            stats.venue_diversity_index = -sum(p * np.log(p) for p in proportions if p > 0)
        ...
        if years:
            ...
            if len(set(years)) > 1:
                stats.citation_growth_rate = self._calculate_citation_growth_rate(papers)
    ...
    def analyze_venue_specific(self, papers: List[Dict[str, Any]], venue: str) -> VenueStatistics:
        """
        ...
        """
        venue_papers = [p for p in papers if p.get('venue', '').lower() == venue.lower()]
        ...
        # Years covered
        years = [p.get('year') for p in venue_papers if p.get('year') and p.get('year') > 1900]
        stats.years_covered = sorted(list(set(years)))
        
        # Impact metrics
        citations = [p.get('citation_count', 0) for p in venue_papers if p.get('citation_count') is not None]
        if citations:
            stats.impact_metrics = {
                ...
                'high_impact_papers': sum(1 for c in citations if c > np.percentile(citations, 90))
            }
        
        # Author metrics
        authors_per_paper = [p.get('author_count', 0) for p in venue_papers if p.get('author_count')]
        if authors_per_paper:
            ...
            stats.author_metrics = {
                ...
                'collaboration_index': np.mean(authors_per_paper) / max(authors_per_paper) if authors_per_paper else 0
            }
        ...
        if years and citations:
            ...
            stats.citation_metrics = {
                'citation_growth_trend': self._analyze_yearly_citation_trend(citation_by_year),
                'peak_year': max(citation_by_year.keys(), key=lambda y: np.mean(citation_by_year[y])) if citation_by_year else None,
    ...
    def generate_analysis_summary(self, papers: List[Dict[str, Any]]) -> AnalysisSummary:
        """
        ...
        years = [p.get('year') for p in papers if p.get('year') and p.get('year') > 1900]
    ...
    def compare_venues(self, papers: List[Dict[str, Any]], venues: List[str]) -> Dict[str, VenueStatistics]:
        """
    ...
    def _calculate_citation_growth_rate(self, papers: List[Dict[str, Any]]) -> float:
        ...
        yearly_averages = {year: np.mean(cites) for year, cites in year_citations.items()}
    ...
    def _analyze_yearly_citation_trend(self, citation_by_year: Dict[int, List[int]]) -> str:
        """Analyze citation trend across years."""
        if len(citation_by_year) < 2:
            return "insufficient_data"
        
        yearly_averages = {year: np.mean(cites) for year, cites in citation_by_year.items()}
        sorted_years = sorted(yearly_averages.keys())
    ...
    def _generate_key_insights(self, papers: List[Dict[str, Any]], stats: PaperStatistics) -> List[str]:
        ...
        if stats.citation_stats:
            ...
            if avg_citations > 20:
                insights.append(f"High-impact collection with average {avg_citations:.1f} citations per paper")
            elif avg_citations < 5:
                insights.append("Collection contains primarily recent or emerging research papers")
            
            # Citation distribution insights
            median_citations = stats.citation_stats['median']
            if avg_citations > median_citations * 2:
                insights.append("Citation distribution is highly skewed with some extremely high-impact papers")
        
        # Venue diversity insights
        if stats.venue_diversity_index > 2.0:
            insights.append("Highly diverse collection spanning multiple research venues")
        ...
        if stats.temporal_coverage > 10:
            insights.append(f"Comprehensive temporal coverage spanning {stats.temporal_coverage} years")
        
        # Collaboration insights
        if stats.author_stats:
            avg_authors = stats.author_stats['mean_authors_per_paper']
            if avg_authors > 5:
                insights.append("Strong collaboration patterns with large research teams")
            elif avg_authors < 2:
                insights.append("Individual research focus with limited collaboration")
        
        # Growth insights
        if stats.citation_growth_rate > 0.1:
            insights.append("Strong citation growth indicating increasing research impact")
