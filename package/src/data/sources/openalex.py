                ...
                if isinstance(authorship, dict):
                    ...
                    if isinstance(author_info, dict):
                        ...
                        if author_name:  # Only add authors with names
                            ...
                            affiliation = ''
                            if institutions and isinstance(institutions, list) and institutions[0]:
                                affiliation = institutions[0].get('display_name', '').strip()
    ...
    def get_paper_details(self, work_id: str) -> Paper:
        ...
        try:
            ...
            if not work_id.startswith('https://openalex.org/'):
                work_id = f"https://openalex.org/W{work_id}" if not work_id.startswith('W') else f"https://openalex.org/{work_id}"
            ...
            response = requests.get(work_url, params=params, timeout=self.config.get('timeout', 30))
            
            if response.status_code != 200:
                raise ValueError(f"Work {work_id} not found or API error: {response.status_code}")
