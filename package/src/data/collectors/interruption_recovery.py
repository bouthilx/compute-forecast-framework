...
from .state_structures import (
    CollectionSession, CheckpointData, RecoveryPlan, SessionResumeResult,
...
class InterruptionRecoveryEngine:
    ...
    def __init__(
        ...
        logger.info(f"InterruptionRecoveryEngine initialized with {recovery_timeout_seconds}s timeout")
    ...
    def resume_interrupted_session(self, session_id: str) -> SessionResumeResult:
        ...
        try:
            ...
            if session_id in self._active_recoveries:
                elapsed = (datetime.now() - self._active_recoveries[session_id]).total_seconds()
                if elapsed < self.recovery_timeout_seconds:
                    result.resume_errors.append(f"Recovery already in progress for {elapsed:.1f}s")
                    return result
                else:
                    logger.warning(f"Previous recovery timed out after {elapsed:.1f}s, restarting")
            
            # Track recovery attempt
            self._active_recoveries[session_id] = datetime.now()
            self._recovery_attempts[session_id] = self._recovery_attempts.get(session_id, 0) + 1
            
            # Check max attempts
            if self._recovery_attempts[session_id] > self.max_recovery_attempts:
                result.resume_errors.append(f"Maximum recovery attempts ({self.max_recovery_attempts}) exceeded")
                return result
            ...
            logger.info(f"Step 2: Generating recovery plan")
            ...
            logger.info(f"Step 3: Validating recovery feasibility")
            if not self._validate_recovery_feasibility(recovery_plan, interruption_analysis):
            ...
            logger.info(f"Step 4: Executing recovery strategy: {recovery_plan.resumption_strategy}")
            ...
            logger.info(f"Step 5: Validating state consistency")
            consistency_validation = self._validate_state_consistency(session_id, result)
            ...
            logger.info(f"Step 6: Restoring session to active state")
            ...
            result.session_state_after_recovery = final_result.session_state_after_recovery
            ...
            if result.recovery_duration_seconds > self.recovery_timeout_seconds:
                logger.warning(f"Recovery took {result.recovery_duration_seconds:.1f}s (>{self.recovery_timeout_seconds}s requirement)")
                result.resume_warnings.append(f"Recovery exceeded 5-minute requirement")
            ...
            if result.success:
                ...
                logger.info(f"Recovery successful for session {session_id} in {result.recovery_duration_seconds:.1f}s")
            else:
    ...
    def detect_interruption_type(self, session_id: str) -> InterruptionType:
        ...
        try:
            ...
            logger.error(f"Error detecting interruption type for session {session_id}: {e}")
            return InterruptionType.UNKNOWN
    ...
    def _analyze_interruption(self, session_id: str) -> InterruptionAnalysis:
        ...
        try:
            ...
            logger.error(f"Failed to analyze interruption for session {session_id}: {e}")
            raise
    ...
    def _validate_recovery_feasibility(
        ...
        try:
            ...
            if interruption_analysis.blocking_issues:
                logger.error(f"Blocking issues found: {interruption_analysis.blocking_issues}")
                return False
            
            # Check recovery confidence
            if recovery_plan.confidence_score < 0.5:
                logger.warning(f"Low recovery confidence: {recovery_plan.confidence_score}")
                return False
            
            # Check estimated recovery time
            if recovery_plan.estimated_recovery_time_minutes > 5.0:
                logger.warning(f"Estimated recovery time exceeds 5 minutes: {recovery_plan.estimated_recovery_time_minutes}")
                return False
    ...
    def _recover_from_checkpoint(
        ...
        try:
            ...
            if not validation.is_valid:
                result.recovery_steps_failed.append("Checkpoint integrity validation failed")
    ...
    def _validate_state_consistency(self, session_id: str, result: SessionResumeResult) -> bool:
        """
        ...
        try:
            ...
            session = self.state_manager.get_session_status(session_id)
            if not session:
                result.recovery_steps_failed.append("Session not found for consistency validation")
                return False
            ...
            for validation in validation_results:
                result.data_integrity_checks.append(f"{validation.validation_type}: {'PASSED' if validation.passed else 'FAILED'}")
            
            if all_passed:
                result.recovery_steps_executed.append("State consistency validation passed")
            else:
                result.recovery_steps_failed.append("State consistency validation failed")
                failed_checks = [v.validation_type for v in validation_results if not v.passed]
                result.resume_errors.append(f"Failed validation checks: {failed_checks}")
