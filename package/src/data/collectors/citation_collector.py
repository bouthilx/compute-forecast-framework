...
class CitationCollector:
    ...
    def collect_from_all_sources(self, query: CollectionQuery) -> Dict[str, CollectionResult]:
        ...
        self.logger.info(f"Starting collection for domain '{query.domain}', year {query.year}, max_results={query.max_results}")
        ...
        for source_name, source in self.sources.items():
            try:
                ...
                if result.errors:
                    self.logger.warning(f"{source_name} errors: {result.errors[:3]}")  # Log first 3 errors
        ...
        self.logger.info(f"Collection complete. Total papers: {total_papers_collected} from {len(results)} sources")
        return results
    ...
    def get_combined_papers(self, results: Dict[str, CollectionResult]) -> List[Paper]:
        ...
        self.logger.info(f"Combined papers by source: {source_counts} (total: {len(all_papers)})")
        return all_papers
    ...
    def collect_from_venue_year(self, venue: str, year: int, citation_threshold: int = 0, working_apis: List[str] = None) -> List[Dict]:
        ...
        if working_apis:
            sources_to_try = {name: source for name, source in self.sources.items() if name in working_apis}
            ...
            try:
                ...
                if result and result.papers:
                    ...
                    for paper in result.papers:
                        ...
                        if (citations >= citation_threshold and 
                            paper_year == year and
                            (venue.lower() in paper_venue.lower() if paper_venue else False)):
                    ...
                    self.logger.info(f"Found {len(venue_papers)} papers from {source_name}")
    ...
    def collect_from_venue_year_with_keywords(self, venue: str, year: int, keywords: List[str], domain: str, working_apis: List[str] = None) -> List[Dict]:
        ...
        all_keywords = [venue] + keywords[:3]  # Limit keywords to avoid too complex queries
        
        for source_name, source in sources_to_try.items():
            try:
                self.logger.info(f"Searching {source_name} for {venue} {year} with keywords: {keywords[:3]}")
                ...
                if result and result.papers:
                    ...
                    for paper in result.papers:
                        ...
                        if (paper_year == year and 
                            (venue.lower() in paper_venue.lower() if paper_venue else True)):
                    ...
                    self.logger.info(f"Found {len(filtered_papers)} papers from {source_name}")
                
            except Exception as e:
                self.logger.warning(f"Failed to search {source_name} for {venue} with keywords: {e}")
                continue
        
        return papers
    
    def collect_from_keywords(self, keywords: List[str], year: int, domain: str, working_apis: List[str] = None) -> List[Dict]:
        ...
        for source_name, source in sources_to_try.items():
            try:
                self.logger.info(f"Searching {source_name} with keywords: {search_keywords}")
                ...
                if result and result.papers:
                    ...
                    self.logger.info(f"Found {len(domain_papers)} papers from {source_name}")
