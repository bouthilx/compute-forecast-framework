...
from ..models import Paper, CollectionQuery, APIResponse, APIConfig
...
class EnhancedCollectionOrchestrator:
    ...
    def collect_from_all_sources(
        ...
        if parallel:
            ...
            logger.info(f"Starting parallel collection from {len(active_sources)} sources")
            
            with ThreadPoolExecutor(max_workers=len(active_sources)) as executor:
                # Submit collection tasks
                future_to_source = {
                    executor.submit(
                        self._collect_from_source_with_rate_limit,
                        source_name,
                        query
                    ): source_name
                ...
                for future in as_completed(future_to_source):
                    ...
                    try:
                        ...
                        logger.info(f"Collected {len(source_papers)} papers from {source_name}")
            ...
            logger.info(f"Starting sequential collection from {len(active_sources)} sources")
    ...
    def _collect_from_source_with_rate_limit(
        ...
        response_time_ms = response.metadata.response_time_ms if response.metadata else 1000
        self.rate_limiter.record_request(source_name, response.success, response_time_ms)
    ...
    def collect_with_fallback(
        ...
        for source_name in preferred_sources:
            ...
            try:
                papers = self._collect_from_source_with_rate_limit(
                    source_name,
                    query
                )
