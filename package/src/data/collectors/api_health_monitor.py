...
class APIHealthMonitor:
    ...
    def _calculate_health_status(self, api_name: str, latest_request: dict) -> APIHealthStatus:
        ...
        avg_response_time = sum(req['response_time_ms'] for req in recent_requests) / total_requests
        ...
        # Determine status
        status = self._determine_status_level(success_rate, avg_response_time, consecutive_errors)
        ...
        logger.debug(f"API {api_name} health: {status} (success_rate={success_rate:.2f}, "
                    f"avg_response_time={avg_response_time:.0f}ms, consecutive_errors={consecutive_errors})")
        
        return health_status
    
    def _determine_status_level(self, success_rate: float, avg_response_time: float, consecutive_errors: int) -> str:
