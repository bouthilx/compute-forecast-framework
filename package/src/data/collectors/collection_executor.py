...
class RateLimiter:
    ...
    def wait(self, operation_type: str):
        ...
        if time_since_last < delay:
            ...
            logger.info(f"Rate limiting: waiting {sleep_time:.2f}s for {operation_type}")
...
class CollectionExecutor:
    ...
    def setup_collection_environment(self) -> bool:
        ...
        try:
            ...
            if len(working_apis) < 2:  # Require at least 2 working APIs
                ...
                logger.info(f"Proceeding with {len(working_apis)} working APIs: {working_apis}")
        ...
        try:
            ...
            self.collection_strategy_optimizer = CollectionStrategyOptimizer(self.venue_database, venue_scorer)
    ...
    def get_domains_from_analysis(self) -> List[str]:
        ...
        if isinstance(self.domain_analysis, dict):
            if 'final_corrected_stats' in self.domain_analysis:
                ...
                excluded_keys = {'methodology', 'inflation_factor', 'total_papers', 'corrections_applied'}
                domain_keys = [k for k in self.domain_analysis.keys() if k not in excluded_keys]
                if domain_keys:
                    return domain_keys
    ...
    def create_setup_status(self, setup_success: bool, api_status: Dict[str, bool]) -> Dict[str, Any]:
        ...
        status = {
            ...
            "dependencies_loaded": {
                ...
                "worker4_computational_analyzer": self.computational_analyzer is not None,
            ...
            "collection_targets": {
                "total_papers_target": len(domains) * 6 * 8,  # domains * years * papers_per_domain_year
                "papers_per_domain_year": 8,
                "domains_count": len(domains),
                "years_span": 6,
                "domains": domains
            },
            "issues": [] if setup_success else ["Setup failed - check logs for details"]
        }
