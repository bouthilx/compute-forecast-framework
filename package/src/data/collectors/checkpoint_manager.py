...
from .state_structures import CheckpointData, CheckpointValidationResult, ValidationResult
...
class CheckpointManager:
    ...
    def __init__(
        ...
        logger.info(f"CheckpointManager initialized with max_checkpoints={max_checkpoints_per_session}")
        ...
        with self._lock:
            try:
                ...
                if success:
                    ...
                    if self._checkpoint_counters[session_id] % self.checkpoint_cleanup_interval == 0:
    ...
    def load_latest_checkpoint(self, session_id: str) -> Optional[CheckpointData]:
        ...
        try:
            ...
            logger.error(f"Error loading latest checkpoint for session {session_id}: {e}")
            return None
    ...
    def validate_checkpoint(self, session_id: str, checkpoint_id: str) -> CheckpointValidationResult:
        """
        ...
        try:
            ...
            total_venues = len(checkpoint.venues_completed) + len(checkpoint.venues_in_progress) + len(checkpoint.venues_not_started)
            if total_venues == 0:
            ...
            if abs(papers_sum - checkpoint.papers_collected) > (checkpoint.papers_collected * 0.1):
    ...
    def cleanup_session_checkpoints(self, session_id: str, keep_latest: int = 10) -> int:
        """
        ...
        with self._lock:
            try:
                ...
                for checkpoint in checkpoints_to_remove:
                    checkpoint_path = self._get_checkpoint_path(session_id, checkpoint.checkpoint_id)
                    try:
                        ...
                        meta_path = checkpoint_path.with_suffix(f"{checkpoint_path.suffix}.meta")
                        if meta_path.exists():
                            meta_path.unlink()
                ...
                logger.info(f"Cleaned up {cleaned_count} old checkpoints for session {session_id}")
    ...
    def validate_session_checkpoints(self, session_id: str) -> List[ValidationResult]:
        ...
        for checkpoint in checkpoints:
            validation_result = self.validate_checkpoint(session_id, checkpoint.checkpoint_id)
            ...
            result = ValidationResult(
                ...
                recommendations=[
                    "Consider recreating checkpoint" if not validation_result.can_be_used_for_recovery else "Checkpoint usable for recovery"
                ]
            )
    ...
    def _cleanup_old_checkpoints(self, session_id: str) -> None:
        ...
        try:
            if self.max_checkpoints_per_session > 0:
                self.cleanup_session_checkpoints(session_id, self.max_checkpoints_per_session)
        except Exception as e:
            logger.warning(f"Automatic checkpoint cleanup failed for session {session_id}: {e}")
    ...
    def get_checkpoint_statistics(self, session_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            checkpoint_types = {}
            for checkpoint in checkpoints:
                checkpoint_types[checkpoint.checkpoint_type] = checkpoint_types.get(checkpoint.checkpoint_type, 0) + 1
            ...
            return {
                ...
                "timespan_hours": (checkpoints[-1].timestamp - checkpoints[0].timestamp).total_seconds() / 3600
            }
            
        except Exception as e:
            logger.error(f"Error getting checkpoint statistics for session {session_id}: {e}")
            return {"error": str(e)}