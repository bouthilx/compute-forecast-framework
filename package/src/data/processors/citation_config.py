...
class CitationConfig:
    ...
    # Venue tier mappings
    venue_tiers: Dict[str, str] = field(default_factory=lambda: {
    ...
    tier2_venues: Set[str] = field(default_factory=lambda: {"AAAI", "CVPR", "ICCV", "ECCV", "ACL", "EMNLP", "NAACL"})
    tier3_venues: Set[str] = field(default_factory=lambda: {"UAI", "AISTATS", "KDD", "WWW", "SIGIR", "WSDM"})
    ...
    velocity_scores: Dict[str, float] = field(default_factory=lambda: {
        "very_high": 1.0,
        "high": 0.8,
        "good": 0.6,
        "moderate": 0.4,
    ...
    breakthrough_weights: Dict[str, float] = field(default_factory=lambda: {
    ...
    statistical_percentile: float = 75.0     # Use 75th percentile for statistical threshold
    min_representation_percentile: float = 70.0  # 70th percentile for minimum representation
    
    # Quality thresholds
    high_impact_citation_threshold: int = 50  # Papers with >50 citations considered high impact
    high_impact_percentile: float = 90.0      # Top 10% by citations
    
    # Venue tier multipliers for prestige calculation
    venue_tier_multipliers: Dict[str, float] = field(default_factory=lambda: {
        "tier1": 1.0,
        "tier2": 0.8,
        "tier3": 0.6,
    ...
    recency_thresholds: Dict[str, int] = field(default_factory=lambda: {
        "very_recent": 2,
        "recent": 3,
        "somewhat_recent": 5
    })
    ...
    # Recency scores
    recency_scores: Dict[str, float] = field(default_factory=lambda: {
        "very_recent": 1.0,
        "recent": 0.8,
        "somewhat_recent": 0.6,
        "older": 0.0
    ...
    author_h_index_thresholds: Dict[str, int] = field(default_factory=lambda: {
        "high": 50,
        "medium": 30
    })
    
    # Author reputation scores
    author_reputation_scores: Dict[str, float] = field(default_factory=lambda: {
    ...
    def get_venue_prestige_multiplier(self, venue: str) -> float:
        ...
        return self.venue_tier_multipliers.get(tier, self.venue_tier_multipliers["tier4"])
