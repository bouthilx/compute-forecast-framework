...
class FuzzyVenueMatcher:
    ...
    def normalize_venue_name(self, venue: str) -> str:
        ...
        stop_words = {'THE', 'OF', 'ON', 'IN', 'FOR', 'AND', 'OR', 'AT', 'TO', 'WITH', 'BY'}
        filtered_words = [word for word in expanded_words if word and word not in stop_words]
    ...
    def batch_find_matches(self, 
                          raw_venues: List[str], 
                          candidates: List[str], 
                          threshold: float = None) -> Dict[str, FuzzyMatchResult]:
        """
        ...
        for raw_venue in raw_venues:
            try:
                ...
                if matches:
                    ...
                    custom_similarity = self.calculate_venue_similarity(raw_venue, best_candidate)
                    ...
                    if custom_similarity >= threshold:
                        results[raw_venue] = FuzzyMatchResult(
                            ...
                            normalized_matched=self.normalize_venue_name(best_candidate),
                            match_type="fuzzy"
                        )
                    else:
                        results[raw_venue] = FuzzyMatchResult(
