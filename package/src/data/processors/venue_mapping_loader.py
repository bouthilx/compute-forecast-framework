...
class VenueMappingLoader:
    ...
    def load_all_mappings(self) -> VenueMappingLoadResult:
        ...
        if worker6_path.exists():
            try:
                ...
                self.logger.info(f"Loaded {len(result.venue_mappings)} mappings from Worker 6")
    ...
    def _load_worker6_mappings(self, file_path: Path, result: VenueMappingLoadResult) -> None:
    ...
    def _load_manual_corrections(self, file_path: Path, result: VenueMappingLoadResult) -> None:
    ...
    def _load_venues_yaml(self, file_path: Path, result: VenueMappingLoadResult) -> None:
        ...
        if 'computational_focus_scores' in data:
            for venue, score in data['computational_focus_scores'].items():
                if venue in result.venue_configs:
                    ...
                    result.venue_configs[venue] = VenueConfig(
                        venue_name=venue,
                        computational_focus=score
                    )
    
    def _load_worker3_venues(self, file_path: Path, result: VenueMappingLoadResult) -> None:
    ...
    def _infer_venue_tier(self, venue: str) -> str:
        ...
        if venue_upper in ['AAAI', 'EMNLP', 'ACL', 'NAACL', 'AISTATS', 'UAI', 'ICRA', 'IROS']:
            return 'tier2'
        
        # Tier 3: Good conferences
        if venue_upper in ['BMVC', 'WACV', 'COLING', 'EACL', 'CHI', 'SIGIR', 'KDD', 'WWW']:
            return 'tier3'
    ...
    def _infer_computational_focus(self, venue: str) -> float:
        ...
        if venue_upper in ['NEURIPS', 'ICML', 'ICLR', 'CVPR', 'ICCV', 'ECCV', 'ICRA', 'IROS']:
            return 0.9
