...
class RecoveryValidator:
    ...
    def validate_recovery(self, error_scenario: ErrorScenario,
                         pre_error_state: Dict[str, Any],
                         post_recovery_state: Dict[str, Any]) -> RecoveryMetrics:
        """
        ...
        data_integrity = self.measure_data_integrity(pre_error_state, post_recovery_state)
    ...
    def verify_graceful_degradation(self, component: str, 
        ...
        if recovery_status.get("recovery_attempts", 0) > recovery_status.get("max_attempts", 3):
            graceful = False
            logger.warning(f"Recovery attempts exceeded for {component}")
        
        # Check if component maintains partial functionality
        if component_status:
            if component_status.get("status") == "failed" and not component_status.get("partial_functionality", False):
                graceful = False
                logger.warning(f"Component {component} completely failed without partial functionality")
        
        return graceful
    
    def validate_recovery_time(self, start_time: datetime, end_time: datetime,
                              max_recovery_seconds: float) -> Dict[str, Any]:
        """
        ...
        if not within_limit:
            logger.warning(f"Recovery exceeded time limit by {result['exceeded_by_seconds']:.1f}s")
    ...
    def generate_validation_report(self) -> Dict[str, Any]:
        ...
        # Average metrics
        avg_recovery_time = sum(m.recovery_time_seconds for m in self.recovery_metrics) / total
        avg_data_loss = sum(m.data_loss_percentage for m in self.recovery_metrics) / total
        
        # Group by error type
        by_error_type = defaultdict(lambda: {"total": 0, "successful": 0, "avg_time": 0.0})
    ...
    def _is_recovery_successful(self, pre_state: Dict[str, Any],
                               post_state: Dict[str, Any],
                               error_scenario: ErrorScenario) -> bool:
        """Determine if recovery was successful."""
        ...
        if not error_scenario.recovery_expected:
            return "papers_collected" in post_state and post_state["papers_collected"] > 0
    ...
    def _measure_dict_integrity(self, expected: dict, actual: dict) -> float:
        ...
        for key in expected:
            if key in actual:
                if expected[key] == actual[key]:
                    ...
                    nested_integrity = self.measure_data_integrity(expected[key], actual[key])
        ...
        value_integrity = (value_integrity / total_keys) * 100 if total_keys > 0 else 100
    ...
    def _log_validation_result(self, metrics: RecoveryMetrics,
    ...
    def _generate_recommendations(self, avg_recovery_time: float,
                                 avg_data_loss: float,
                                 by_error_type: Dict[str, Any]) -> List[str]:
