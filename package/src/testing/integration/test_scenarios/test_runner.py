...
from .performance_regression import run_performance_regression_test, PerformanceRegressionResult
...
class EndToEndTestRunner:
    ...
    def run_all_tests(self, 
                     ...
                     save_performance_baseline: bool = False) -> TestSuiteResult:
    ...
    def _analyze_suite_results(self, 
                             results: Dict[str, Any],
                             execution_time: float,
                             tests_run: int,
                             tests_passed: int,
                             tests_failed: int) -> TestSuiteResult:
        ...
        if results["performance_regression"]:
            if results["performance_regression"].success:
                ...
                reg_score = getattr(results["performance_regression"], "overall_regression_score", 0.0)
        ...
        if results["normal_flow"] and not results["normal_flow"].success:
            critical_issues.append("Normal flow test failed - basic pipeline functionality compromised")
            
        if results["large_scale"] and hasattr(results["large_scale"], "scaling_issues"):
            critical_issues.extend([
                issue for issue in results["large_scale"].scaling_issues
                if "critical" in issue.lower()
            ])
            
        if results["performance_regression"] and hasattr(results["performance_regression"], "critical_issues"):
        ...
        # Generate summary
        summary = {
            "pipeline_functionality": "✅ Working" if results["normal_flow"] and results["normal_flow"].success else "❌ Issues",
            "scalability": self._assess_scalability(results.get("large_scale")),
            "error_resilience": self._assess_error_resilience(results.get("error_recovery")),
            "performance_stability": self._assess_performance_stability(results.get("performance_regression")),
            "overall_health": "✅ Healthy" if overall_score > 0.8 else "⚠️ Needs Attention" if overall_score > 0.6 else "❌ Critical Issues"
        }
    ...
    def _assess_scalability(self, large_scale_result) -> str:
        ...
        if large_scale_result.success:
            return "✅ Good"
        elif hasattr(large_scale_result, "scaling_factor") and large_scale_result.scaling_factor < 2.0:
            return "⚠️ Acceptable"
        else:
            return "❌ Poor"
    ...
    def _assess_error_resilience(self, error_recovery_result) -> str:
        ...
        if error_recovery_result.success:
            return "✅ Resilient"
        elif hasattr(error_recovery_result, "resilience_score") and error_recovery_result.resilience_score > 0.6:
            return "⚠️ Moderate"
        else:
            return "❌ Fragile"
    ...
    def _assess_performance_stability(self, regression_result) -> str:
        ...
        if regression_result.success:
            return "✅ Stable"
        elif hasattr(regression_result, "overall_regression_score") and regression_result.overall_regression_score > 0.7:
    ...
    def _print_final_summary(self, result: TestSuiteResult) -> None:
        ...
        if result.normal_flow_result:
            ...
            print(f"   Normal Flow: {status} ({result.normal_flow_result.execution_time_seconds:.1f}s)")
        ...
        if result.performance_regression_result:
            ...
            reg_score = getattr(result.performance_regression_result, "overall_regression_score", 0)
        ...
        if result.overall_score > 0.9:
            ...
            print("👍 Good! Pipeline is performing well with minor areas for improvement.")
        elif result.overall_score > 0.6:
            print("⚠️ Caution! Pipeline has some issues that should be addressed.")
        else:
            print("🚨 Alert! Pipeline has significant issues requiring immediate attention.")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--skip-large-scale", action="store_true", 
                       help="Skip large scale test")
    parser.add_argument("--skip-error-recovery", action="store_true",
                       help="Skip error recovery test")
