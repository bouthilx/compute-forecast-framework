...
class PerformanceProfile:
    ...
    def finalize(self) -> None:
        ...
        if len(self.snapshots) >= 2:
            ...
            self.total_io_bytes = (
                (last.io_read_bytes - first.io_read_bytes) +
    ...
    def calculate_averages(self) -> Dict[str, float]:
        ...
        return {
            ...
            "memory_std_dev": statistics.stdev(memory_values) if len(memory_values) > 1 else 0.0
        }
...
class BottleneckAnalyzer:
    ...
    def analyze_profile(self, 
                       ...
                       max_memory_mb: float = 4096.0) -> List[str]:
        ...
        if io_rate > self.io_threshold_mbps:
            bottlenecks.append(
                f"High I/O rate detected ({io_rate:.1f} MB/s)"
            )
    ...
    def analyze_all_profiles(self,
                           ...
                           max_memory_mb: float = 4096.0) -> Dict[PipelinePhase, List[str]]:
    ...
    def generate_recommendations(self, bottlenecks: List[str]) -> List[str]:
        ...
        for bottleneck in bottlenecks:
            if "High CPU usage" in bottleneck:
                ...
                recommendations.append(
                    "Investigate sources of performance variance"
                )
                recommendations.append(
