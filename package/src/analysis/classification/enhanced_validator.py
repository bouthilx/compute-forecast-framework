...
class EnhancedClassificationValidator(ClassificationValidator):
    ...
    def _load_default_test_cases(self):
        ...
        self.test_cases.extend([
            ValidationCase("MIT", "academic", "Massachusetts Institute of Technology"),
            ValidationCase("Stanford University", "academic", "Stanford University"),
            ValidationCase("University of Toronto", "academic", "University of Toronto"),
            ValidationCase("ETH Zurich", "academic", "ETH Zurich"),
            ValidationCase("Cambridge University", "academic", "University of Cambridge"),
        ])
        ...
        self.test_cases.extend([
            ValidationCase("National Science Foundation", "government", "National Science Foundation"),
            ValidationCase("DARPA", "government", "DARPA"),
            ValidationCase("National Institutes of Health", "government", "National Institutes of Health"),
        ])
        
        # Non-profit test cases
        self.test_cases.extend([
            ValidationCase("Allen Institute for AI", "non_profit", "Allen Institute for AI"),
            ValidationCase("Chan Zuckerberg Initiative", "non_profit", "Chan Zuckerberg Initiative"),
        ])
    ...
    def validate_accuracy(self) -> Dict[str, float]:
        ...
        for test_case in self.test_cases:
            classification = self.enhanced_classifier.classify_with_confidence(test_case.affiliation)
        ...
        results["overall_accuracy"] = overall_correct / total_cases if total_cases > 0 else 0.0
        results["unknown_rate"] = unknown_count / total_cases if total_cases > 0 else 0.0
        results["average_confidence"] = total_confidence / total_cases if total_cases > 0 else 0.0
        ...
        for paper in papers:
            for author in paper.authors:
                if hasattr(author, 'affiliation'):
                    result = self.enhanced_classifier.classify_with_confidence(author.affiliation)
    ...
    def generate_detailed_report(self, papers: List[Paper]) -> str:
        ...
        report += f"Average Confidence: {accuracy_results['average_confidence']:.3f}\n\n"
