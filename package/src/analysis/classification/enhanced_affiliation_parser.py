    ...
    def parse_complex_affiliation(self, affiliation: str) -> Dict[str, Any]:
                ...
                for pattern in self.department_patterns:
                    ...
                    if match:
                        ...
                        for abbrev, expansion in self.abbreviations.items():
                            dept = re.sub(rf'\b{abbrev}\b', expansion, dept, flags=re.IGNORECASE)
        ...
        if not result["organization"] and paren_content:
            for content in paren_content:
                if any(keyword in content.lower() for keyword in ['university', 'institute', 'research']):
        ...
        if not result["organization"]:
            confidence *= 0.3
        if not result["department"] and any(word in affiliation.lower() for word in ['dept', 'department']):
            confidence *= 0.8
