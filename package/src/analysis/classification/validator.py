...
class ClassificationValidator(BaseValidator):
    ...
    def validate(self, papers: List[Paper]) -> Dict[str, Any]:
        ...
        validation_results['confidence_analysis'] = self._analyze_confidence_scores(papers)
        ...
        validation_results['overall_score'] = self.get_validation_score(validation_results)
    ...
    def validate_known_papers(self) -> Dict[str, float]:
        ...
        academic_accuracy = academic_correct / academic_total if academic_total > 0 else 0.0
        industry_accuracy = industry_correct / industry_total if industry_total > 0 else 0.0
    ...
    def flag_edge_cases(self, papers: List[Paper]) -> List[Dict[str, Any]]:
        ...
        for i, paper in enumerate(papers):
            ...
            if analysis.confidence < 0.5:
                edge_cases.append({
                    'paper_index': i,
                    'paper_title': paper.title if hasattr(paper, 'title') else f"Paper {i}",
            ...
            total_authors = analysis.academic_count + analysis.industry_count + analysis.unknown_count
            if total_authors > 0:
            ...
            if analysis.academic_count + analysis.industry_count > 0:
                industry_percentage = analysis.industry_count / (analysis.academic_count + analysis.industry_count)
    ...
    def _analyze_confidence_scores(self, papers: List[Paper]) -> Dict[str, float]:
        ...
        confidences = []
        category_confidences = {'academic_eligible': [], 'industry_eligible': [], 'needs_manual_review': []}
