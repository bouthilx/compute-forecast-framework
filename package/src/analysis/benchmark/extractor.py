...
class AcademicBenchmarkExtractor:
    ...
    def __init__(self):
        ...
        self.landmark_papers = [
            "bert", "gpt", "transformer", "resnet", "efficientnet",
            "yolo", "vit", "vision transformer", "alphago", "alphafold",
        ]
    
    def extract_benchmark_batch(self, 
                              papers: List[Paper], 
                              domain: BenchmarkDomain) -> ExtractionBatch:
        ...
        for paper in papers:
            ...
            if comp_analysis.resource_metrics and any(comp_analysis.resource_metrics.values()):
    ...
    def extract_computational_details(self, paper: Paper) -> Dict[str, Any]:
        ...
        if details["gpu_count"] and details["training_time_days"]:
            details["gpu_hours"] = details["gpu_count"] * details["training_time_days"] * 24
    ...
    def _extract_benchmark_datasets(self, paper: Paper, domain: BenchmarkDomain) -> List[str]:
        ...
        if domain in domain_datasets:
            for dataset in domain_datasets[domain]:
                if dataset in text:
                    datasets.append(dataset.upper() if len(dataset) <= 4 else dataset.title())
