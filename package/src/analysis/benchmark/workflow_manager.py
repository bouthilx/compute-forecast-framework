...
class ExtractionWorkflowManager:
    ...
    def detect_domain(self, paper: Paper) -> BenchmarkDomain:
        ...
        # NLP keywords
        nlp_keywords = [
            "natural language", "nlp", "language model", "bert", "gpt", 
            "transformer", "text", "sentiment", "translation", "tokeniz",
            "word", "sentence", "corpus", "vocabulary", "semantic",
        ]
        
        # CV keywords  
        cv_keywords = [
            "computer vision", "image", "visual", "convolutional", "cnn",
            "resnet", "efficientnet", "yolo", "detection", "segmentation",
            "classification", "pixel", "convolution", "imagenet", "coco",
        ]
        
        # RL keywords
        rl_keywords = [
            "reinforcement learning", "rl", "agent", "environment", "reward",
            "policy", "q-learning", "dqn", "actor-critic", "atari", "mujoco",
            "game", "simulation", "episode", "action", "state",
        ]
    ...
    def execute_parallel_extraction(self, batches: Dict[str, List[Paper]]) -> List[ExtractionBatch]:
        ...
        with ThreadPoolExecutor(max_workers=4) as executor:
            ...
            for batch_key, papers in batches.items():
                ...
                future = executor.submit(
                    self.base_extractor.extract_benchmark_batch,
                    papers, domain
                )
    ...
    def generate_extraction_report(self, results: List[ExtractionBatch]) -> pd.DataFrame:
    ...
    def identify_manual_review_candidates(self, results: List[ExtractionBatch]) -> List[Paper]:
