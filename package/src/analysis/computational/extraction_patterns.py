...
class CommonPatterns:
    ...
    PATTERNS = [
        ...
        ExtractionPattern(
            ...
            expected_fields=["nodes", "gpus_per_node", "training_time", "total_gpu_hours"],
            confidence_level="high",
...
class EdgeCaseHandler:
    ...
    @classmethod
    def estimate_from_cost(cls, cost_usd: float, provider: str = "aws", 
                          gpu_type: str = "V100") -> float:
        ...
        logger.info(f"Estimated {estimated_gpu_hours:.0f} GPU-hours from ${cost_usd} "
    ...
    def resolve_model_reference(cls, model_name: str) -> Dict[str, any]:
        ...
        for known_model, config in cls.KNOWN_MODELS.items():
            if known_model.upper().replace("-", "") in model_name_clean:
                logger.info(f"Partially resolved {model_name} to {known_model} configuration")
                return config.copy()
    ...
    def handle_multiple_experiments(cls, text: str) -> Dict[str, any]:
        ...
        if any(term in text_lower for term in ['statistical significance', 'confidence interval', 'standard deviation']):
...
class PatternMatcher:
    ...
    def identify_pattern_type(self, text: str) -> List[PatternType]:
        ...
        if any(term in text_lower for term in ['pre-training', 'fine-tuning', 'pretraining', 'finetuning']):
        ...
        if any(term in text_lower for term in ['$', 'dollar', 'cost', 'credit', 'cloud']):
            patterns.append(PatternType.CLOUD_CREDITS)
        
        # Check for relative references
        if any(term in text_lower for term in ['more than', 'times', 'compared to', 'relative to']):
    ...
    def extract_all_patterns(self, text: str) -> Dict[PatternType, Dict[str, any]]:
        ...
        for pattern_type in pattern_types:
            if pattern_type == PatternType.EXPLICIT_RESOURCE:
                ...
                results[pattern_type] = self.edge_case_handler.handle_distributed_training(text)
    ...
    def _extract_explicit_resources(self, text: str) -> Dict[str, any]:
        ...
        if time_info:
            ...
            result['training_time_hours'] = self.regex_patterns.normalize_time_to_hours(value, unit)
        ...
        if param_info:
            ...
            result['parameters_millions'] = self.regex_patterns.normalize_parameters_to_millions(value, unit)
