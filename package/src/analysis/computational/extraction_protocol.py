...
class ExtractionDecisionTree:
    ...
    DECISION_TREE = {
        ...
        "proceed_validation": {
            "action": "Move to validation phase"
        }
    }
...
class ExtractionProtocol:
    ...
    PREPARATION_KEYWORDS = [
        "computational", "gpu", "tpu", "training", "parameters",
        "dataset", "experiments", "implementation", "resources"
    ]
    ...
    MANUAL_KEYWORDS = {
        ...
        "training_time": ["hours", "days", "weeks", "training time", "wall clock", "runtime"],
        "model_scale": ["parameters", "layers", "dimensions", "model size", "architecture"],
        "dataset": ["dataset", "samples", "tokens", "size", "batch size", "sequence length"],
    ...
    def phase1_preparation(self) -> Dict[str, Any]:
        ...
        computational_indicators = sum(
            1 for keyword in self.PREPARATION_KEYWORDS 
            if keyword.lower() in self.paper_content.lower()
        )
        preparation_results["has_computational_experiments"] = computational_indicators >= 3
        
        # 1.2 Quick Scan Locations
        for location in self.SCAN_LOCATIONS:
            section_content = self._find_section_content(location)
            if section_content:
                preparation_results["quick_scan_results"][location] = len(section_content)
                preparation_results["structure_notes"].append(f"Found {location} section")
    ...
    def phase2_automated_extraction(self, analyzer) -> Dict[str, Any]:
        ...
        self.extraction_result.automated_extraction = {
            ...
            "fields_found": list(automated_results.__dict__.keys()) if hasattr(automated_results, '__dict__') else [],
            "fields_missing": [],
    ...
    def phase5_documentation(self) -> Dict[str, Any]:
        ...
        logger.info(f"Total extraction time: {self.extraction_result.metadata.time_spent_minutes} minutes")
    ...
    def _find_section_content(self, section_name: str) -> str:
        ...
        for pattern in patterns:
            if pattern in content_lower:
                ...
                return self.paper_content[start_idx:start_idx + 1000]  # Return up to 1000 chars
    ...
    def _extract_computation_specs(self) -> Dict[str, Any]:
        ...
        elif "cost" in content_lower and ("$" in self.paper_content or "dollar" in content_lower):
    ...
    def _run_consistency_checks(self) -> List[str]:
        ...
        if (self.extraction_result.hardware.gpu_count and 
            self.extraction_result.training.total_time_hours):
            calculated_gpu_hours = (self.extraction_result.hardware.gpu_count * 
                                  self.extraction_result.training.total_time_hours)
            if (self.extraction_result.computation.total_gpu_hours and
                abs(calculated_gpu_hours - self.extraction_result.computation.total_gpu_hours) > 
                calculated_gpu_hours * 0.1):  # 10% tolerance
        ...
        if self.extraction_result.model.parameters_count:
            if self.extraction_result.model.parameters_count > 1000000:  # > 1T parameters
    ...
    def _assess_confidence(self, category: str) -> ConfidenceLevel:
        ...
        non_none_fields = sum(1 for value in category_data.__dict__.values() if value is not None)
    ...
    def _assess_overall_confidence(self, validation: ValidationResults) -> ConfidenceLevel:
        ...
        total_score = (confidence_scores[validation.confidence_hardware] +
    ...
    def _detect_outliers(self) -> List[str]:
        ...
        if (self.extraction_result.training.total_time_hours and 
            self.extraction_result.training.total_time_hours > 8760):  # > 1 year
            outliers.append("Training time exceeds 1 year")
        
        if (self.extraction_result.computation.total_gpu_hours and
            self.extraction_result.computation.total_gpu_hours > 1000000):  # > 1M GPU-hours
    ...
    def _generate_extraction_summary(self) -> str:
        ...
        if self.extraction_result.hardware.gpu_type:
            summary_parts.append(f"Hardware: {self.extraction_result.hardware.gpu_type}")
        
        if self.extraction_result.training.total_time_hours:
            summary_parts.append(f"Training: {self.extraction_result.training.total_time_hours} hours")
        
        if self.extraction_result.model.parameters_count:
            summary_parts.append(f"Model: {self.extraction_result.model.parameters_count}M parameters")
        
        if self.extraction_result.computation.total_gpu_hours:
            summary_parts.append(f"Compute: {self.extraction_result.computation.total_gpu_hours} GPU-hours")
        
        return " | ".join(summary_parts) if summary_parts else "Limited extraction results"
    ...
    def _calculate_completeness_score(self) -> float:
        ...
        for spec_category in [self.extraction_result.hardware, 
    ...
    def _calculate_quality_score(self) -> float:
        ...
        confidence_score = confidence_scores.get(self.extraction_result.validation.confidence_overall, 0.0)
        consistency_score = 1.0 if self.extraction_result.validation.consistency_checks_passed else 0.5
    ...
    def run_full_protocol(self, analyzer) -> ExtractionResult:
        ...
        try:
            ...
            logger.info(f"Extraction protocol completed successfully for {self.paper_id}")
            return self.extraction_result
            
        except Exception as e:
            logger.error(f"Extraction protocol failed for {self.paper_id}: {str(e)}")
            self.extraction_result.notes.quality_issues.append(f"Protocol failed: {str(e)}")
            raise