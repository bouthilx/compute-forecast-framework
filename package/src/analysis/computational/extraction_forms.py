...
class ExtractionFormTemplate:
    ...
    def get_example_template() -> Dict[str, Any]:
        ...
        template.update({
            ...
            "extraction_notes": {
                "paper_sections_reviewed": ["abstract", "experimental_setup", "appendix_a"],
...
class FormValidator:
    ...
    def validate_form(self, form_data: Dict[str, Any]) -> FormValidationResult:
        ...
        for section, fields in self.RECOMMENDED_FIELDS.items():
            ...
            for field in fields:
                total_recommended += 1
                if (field in form_data[section] and 
                    form_data[section][field] is not None and
                    form_data[section][field] != ""):
                    filled_recommended += 1
                else:
                    warnings.append(f"Recommended field missing: {section}.{field}")
        
        # Calculate completeness score
        completeness_score = filled_recommended / total_recommended if total_recommended > 0 else 1.0
    ...
    def _validate_confidence_levels(self, form_data: Dict[str, Any]) -> List[str]:
        ...
        if "validation" in form_data:
            confidence_fields = [
                "confidence_hardware", "confidence_training", "confidence_model",
                "confidence_dataset", "confidence_computation", "confidence_overall"
            ]
            ...
            for field in confidence_fields:
                if field in form_data["validation"]:
                    ...
                    if value not in valid_levels:
                        errors.append(f"Invalid confidence level '{value}' in {field}. Must be one of: {valid_levels}")
    ...
    def _validate_consistency(self, form_data: Dict[str, Any]) -> List[str]:
        ...
        if ("hardware" in form_data and "training" in form_data and "computation" in form_data):
            ...
            if all(x is not None for x in [gpu_count, training_hours, total_gpu_hours]):
                ...
                if abs(calculated - total_gpu_hours) > calculated * 0.1:  # 10% tolerance
                    warnings.append(f"GPU-hours inconsistent: calculated {calculated}, reported {total_gpu_hours}")
        ...
        if "model" in form_data:
            ...
            if params is not None and size_gb is not None:
                ...
                if abs(estimated_size - size_gb) > estimated_size * 0.5:  # 50% tolerance
...
class FormManager:
    ...
    def _create_default_templates(self):
        ...
        if not example_template_path.exists():
            ...
            ExtractionFormTemplate.save_template(example_template, example_template_path)
    
    def create_form_for_paper(self, paper_id: str, analyst: str, 
                             paper_title: str = "", paper_year: Optional[int] = None) -> Dict[str, Any]:
    ...
    def save_form(self, form_data: Dict[str, Any], paper_id: str, 
                  forms_dir: Optional[Path] = None) -> Path:
    ...
    def convert_extraction_result_to_form(self, result: ExtractionResult) -> Dict[str, Any]:
        ...
        if result.hardware:
            form["hardware"].update({k: v for k, v in asdict(result.hardware).items() if v is not None})
        
        if result.training:
            form["training"].update({k: v for k, v in asdict(result.training).items() if v is not None})
        
        if result.model:
            form["model"].update({k: v for k, v in asdict(result.model).items() if v is not None})
        
        if result.dataset:
            form["dataset"].update({k: v for k, v in asdict(result.dataset).items() if v is not None})
        
        if result.computation:
            form["computation"].update({k: v for k, v in asdict(result.computation).items() if v is not None})
    ...
    def generate_form_summary(self, form_data: Dict[str, Any]) -> str:
        ...
        if hardware.get("gpu_type") or hardware.get("gpu_count"):
            gpu_info = f"{hardware.get('gpu_count', '?')} × {hardware.get('gpu_type', 'GPU')}"
