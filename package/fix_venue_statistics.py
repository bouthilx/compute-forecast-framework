...
class VenueStatisticsFixer:
    ...
    def __init__(self, input_file: str = "data/mila_venue_statistics.json"):
        ...
        self.additional_venue_patterns = [
            ...
            (r'Proceedings of the .*Conference on Computer Vision and Pattern Recognition', 'CVPR'),  
            (r'Proceedings of the .*International Conference on Computer Vision', 'ICCV'),
    ...
    def fix_venue_normalization(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for old_venue, venue_data in venue_counts.items():
            ...
            new_venue_counts[new_venue]['unique_paper_count'] += venue_data['unique_paper_count']
        ...
        logger.info(f"Normalized venues from {len(venue_counts)} to {len(new_venue_counts)}")
        return data
    ...
    def enhance_worker3_interface(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        required_sections = ['computational_scores', 'citation_averages', 'venue_types', 'year_coverage', 'domain_diversity']
        ...
        venue_rankings = {}
        sorted_venues = sorted(venue_counts.items(), key=lambda x: x[1]['total'], reverse=True)
        
        for rank, (venue, venue_data) in enumerate(sorted_venues, 1):
            venue_rankings[venue] = {
                'rank': rank,
                'percentile': round((len(sorted_venues) - rank + 1) / len(sorted_venues) * 100, 1),
                'tier': 'top' if rank <= 10 else 'high' if rank <= 50 else 'medium' if rank <= 150 else 'low'
            }
        ...
        venue_metadata['summary_stats'] = {
            ...
            'major_conferences': [venue for venue in venue_counts.keys() 
                                if any(conf in venue.upper() for conf in ['NEURIPS', 'ICML', 'ICLR', 'CVPR', 'ICCV', 'AAAI'])],
            'venue_type_distribution': dict(Counter(venue_metadata['venue_types'].values())),
            'years_with_data': sorted(list(set().union(*[venue_data['by_year'].keys() for venue_data in venue_counts.values()])))
        }
    ...
    def update_analysis_summary(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        active_venues = sum(1 for venue_data in venue_counts.values() if venue_data['total'] > 1)
        ...
        data['analysis_summary'].update({
            ...
            'venue_coverage_rate': round(active_venues / total_venues, 3) if total_venues > 0 else 0,
    ...
    def validate_data_integrity(self, data: Dict[str, Any]) -> bool:
        ...
        for venue, venue_data in venue_counts.items():
            ...
            if by_year_sum != venue_data['total']:
                issues.append(f"Count mismatch for {venue}: total={venue_data['total']}, by_year_sum={by_year_sum}")
                
        # Check 2: Future data
        for venue, venue_data in venue_counts.items():
            invalid_years = [year for year in venue_data['by_year'].keys() if year not in self.valid_years]
            if invalid_years:
        ...
        missing_sections = [section for section in required_sections if section not in data['venue_metadata']]
        if missing_sections:
...
def main():
    ...
    print(f"Data quality fixes applied: {summary.get('data_quality_fixes_applied', False)}")
    print(f"{'='*60}\n")
