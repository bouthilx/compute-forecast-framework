...
def calculate_complete_paper_flow():
    ...
    print(f"Papers WITH analysis: {len(papers_with_analysis):,} ({len(papers_with_analysis)/total_papers*100:.1f}%)")
    print(f"Papers WITHOUT analysis: {len(papers_without_analysis):,} ({len(papers_without_analysis)/total_papers*100:.1f}%)")
    print(f"Total accounted: {len(papers_with_analysis) + len(papers_without_analysis):,}")
    ...
    neither_method = papers_with_analysis - papers_with_research_domains - papers_with_datasets
    ...
    print(f"Sub-total classified: {len(research_only) + len(dataset_only) + len(both_methods):,}")
    print(f"Sub-total analyzed: {len(papers_with_analysis):,}")
    
    # Verify step 3
    classified_total = len(research_only) + len(dataset_only) + len(both_methods) + len(neither_method)
    assert classified_total == len(papers_with_analysis), f"Mismatch: {classified_total} vs {len(papers_with_analysis)}"
    ...
    print(f"Union of both methods: {len(papers_with_research_domains | papers_with_datasets):,}")
...
def analyze_domain_breakdown_with_accounting():
    ...
    for category, papers in sorted(papers_by_research_category.items(), key=lambda x: len(x[1]), reverse=True):
        count = len(papers)
    ...
    print(f"(Note: Papers can have multiple domains)")
    
    print(f"\nDATASET/ENVIRONMENT METHOD ({stats['total_datasets']:,} papers):")
    print("┌────────────────────────────────────────┐")
    total_dataset_assigned = 0
    for category, papers in sorted(papers_by_dataset_category.items(), key=lambda x: len(x[1]), reverse=True):
        count = len(papers)
    ...
    print(f"(Note: Each paper has exactly one primary domain)")
    
    # Show overlap analysis for major domains
    print(f"\nOVERLAP ANALYSIS FOR MAJOR DOMAINS:")
    print("┌────────────────────────────────────┬─────────┬─────────┬─────────┬─────────┐")
    print("│ Domain                             │Research │Dataset  │ Overlap │ Union   │")
    print("├────────────────────────────────────┼─────────┼─────────┼─────────┼─────────┤")
    ...
    for dataset_domain, research_domain in domain_mapping.items():
        ...
        print(f"│ {dataset_domain[:30]:<30} │ {len(research_papers):>7} │ {len(dataset_papers):>7} │ {len(overlap):>7} │ {len(union):>7} │")
