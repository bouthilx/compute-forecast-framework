...
def extract_multi_domain_classifications():
    ...
    for paper_id, domain_entries in papers_domains.items():
        ...
        for domain_entry in domain_entries:
            ...
            for category, info in taxonomy.items():
                matches = sum(1 for keyword in info['keywords'] if keyword in domain_text)
                if matches > 0:
    ...
    print(f"Papers with multi-domain classification: {len(multi_domain_classifications)}")
    ...
    print(f"Average domains per paper: {total_domain_assignments / len(multi_domain_classifications):.2f}")
    
    # Analyze multi-domain papers
    multi_domain_papers = {pid: info for pid, info in multi_domain_classifications.items() 
...
def analyze_interdisciplinary_patterns():
    ...
    multi_classifications, domain_counts, multi_papers = extract_multi_domain_classifications()
    ...
    for domain1, connections in cooccurrence.items():
        for domain2, count in connections.items():
            if domain1 < domain2:  # Avoid duplicates
                strength = count / min(domain_counts[domain1], domain_counts[domain2]) * 100
    ...
    for domain1, domain2, count, strength in connection_strengths[:10]:
        print(f"  {domain1} ↔ {domain2}: {count} papers ({strength:.1f}% connection rate)")
    ...
    for year in sorted(yearly_multi_domain.keys()):
        ...
        print(f"  {year}: {len(papers)} multi-domain papers, avg {avg_domains:.2f} domains/paper")
...
def compare_single_vs_multi_domain():
    ...
    multi_classifications, multi_domain_counts, multi_papers = extract_multi_domain_classifications()
    
    print("Comparison of classification methods:")
    print("-" * 80)
    print(f"{'Domain':<35} {'Single':<10} {'Multi':<10} {'Difference':<12} {'Multi %':<10}")
    print("-" * 80)
    ...
    for domain in sorted(multi_domain_counts.keys()):
        ...
        print(f"{domain:<35} {single_count:<10} {multi_count:<10} {difference:+<12} {multi_pct:<10.1f}%")
    
    print("-" * 80)
    print(f"{'TOTAL':<35} {total_papers_single:<10} {sum(multi_domain_counts.values()):<10}")
    ...
    print(f"This means multi-domain classification assigns {inflation_factor:.1f}x more domain labels")
...
def create_corrected_multi_domain_stats():
    ...
    print("=" * 100)
    print(f"{'Domain':<35} {'Original':<12} {'Multi-Only':<12} {'Dataset+Multi':<15} {'Final %':<10}")
    print("=" * 100)
    ...
    for domain in method2_corrections.keys():
        ...
        multi_adjusted = dataset_corrected * (multi_only / original) if original > 0 else multi_only
        
        final_corrected_stats[domain] = multi_adjusted
    
    total_final = sum(final_corrected_stats.values())
    
    for domain in sorted(final_corrected_stats.keys(), key=lambda x: final_corrected_stats[x], reverse=True):
        original = correction_data['original_stats'].get(domain, 0)
        multi_only = multi_stats.get(domain, 0)
        final_count = final_corrected_stats[domain]
        final_pct = final_count / total_final * 100
        
        print(f"{domain:<35} {original:<12} {multi_only:<12} {final_count:<15.0f} {final_pct:<10.1f}%")
    
    print("=" * 100)
    print(f"{'TOTAL':<35} {sum(correction_data['original_stats'].values()):<12} {sum(multi_stats.values()):<12} {total_final:<15.0f} {'100.0':<10}%")
    ...
    final_results = {
        ...
        'corrections_applied': ['Multi-domain support', 'Dataset detection rate adjustment', 'Proportional scaling']
    }
...
def main():
    ...
    print("\\n=== KEY INSIGHTS ===")
    print(f"1. {len(multi_papers)} papers ({len(multi_papers)/len(multi_classifications)*100:.1f}%) are interdisciplinary")
    print(f"2. Average domains per paper: {sum(domain_counts.values())/len(multi_classifications):.2f}")
    print(f"3. Domain assignment inflation: {comparison['inflation_factor']:.2f}x")
    print("4. Multi-domain approach provides more accurate representation of research breadth")
