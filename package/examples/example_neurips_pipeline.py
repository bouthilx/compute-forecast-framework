...
class EnhancedNeurIPS2024Pipeline:
        ...
        if self.use_google_drive:
            try:
                ...
                if not os.path.exists('credentials.json'):
                    logger.warning("Google Drive credentials.json not found - disabling Drive storage")
    ...
    def collect_neurips_2024_papers(self, limit: int = 3) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for paper in all_papers:
                ...
                if any(kw in paper.title.lower() for kw in ["neural", "learning", "model"]):
                    # Discover PDF URL
                    pdf_url = None
                    if paper.urls:
                    ...
                    if pdf_url:
                        paper_dict = {
                            "paper_id": paper.paper_id or f"neurips_2024_{len(collected_papers)}",
                            "title": paper.title,
    ...
    def download_and_store_pdfs(self, papers: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for paper in papers:
            ...
            try:
                ...
                try:
                    ...
                    is_valid = validator.validate_affiliations(affiliation_result, paper_metadata)
                    ...
                    extraction_log['affiliation_extraction']['validation'] = {
                        ...
                        'affiliations_found': len(affiliation_result.get('affiliations', []))
                    }
                    
                    logger.debug(f"Affiliation validation: {is_valid}")
                    logger.debug(f"Confidence: {affiliation_result.get('confidence', 0.0)}")
                    logger.debug(f"Text length: {len(affiliation_result.get('text', ''))}")
                ...
                if full_text:
                    ...
                    logger.info(f"  Text length: {len(full_text)} chars, {len(full_text.split())} words")
                else:
    ...
    def run_complete_pipeline(self, num_papers: int = 3) -> Dict[str, Any]:
        ...
        try:
            ...
            self.results['pipeline_summary'] = {
                ...
                'success_rate': round((self.results['successful_extractions'] / 
                                     max(1, self.results['successful_extractions'] + self.results['failed_extractions'])) * 100, 1),
    ...
    def _print_pipeline_summary(self) -> None:
        ...
        logger.info(f"   Google Drive: {'Enabled' if summary['google_drive_enabled'] else 'Disabled'}")
        
        logger.info(f"\n📄 PAPER PROCESSING:")
        logger.info(f"   Papers Collected: {self.results['papers_collected']}")
        logger.info(f"   PDFs Downloaded: {self.results['pdfs_downloaded']}")
        if self.use_google_drive:
            logger.info(f"   PDFs Uploaded to Drive: {self.results['pdfs_uploaded_to_drive']}")
        ...
        for detail in self.results['extraction_details'][:3]:  # Show first 3
            ...
            if aff_info.get('status') == 'extracted':
                ...
                logger.info(f"   - Affiliation extraction: {'PASSED' if validation.get('passed') else 'FAILED'}")
                logger.info(f"     Confidence: {aff_info.get('confidence', 0):.3f} (threshold: {validation.get('confidence_threshold', 0.5)})")
                logger.info(f"     Text length: {aff_info.get('text_length', 0)} (min: {validation.get('min_text_length', 20)})")
                logger.info(f"     Affiliations found: {validation.get('affiliations_found', 0)}")
            else:
                logger.info(f"   - Affiliation extraction: FAILED - {aff_info.get('error', 'Unknown error')}")
            
            # Full text extraction
            full_info = detail.get('full_text_extraction', {})
            if full_info.get('status') == 'success':
                logger.info(f"   - Full text: {full_info.get('word_count', 0)} words in {full_info.get('extraction_time', 0):.2f}s")
    ...
    def _save_results(self) -> None:
        ...
        results_file = self.cache_dir / f"pipeline_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def main():
    ...
    parser.add_argument("--no-drive", action="store_true", help="Disable Google Drive storage")
    parser.add_argument("--drive-folder", type=str, help="Google Drive folder ID")
    parser.add_argument("--cache-dir", type=str, default="neurips_2024_enhanced_cache", 
