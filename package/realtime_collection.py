...
DOMAINS = {
    ...
    "Natural Language Processing": [
        "natural language processing", "NLP", "language model", "text analysis", "transformer"
    ],
    ...
    "Scientific Computing & Applications": [
        "computational biology", "scientific computing", "numerical methods", "simulation"
    ]
}
...
class RealTimeTracker:
    ...
    def load_existing_papers(self):
        ...
        try:
            ...
            return papers, {paper.get('title', '').lower().strip() for paper in papers if paper.get('title')}
    ...
    def print_status(self):
        ...
        print("\n" + "="*80)
        print(f"🚀 WORKER 6 PAPER COLLECTION - REAL-TIME STATUS")
        print(f"⏱️  Runtime: {elapsed}")
        print(f"📊 Progress: {self.stats['total_papers']}/{TARGET_TOTAL} papers ({progress_pct:.1f}%)")
        print(f"📈 New this session: {self.stats['new_papers']} papers")
        print(f"🔧 API calls: {self.stats['api_calls']} | Rate limits: {self.stats['rate_limits']} | Errors: {self.stats['errors']}")
        ...
        for domain, years in self.domain_stats.items():
            ...
            year_str = " | ".join([f"{year}:{count}" for year, count in sorted(years.items())])
            print(f"    {year_str}")
    ...
    try:
        ...
        for domain_name, keywords in DOMAINS.items():
            ...
            for year in YEARS:
                ...
                target = max(12, current_count + 5)  # Target at least 12 per domain/year
                needed = target - current_count
                
                if needed > 0:
                    tracker.log(f"📅 {year}: need {needed} more papers (have {current_count}, target {target})")
                    ...
                    for keyword in keywords[:3]:
                        ...
                        # Semantic Scholar
                        ss_papers = collector.semantic_scholar_search(keyword, year, limit=8)
                        
                        # Process results
                        for paper in ss_papers:
                            title = paper.get('title', '').lower().strip()
                            if title and title not in existing_titles and len(year_papers) < needed:
                                ...
                                tracker.log(f"➕ Added: \"{paper.get('title', '')[:50]}...\" ({paper.get('citations', 0)} cites)")
                    ...
                    tracker.domain_stats[domain_name][str(year)] = current_count + len(year_papers)
                    
                    tracker.log(f"✅ {domain_name} {year}: collected {len(year_papers)} papers")
                    
                    # Save progress every 20 new papers
                    if tracker.stats['new_papers'] % 20 == 0 and tracker.stats['new_papers'] > 0:
                    ...
                    tracker.log(f"✅ {domain_name} {year}: target already met ({current_count} papers)")
                ...
                if len(all_papers) >= TARGET_TOTAL:
                    tracker.log(f"🎯 TARGET REACHED! {len(all_papers)} papers collected!")
                    break
        ...
        final_stats = {
            ...
            'domain_distribution': {domain: dict(years) for domain, years in tracker.domain_stats.items()},
            'collection_metadata': {
