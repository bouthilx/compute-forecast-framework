...
class SuppressionIndicators:
    ...
    experimental_scope: Dict = field(default_factory=lambda: {
        "num_ablations": None,
        "num_seeds": None,
        "num_baselines": None,
    ...
    scale_analysis: Dict = field(default_factory=lambda: {
    ...
    method_classification: Dict = field(default_factory=lambda: {
        "efficiency_focused": False,
        "compute_saving_techniques": [],
        "method_type": None  # "compute_intensive" or "compute_efficient"
    })
    
    explicit_constraints: Dict = field(default_factory=lambda: {
...
class SuppressionTemplates:
    ...
    SUPPRESSION_PATTERNS = {
        "ablation_count": re.compile(
            r'\b(\d+)\s*ablation(?:s)?\s*(?:stud(?:y|ies)|experiment)',
            re.IGNORECASE
        ),
        ...
        "early_stopping": re.compile(
            r'(?:stopped?|terminated?|ended?)\s*(?:early|before|prior)',
            re.IGNORECASE
        ),
    ...
    def _create_extractor(cls):
        ...
        def extract_suppression_indicators(text: str) -> SuppressionIndicators:
            ...
            if ablation_match:
                indicators.experimental_scope["num_ablations"] = int(ablation_match.group(1))
            ...
            if baseline_match:
                indicators.experimental_scope["num_baselines"] = int(baseline_match.group(1))
            
            variant_match = cls.SUPPRESSION_PATTERNS["model_variants"].search(text)
            if variant_match:
                indicators.experimental_scope["num_model_variants"] = int(variant_match.group(1))
            ...
            if limited_match:
                ...
                if count and int(count) <= 3:
                    indicators.experimental_scope["standard_experiments_missing"].append(
                        "limited_experiment_count"
                    )
            ...
            if subsample_match:
                ...
                indicators.scale_analysis["subsample_ratio"] = float(subsample_match.group(1)) / 100
            
            # Extract method classification
            efficiency_matches = cls.SUPPRESSION_PATTERNS["efficiency_techniques"].findall(text)
            if efficiency_matches:
                indicators.method_classification["efficiency_focused"] = True
                indicators.method_classification["compute_saving_techniques"] = list(set(
                    match.lower() for match in efficiency_matches
                ))
            ...
            if not indicators.method_classification["method_type"]:
                if indicators.explicit_constraints["mentions_constraints"] or \
                   indicators.method_classification["efficiency_focused"]:
                    indicators.method_classification["method_type"] = "compute_efficient"
                else:
                    indicators.method_classification["method_type"] = "compute_intensive"
    ...
    def calculate_suppression_score(indicators: SuppressionIndicators) -> float:
        ...
        if indicators.experimental_scope["num_ablations"] is not None:
            ...
            ablation_score = max(0, 1 - (indicators.experimental_scope["num_ablations"] / 5))
        ...
        if indicators.experimental_scope["num_baselines"] is not None:
            ...
            baseline_score = max(0, 1 - (indicators.experimental_scope["num_baselines"] / 3))
        ...
        if indicators.scale_analysis["parameter_percentile"] is not None:
            ...
            percentile_score = 1 - (indicators.scale_analysis["parameter_percentile"] / 100)
