...
class ExtractionWorkflowOrchestrator:
    ...
    def run_extraction_workflow(self, paper_content: str, paper_id: str, 
                               analyst: str, paper_metadata: Optional[Dict[str, Any]] = None) -> WorkflowResult:
        ...
        try:
            ...
            self._run_stage_initialization(workflow_result, paper_content, paper_metadata)
            ...
            workflow_result.total_duration_seconds = (datetime.now() - start_time).total_seconds()
            
            logger.info(f"Workflow {workflow_id} completed successfully in {workflow_result.total_duration_seconds:.1f}s")
    ...
    def _run_stage_initialization(self, workflow_result: WorkflowResult, 
                                 paper_content: str, paper_metadata: Optional[Dict[str, Any]]):
    ...
    def _run_stage_automated_analysis(self, workflow_result: WorkflowResult, paper_content: str):
        ...
        try:
            ...
            protocol = ExtractionProtocol(paper_content, workflow_result.paper_id, workflow_result.analyst)
    ...
    def _run_stage_pattern_matching(self, workflow_result: WorkflowResult, paper_content: str):
        ...
        try:
            ...
            if workflow_result.extraction_result:
                self._merge_pattern_results(workflow_result.extraction_result, pattern_results)
    ...
    def _run_stage_manual_extraction(self, workflow_result: WorkflowResult, paper_content: str):
    ...
    def _run_stage_quality_validation(self, workflow_result: WorkflowResult):
        ...
        try:
            if not workflow_result.extraction_result:
                raise ValueError("No extraction result available for quality validation")
            
            # Run quality checks
            quality_report = self.quality_controller.run_quality_checks(workflow_result.extraction_result)
            ...
            if quality_report.overall_score < self.config.quality_threshold:
                ...
                step.warnings.append(f"Quality score {quality_report.overall_score:.2f} below threshold {self.config.quality_threshold}")
    ...
    def _run_stage_form_generation(self, workflow_result: WorkflowResult):
        ...
        try:
            ...
            form_data = self.form_manager.convert_extraction_result_to_form(workflow_result.extraction_result)
    ...
    def _run_stage_review_preparation(self, workflow_result: WorkflowResult):
        ...
        try:
            ...
            step.result = {
                ...
                "requires_review": workflow_result.overall_status == WorkflowStatus.REQUIRES_REVIEW
            }
    ...
    def _should_run_manual_extraction(self, workflow_result: WorkflowResult) -> bool:
        ...
        if automated_step and automated_step.result:
            ...
            if confidence >= self.config.automated_confidence_threshold and self.config.skip_manual_if_high_confidence:
                logger.info(f"Skipping manual extraction due to high automated confidence: {confidence:.2f}")
                return False
        
        return True
    
    def _merge_pattern_results(self, extraction_result: ExtractionResult, 
        ...
        for pattern_type, results in pattern_results.items():
            if pattern_type == PatternType.EXPLICIT_RESOURCE:
                ...
                if "training_time_hours" in results and not extraction_result.training.total_time_hours:
                    extraction_result.training.total_time_hours = results["training_time_hours"]
            ...
            # Add pattern notes
            extraction_result.notes.assumptions.append(f"Used {pattern_type.value} pattern for extraction")
    ...
    def _generate_workflow_recommendations(self, workflow_result: WorkflowResult) -> List[str]:
        ...
        failed_steps = [step for step in workflow_result.steps if step.status == WorkflowStatus.FAILED]
        if failed_steps:
        ...
        if workflow_result.quality_report:
            if workflow_result.quality_report.overall_score < 0.6:
                ...
                recommendations.append("Quality score is moderate - manual review recommended")
        
        # Check extraction completeness
        if workflow_result.extraction_result:
            completeness = self._calculate_extraction_completeness(workflow_result.extraction_result)
            if completeness < 0.5:
                recommendations.append("Extraction completeness is low - additional sources may be needed")
        
        # Check timing
        if workflow_result.total_duration_seconds > 3600:  # > 1 hour
            recommendations.append("Extraction took longer than expected - review process efficiency")
        
        # Add quality recommendations
        if workflow_result.quality_report and workflow_result.quality_report.recommendations:
    ...
    def _generate_review_summary(self, workflow_result: WorkflowResult) -> str:
        ...
        if workflow_result.quality_report:
            ...
            lines.append(f"  Overall Score: {workflow_result.quality_report.overall_score:.2f}")
            lines.append(f"  Critical Issues: {len(workflow_result.quality_report.critical_issues)}")
    ...
    def _calculate_extraction_completeness(self, extraction_result: ExtractionResult) -> float:
        """Calculate overall completeness of extraction."""
        total_fields = 0
        filled_fields = 0
        
        # Check main data fields
        for category in [extraction_result.hardware, extraction_result.training, 
                        extraction_result.model, extraction_result.dataset,
    ...
    def save_workflow_result(self, workflow_result: WorkflowResult, 
        ...
        if workflow_result.quality_report:
            quality_path = output_path / "quality_reports" / f"{workflow_result.paper_id}_quality.txt"
            quality_path.parent.mkdir(parents=True, exist_ok=True)
            
            quality_text = self.quality_controller.generate_quality_report_text(workflow_result.quality_report)
            quality_path.write_text(quality_text)
            logger.info(f"Saved quality report to {quality_path}")
        
        # Save workflow summary
        summary_path = output_path / "summaries" / f"{workflow_result.workflow_id}_summary.txt"
