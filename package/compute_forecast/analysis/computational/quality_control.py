...
class CompletenessChecker:
    ...
    def check_completeness(self, extraction: ExtractionResult) -> QualityCheckResult:
        ...
        important_score = filled_important / total_important if total_important > 0 else 1.0
...
class AccuracyChecker:
    ...
    def _check_gpu_accuracy(self, hardware) -> List[QualityIssue]:
        ...
        if gpu_type in self.GPU_SPECS:
            ...
            if (hardware.gpu_memory_gb and 
                abs(hardware.gpu_memory_gb - expected_memory) > expected_memory * 0.1):
                issues.append(QualityIssue(
    ...
    def _check_model_accuracy(self, model) -> List[QualityIssue]:
        ...
        for arch_pattern, (min_params, max_params) in self.ARCHITECTURE_PARAMS.items():
            if arch_pattern.upper() in architecture:
                if (model.parameters_count < min_params or 
                    model.parameters_count > max_params):
                    issues.append(QualityIssue(
                        check_type=QualityCheckType.ACCURACY,
                        level=QualityIssueLevel.WARNING,
    ...
    def _check_temporal_accuracy(self, extraction: ExtractionResult) -> List[QualityIssue]:
...
class ConsistencyChecker:
    ...
    def _check_gpu_hours_consistency(self, extraction: ExtractionResult) -> List[QualityIssue]:
        """Check consistency between GPU count, training time, and total GPU-hours."""
        issues = []
        
        if (extraction.hardware.gpu_count and 
            extraction.training.total_time_hours and
            extraction.computation.total_gpu_hours):
            
            calculated_gpu_hours = extraction.hardware.gpu_count * extraction.training.total_time_hours
            ...
            if abs(calculated_gpu_hours - reported_gpu_hours) > tolerance:
                issues.append(QualityIssue(
                    ...
                    expected_range=(calculated_gpu_hours - tolerance, calculated_gpu_hours + tolerance)
                ))
        
        return issues
    
    def _check_model_size_consistency(self, extraction: ExtractionResult) -> List[QualityIssue]:
        """Check consistency between parameter count and model size."""
        issues = []
        
        if (extraction.model.parameters_count and 
            extraction.model.model_size_gb):
            
            # Rough estimate: 4 bytes per parameter (float32)
            estimated_size_gb = (extraction.model.parameters_count * 1_000_000 * 4) / (1024**3)
            ...
            if abs(estimated_size_gb - reported_size_gb) > tolerance:
                issues.append(QualityIssue(
                    ...
                    expected_range=(estimated_size_gb - tolerance, estimated_size_gb + tolerance)
                ))
        
        return issues
    
    def _check_cost_consistency(self, extraction: ExtractionResult) -> List[QualityIssue]:
        """Check consistency between computational resources and estimated cost."""
        issues = []
        
        if (extraction.computation.total_gpu_hours and 
    ...
    def _check_dataset_training_consistency(self, extraction: ExtractionResult) -> List[QualityIssue]:
        ...
        if (extraction.dataset.samples_count and 
            extraction.training.total_time_hours):
            
            # Very rough heuristic: > 1M samples should take > 1 hour
            if (extraction.dataset.samples_count > 1_000_000 and 
                extraction.training.total_time_hours < 1):
                issues.append(QualityIssue(
                    check_type=QualityCheckType.CONSISTENCY,
...
class PlausibilityChecker:
    ...
    def check_plausibility(self, extraction: ExtractionResult) -> QualityCheckResult:
        ...
        checks = [
            ...
            ("gpu_memory_gb", extraction.hardware.gpu_memory_gb, "hardware.gpu_memory_gb"),
            ("training_hours", extraction.training.total_time_hours, "training.total_time_hours"),
            ("parameters_millions", extraction.model.parameters_count, "model.parameters_count"),
            ("dataset_size_gb", extraction.dataset.size_gb, "dataset.size_gb"),
            ("gpu_hours", extraction.computation.total_gpu_hours, "computation.total_gpu_hours"),
            ("cost_usd", extraction.computation.estimated_cost_usd, "computation.estimated_cost_usd")
        ]
        ...
        for range_key, value, field_name in checks:
            if value is not None and range_key in self.PLAUSIBILITY_RANGES:
                ...
                if value < min_val or value > max_val:
                    level = QualityIssueLevel.CRITICAL if (value < min_val * 0.1 or value > max_val * 10) else QualityIssueLevel.WARNING
                    issues.append(QualityIssue(
...
class QualityController:
    ...
    def run_quality_checks(self, extraction: ExtractionResult) -> QualityReport:
        ...
        logger.info(f"Running quality checks for extraction {extraction.metadata.paper_id}")
        ...
        for check_name, check_function in checks:
            try:
                ...
                logger.debug(f"{check_name} check completed: score={result.score:.2f}, issues={len(result.issues)}")
        ...
        critical_issues = [issue for issue in all_issues if issue.level == QualityIssueLevel.CRITICAL]
        warnings = [issue for issue in all_issues if issue.level == QualityIssueLevel.WARNING]
        
        # Calculate overall score (weighted average)
        weights = {"completeness": 0.3, "accuracy": 0.25, "consistency": 0.25, "plausibility": 0.2}
        overall_score = sum(scores.get(check, 0) * weight for check, weight in weights.items())
        
        # Generate recommendations
        recommendations = self._generate_recommendations(critical_issues, warnings, scores)
        ...
        logger.info(f"Quality checks completed: overall_score={overall_score:.2f}, "
                   f"critical_issues={len(critical_issues)}, warnings={len(warnings)}")
        
        return report
    
    def _generate_recommendations(self, critical_issues: List[QualityIssue], 
                                warnings: List[QualityIssue], scores: Dict[str, float]) -> List[str]:
        ...
        if critical_issues:
            ...
            for field, issues in field_issues.items():
                if len(issues) > 1:
                    recommendations.append(f"Multiple critical issues in {field} - comprehensive review needed")
        
        # Score-based recommendations
        if scores.get("completeness", 0) < 0.6:
            recommendations.append("Completeness score low - perform additional targeted extraction")
        
        if scores.get("accuracy", 0) < 0.7:
            recommendations.append("Accuracy issues detected - verify extracted values against paper")
        
        if scores.get("consistency", 0) < 0.8:
            recommendations.append("Consistency issues found - review calculations and cross-references")
        
        if scores.get("plausibility", 0) < 0.7:
            recommendations.append("Implausible values detected - verify units and extraction method")
        
        # General recommendations
        if len(warnings) > 5:
            recommendations.append("High number of warnings - consider additional quality review")
