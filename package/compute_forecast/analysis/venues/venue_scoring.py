...
from compute_forecast.analysis.venues.venue_database import VenueDatabase, VenueInfo
...
class VenueScorer:
    ...
    def calculate_venue_score(self, venue: str, domain: str, mila_data: Dict = None) -> VenueScore:
        """Comprehensive venue scoring for paper collection priority"""
        ...
        if venue_info:
            scores[ScoringFactor.COMPUTATIONAL_FOCUS.value] = venue_info.computational_focus
    ...
    def _assess_domain_match(self, venue: str, target_domain: str, venue_info: VenueInfo = None) -> float:
    ...
    def get_top_venues_by_domain(self, domain: str, limit: int = 10) -> List[VenueScore]:
    ...
    def generate_venue_recommendations(self, domain: str) -> Dict[str, Any]:
        ...
        if venue_scores:
            ...
            for factor in ScoringFactor:
                avg_factor_score = sum(v.component_scores.get(factor.value, 0) for v in top_venues) / len(top_venues)
    ...
    def export_scoring_results(self) -> Dict[str, Any]:
        ...
        for domain in all_domains:
            results['domain_recommendations'][domain] = self.generate_venue_recommendations(domain)
        ...
        if all_venue_scores:
            ...
            for score in all_venue_scores:
                ...
                if venue not in global_rankings or score.final_score > global_rankings[venue]['score']:
            ...
            sorted_global = sorted(global_rankings.items(), key=lambda x: x[1]['score'], reverse=True)
            
            results['cross_domain_analysis'] = {
                'top_venues_globally': [(v, data['score'], data['best_domain']) for v, data in sorted_global[:20]],
                'venue_distribution': {
                    'high_priority': sum(1 for _, data in global_rankings.items() if data['recommendation'] == 'high'),
                    'medium_priority': sum(1 for _, data in global_rankings.items() if data['recommendation'] == 'medium'),
                    'low_priority': sum(1 for _, data in global_rankings.items() if data['recommendation'] == 'low')
                }
            }
