...
from compute_forecast.analysis.venues.venue_analyzer import MilaVenueAnalyzer
from compute_forecast.analysis.venues.venue_database import VenueDatabase
from compute_forecast.analysis.venues.venue_scoring import VenueScorer
...
class VenueStrategist:
    ...
    def calculate_venue_priority(self, venue: str, venue_stats: Dict[str, Any], 
                               mila_data: Dict[str, Any]) -> Tuple[float, VenueMetadata]:
    ...
    def _estimate_venue_citations(self, venue: str, venue_stats: Dict[str, Any]) -> float:
        ...
        citation_estimates = {
            'NeurIPS': 45.0, 'ICML': 42.0, 'ICLR': 38.0,
            'CVPR': 35.0, 'ICCV': 32.0, 'ECCV': 30.0,
            'ACL': 25.0, 'EMNLP': 22.0, 'NAACL': 20.0,
            'AAAI': 28.0, 'IJCAI': 26.0, 'AAMAS': 18.0,
            'ICRA': 24.0, 'IROS': 20.0, 'RSS': 22.0
        }
        
        return citation_estimates.get(venue, 15.0)  # Default estimate
    
    def generate_primary_venues(self, mila_data: Dict[str, Any]) -> List[StrategicVenue]:
        ...
        for venue in all_venues:
            try:
                priority_score, metadata = self.calculate_venue_priority(venue, {}, mila_data)
        ...
        strategic_venues = []
        for i, (venue, score, metadata) in enumerate(venue_priorities[:self.target_venue_count]):
    ...
    def generate_domain_specific_venues(self, mila_data: Dict[str, Any]) -> Dict[str, List[Dict[str, Any]]]:
        ...
        for domain in research_domains:
            ...
            if db_domain:
                ...
                for venue_info in domain_db_venues:
                    ...
                    papers_per_year = min(self.papers_per_venue_base + 2, 12)  # 10-12 papers
        ...
        logger.info(f"Generated domain-specific venues for {len(domain_venues)} domains")
        return domain_venues
    
    def generate_collection_strategy(self, primary_venues: List[StrategicVenue]) -> CollectionStrategy:
        """Generate overall collection strategy"""
        logger.info("Generating collection strategy")
        
        # Calculate total target papers
        total_papers = sum(venue.collection_params.papers_per_year for venue in primary_venues)
        ...
        logger.info(f"Collection strategy: {total_papers} papers across {len(primary_venues)} venues")
        return strategy
    ...
    def generate_strategic_venue_collection(self) -> Dict[str, Any]:
        ...
        output = {
            'metadata': {
                ...
                'worker': 'worker3',
                'data_source': 'venue_analysis_fallback' if not Path("data/mila_venue_statistics.json").exists() else 'worker2_data',
