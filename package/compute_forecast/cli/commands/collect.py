...
def save_checkpoint(venue: str, year: int, papers: List[SimplePaper], completed: bool = False):
...
def main(
    venue: Optional[str] = typer.Option(None, "--venue", help="Conference venue (e.g., neurips, icml)"),
    year: Optional[int] = typer.Option(None, "--year", help="Publication year"),
    years: Optional[str] = typer.Option(None, "--years", help="Year range (e.g., 2020-2024) or list (2020,2022,2024)"),
    venues: Optional[str] = typer.Option(None, "--venues", help="Multiple venues (comma-separated)"),
    venue_file: Optional[Path] = typer.Option(None, "--venue-file", help="YAML file with venue specifications"),
    max_papers: int = typer.Option(0, "--max-papers", help="Maximum papers per venue/year (0 = unlimited)"),
    ...
    if venue_file:
        ...
        if venue:
            ...
            console.print("[red]Error:[/red] Must specify --venue, --venues, or --venue-file")
            raise typer.Exit(1)
        ...
        if year:
            ...
            console.print(f"[yellow]No year specified, using current year: {year_list[0]}[/yellow]")
    ...
    config = ScrapingConfig(
        ...
        batch_size=max_papers if max_papers > 0 else 10000,  # Use large batch size for unlimited
        max_retries=3,
        timeout=30
    )
        ...
        if scraper:
            for year in venue_year_list:
                ...
                if estimate:
                    ...
                    default_estimate = max_papers if max_papers > 0 else 1000  # Default for unlimited
    ...
    with Progress(
        ...
        BarColumn(),
        TaskProgressColumn(),
        TextColumn("• {task.fields[papers_collected]}/{task.fields[papers_total]} papers"),
        console=console,
        disable=no_progress
    ) as progress:
        ...
        for venue_name, venue_year_list in venue_years.items():
            ...
            if not scraper:
                console.print(f"[red]Error:[/red] No scraper available for venue {venue_name}")
                errors.append(f"No scraper for {venue_name}")
                # Advance by estimated papers for this venue
                venue_estimate = sum(venue_estimates.get((venue_name, y), 0) for y in venue_year_list)
            ...
            for year in venue_year_list:
                ...
                if resume:
                    ...
                    if checkpoint and checkpoint.get("completed"):
                        console.print(f"[yellow]Skipping {venue_name} {year} (already completed)[/yellow]")
                        progress.advance(main_task, expected_papers)
                        papers_collected = progress.tasks[main_task].fields['papers_collected'] + expected_papers
                    ...
                    if result.success:
                        ...
                        if max_papers == 0:
                            ...
                            if actual_diff != 0:
                                ...
                                new_total = max(progress.tasks[main_task].total + actual_diff, len(collected))
                            ...
                            if len(collected) < expected_papers:
                                progress.advance(main_task, expected_papers - len(collected))
                        
                        papers_collected = progress.tasks[main_task].fields['papers_collected'] + len(collected)
                        progress.update(main_task, papers_collected=papers_collected)
                    else:
    ...
    if all_papers:
        ...
        console.print(f"Years: {', '.join(str(y) for y in sorted(set(p.year for p in all_papers)))}")
