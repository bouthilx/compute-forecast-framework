...
class SystemInitializer:
    ...
    def get_initialization_order(self) -> List[InitializationStep]:
        ...
        while len(ordered_steps) < len(self.initialization_steps):
            ...
            for step in self.initialization_steps:
                ...
                dependencies_met = all(dep in completed_components for dep in step.dependencies)
            ...
            if not progress_made:
                ...
                remaining_steps = [
                    step for step in self.initialization_steps 
                    if step.component_name not in completed_components
                ]
                logger.error(f"Cannot resolve dependencies for remaining steps: {[s.step_name for s in remaining_steps]}")
                break
    ...
    def execute_initialization_step(self, step: InitializationStep, 
                                  initialization_function) -> Tuple[bool, float, Optional[str]]:
    ...
    def check_initialization_health(self, completed_steps: List[str], 
                                  failed_steps: List[str]) -> Dict[str, Any]:
        ...
        critical_completed = [
            ...
            if step.component_name in completed_steps
        ]
        critical_failed = [
            step for step in critical_steps 
            if step.component_name in failed_steps
        ]
        ...
        return {
            ...
            'system_ready': len(critical_failed) == 0 and len(critical_completed) == len(critical_steps)
        }