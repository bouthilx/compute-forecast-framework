...
class SimpleDeduplicator:
    ...
    def _calculate_similarity(self, paper1: Paper, paper2: Paper) -> float:
        ...
        title_similarity = SequenceMatcher(None, paper1.title.lower(), paper2.title.lower()).ratio()
        ...
        if authors1 and authors2:
            ...
            author_similarity = author_intersection / author_union if author_union > 0 else 0
        else:
            author_similarity = 0
        ...
        # Venue similarity
        venue_similarity = SequenceMatcher(None, paper1.venue.lower(), paper2.venue.lower()).ratio()
...
class SimpleCitationAnalyzer:
    ...
    def analyze_citation_distributions(self, papers: List[Paper]) -> CitationAnalysisReport:
        ...
        high_impact_papers = [
            ...
            if paper.citations >= self.high_impact_threshold
        ]
        
        # Breakthrough candidates
        breakthrough_candidates = [
            paper for paper in papers 
            if paper.citations >= self.breakthrough_threshold
        ]
        ...
        logger.info(f"Citation analysis complete: avg={average_citations:.1f}, median={median_citations}, high_impact={len(high_impact_papers)}")
        return report
    
    def filter_papers_by_citations(self, papers: List[Paper], min_citations: int) -> List[Paper]:
        """Filter papers by minimum citation count"""
        filtered = [paper for paper in papers if paper.citations >= min_citations]
        logger.info(f"Citation filter: {len(papers)} -> {len(filtered)} papers (min_citations={min_citations})")
        return filtered
    
    def identify_breakthrough_papers(self, papers: List[Paper], year_thresholds: Dict[int, int]) -> List[Paper]:
