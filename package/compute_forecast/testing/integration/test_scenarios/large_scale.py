...
from compute_forecast.testing.integration.performance_monitor import PerformanceMonitor, BottleneckAnalyzer
...
class LargeScaleTestScenario:
    ...
    def __init__(self, baseline_time_1k: float = 300.0):
        ...
        self.performance_monitor = PerformanceMonitor(monitoring_interval=1.0)  # Sample every second
    ...
    def _generate_test_data_batched(self) -> List[Paper]:
        ...
        from compute_forecast.testing.mock_data.configs import MockDataConfig, DataQuality
    ...
    def _start_memory_tracking(self) -> None:
        ...
        self._memory_tracking_thread = threading.Thread(target=self._memory_tracking_loop)
    ...
    def _analyze_large_scale_results(self, pipeline_result: Dict[str, Any], 
                                   execution_time: float, 
                                   papers_processed: int) -> LargeScaleResult:
        ...
        scaling_factor = execution_time / self.expected_time_10k if self.expected_time_10k > 0 else 0
        ...
        success = (
            ...
            peak_memory < self.config.max_memory_usage_mb and
            throughput > 5.0 and  # Minimum 5 papers/second
            scaling_factor < 2.0  # No worse than 2x linear scaling
        )
    ...
    def _identify_bottlenecks(self, performance_metrics: Dict[str, Any]) -> List[str]:
        ...
        for phase_name, phase_data in performance_metrics.items():
            ...
            if phase_data.get("avg_cpu", 0) > 85:
                bottlenecks.append(f"{phase_name}: Very high CPU usage ({phase_data['avg_cpu']:.1f}%)")
                
            # Memory bottlenecks
            if phase_data.get("peak_memory", 0) > 6000:  # 75% of 8GB
                bottlenecks.append(f"{phase_name}: High memory usage ({phase_data['peak_memory']:.0f}MB)")
                
            # Duration bottlenecks (proportional to scale)
            if phase_data.get("duration", 0) > 600:  # 10 minutes per phase
                bottlenecks.append(f"{phase_name}: Very slow execution ({phase_data['duration']:.1f}s)")
                
            # High variance indicates instability
            if phase_data.get("cpu_variance", 0) > 30:
                bottlenecks.append(f"{phase_name}: Unstable CPU performance (variance: {phase_data['cpu_variance']:.1f})")
                
        return bottlenecks
        
    def _identify_scaling_issues(self, execution_time: float, peak_memory: float,
                               throughput: float, scaling_factor: float) -> List[str]:
        ...
        if peak_memory > expected_memory * 0.5:  # More than 50% of linear scaling
            issues.append(f"Memory scaling issue: {peak_memory:.0f}MB (expected ~{expected_memory*0.5:.0f}MB)")
        ...
        if len(self.memory_samples) > 10:
            ...
            if growth_rate > 0.5:  # 50% growth suggests leak
                issues.append(f"Potential memory leak: {growth_rate*100:.1f}% growth during execution")
                
        return issues
        
    def _generate_scaling_recommendations(self, bottlenecks: List[str], 
                                        scaling_issues: List[str],
    ...
    def _print_results(self, result: LargeScaleResult) -> None:
        ...
        print(f"Peak Memory: {result.peak_memory_mb:.0f}MB (limit: {self.config.max_memory_usage_mb}MB)")
