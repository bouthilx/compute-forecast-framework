...
class ErrorInjectionFramework:
    ...
    def register_injection_point(self, component: str, 
                                injector: Callable[[ErrorType], None]) -> None:
        """
    ...
    def add_scenario(self, scenario: ErrorScenario) -> None:
        ...
        """
        logger.info(f"Adding error scenario: {scenario.error_type.value} for {scenario.component}")
    ...
    def inject_error(self, scenario: ErrorScenario) -> None:
        ...
        if scenario.component not in self.injection_points:
            raise ValueError(f"No injection point registered for component: {scenario.component}")
        
        # Check probability
        if random.random() >= scenario.probability:
            logger.debug(f"Skipping injection due to probability ({scenario.probability})")
            return
        
        # Perform injection
        logger.warning(f"Injecting {scenario.error_type.value} into {scenario.component}")
    ...
    def run_scenario_suite(self) -> Dict[str, Any]:
        ...
        """
        logger.info(f"Running error scenario suite with {len(self.scenarios)} scenarios")
        ...
        if results["scenarios_executed"] > 0:
            results["injection_success_rate"] = actual_injections / results["scenarios_executed"]
