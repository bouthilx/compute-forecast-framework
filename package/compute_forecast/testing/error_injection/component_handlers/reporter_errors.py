...
class ReporterErrorHandler:
    ...
    def add_alternative_output(self, path: str, name: str, 
                             output_type: str = "file", priority: int = 5) -> None:
        """
    ...
    def verify_alternative_output(self) -> Dict[str, Any]:
        ...
        available = [
            (name, config) for name, config in self._alternative_outputs.items()
            if config.enabled and (
                config.output_type != "file" or self._output_error_type != "disk_full"
            )
        ]
    ...
    def attempt_recovery(self) -> Dict[str, Any]:
        ...
        if self._output_error_type is None:
            return {
                "recovered": True,
                "reason": "No error to recover from"
            }
        ...
        if alt_result["alternative_available"]:
            ...
            if (self._output_error_type == "disk_full" and 
                alt_result["output_type"] != "file"):
