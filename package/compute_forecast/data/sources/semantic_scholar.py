...
class SemanticScholarSource(BaseCitationSource):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if total_papers == 0 and query.venue:
                    self.logger.info("Venue search returned 0 results, trying fallback strategy")
                    fallback_params = self._build_fallback_search_params(query)
                    self.logger.info(f"Fallback search params: {fallback_params}")
                    
                    response = self._make_rate_limited_request(search_url, fallback_params)
                    if response.status_code == 200:
                        data = response.json()
                        total_papers = data.get('total', 0)
                        self.logger.info(f"Fallback search found {total_papers} total papers")
                
                for paper_data in data.get('data', []):
                    try:
                        paper = self._parse_semantic_result(paper_data, query)
                        # Use dynamic citation threshold for recent papers
                        effective_threshold = self._get_dynamic_citation_threshold(query.year, query.min_citations)
                ...
                errors.append(f"API request failed: {response.status_code} - {response.text[:200]}")
    ...
    def _build_fallback_search_params(self, query: CollectionQuery) -> dict:
        ...
        if query.venue:
            ...
            if venue_name in ['neurips', 'nips']:
                query_parts.append('("NeurIPS" OR "Neural Information Processing Systems")')
            elif venue_name == 'icml':
                query_parts.append('("ICML" OR "International Conference on Machine Learning")')
            elif venue_name == 'iclr':
                query_parts.append('("ICLR" OR "International Conference on Learning Representations")')
            else:
    ...
    def _make_rate_limited_request(self, url: str, params: dict) -> requests.Response:
        ...
        for attempt in range(self.max_retries):
            ...
            try:
                self.logger.debug(f"Making request (attempt {attempt + 1}/{self.max_retries})")
                ...
                if response.status_code == 200:
                    ...
                    self.logger.warning(f"Rate limited (429). Retrying in {retry_delay:.1f}s (attempt {attempt + 1}/{self.max_retries})")
        ...
        raise requests.exceptions.RequestException(f"All {self.max_retries} retry attempts failed")
    ...
    def _get_dynamic_citation_threshold(self, paper_year: int, base_threshold: int) -> int:
    ...
    def get_paper_details(self, paper_id: str) -> Paper:
        ...
        try:
            ...
            if response.status_code != 200:
                raise ValueError(f"Paper {paper_id} not found or API error: {response.status_code}")
