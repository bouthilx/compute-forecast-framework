...
class CVFScraper(ConferenceProceedingsScraper):
    ...
    def _extract_paper_from_entry(self, entry, venue: str, year: int) -> Optional[SimplePaper]:
        ...
        paper_id_match = re.search(r'/([^/]+)\.html$', paper_detail_url) if paper_detail_url else None
    ...
    def _extract_authors_from_element(self, element) -> List[str]:
        ...
        for form in forms:
            hidden_input = form.find('input', {'type': 'hidden', 'name': 'query_author'})
            if hidden_input:
