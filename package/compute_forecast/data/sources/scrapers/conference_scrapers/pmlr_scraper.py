...
class PMLRScraper(ConferenceProceedingsScraper):
    ...
    def _extract_paper_from_entry(self, entry, venue: str, year: int, index: int) -> Optional[SimplePaper]:
    ...
    def _extract_from_div_paper(self, entry, venue: str, year: int, index: int) -> Optional[SimplePaper]:
        ...
        # Get authors
        authors_elem = entry.find('p', class_='authors') or entry.find('span', class_='authors')
    ...
    def _extract_from_p_title(self, entry, venue: str, year: int, index: int) -> Optional[SimplePaper]:
        ...
        if next_elem and next_elem.name == 'p' and 'authors' in next_elem.get('class', []):
    ...
    def _extract_authors(self, element) -> List[str]:
        ...
        for part in author_parts:
            ...
            author = re.sub(r'\s*\([^)]*\)', '', author)  # Remove affiliations in parentheses
            author = author.strip()
