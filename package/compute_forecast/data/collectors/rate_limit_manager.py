...
class RateLimitManager:
    ...
    def record_request(self, api_name: str, success: bool, response_time_ms: int, request_size: int = 1) -> None:
        ...
        with self._locks[api_name]:
            ...
            logger.debug(f"Recorded {request_size} requests for {api_name}: success={success}, response_time={response_time_ms}ms")
    ...
    def get_current_usage(self, api_name: str) -> Optional[RateLimitStatus]:
        ...
        with self._locks[api_name]:
            ...
            current_count = window.get_current_count()
            next_slot_time = datetime.now() + timedelta(seconds=window.time_until_next_slot())
