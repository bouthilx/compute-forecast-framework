...
from compute_forecast.data.processors.citation_statistics import AdaptiveThreshold
...
class AdaptiveThresholdCalculator:
    ...
    def calculate_venue_threshold(self, venue: str, year: int, papers: List[Paper], venue_tier: str) -> int:
        ...
        if citations:
            statistical_threshold = np.percentile(citations, self.config.statistical_percentile)
        else:
        ...
        if citations:
            min_representation_threshold = np.percentile(
                citations, 
                self.config.min_representation_percentile
            )
            ...
            if adaptive_threshold >= self.config.min_citation_threshold and max(citations) < self.config.min_citation_threshold:
    ...
    def calculate_adaptive_threshold(self, venue: str, year: int, papers: List[Paper], 
        ...
        confidence = min(
            len(venue_papers) / self.config.min_papers_for_full_confidence, 
            1.0
        ) if venue_papers else 0.1
        ...
        return AdaptiveThreshold(
            ...
            venue_prestige_multiplier=self.config.venue_tier_multipliers.get(venue_tier, 0.5),
            representation_requirement=max(
                int(len(venue_papers) * self.config.min_representation_percent), 
                5
            ) if venue_papers else 5,
