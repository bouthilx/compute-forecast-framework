...
from compute_forecast.data.models import Paper, Author, APIResponse, ResponseMetadata, APIError
...
class UnpaywallClient:
    ...
    def __init__(self, email: str):
        ...
        self.headers = {
            'User-Agent': f'research-paper-collector/1.0 (mailto:{email})'
        }
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if response.status_code == 200:
                    return self._parse_unpaywall_response(response, normalized_doi, response_time_ms)
                    ...
                    if attempt < self.max_retries - 1:
                        ...
                        logger.warning(f"Unpaywall server error {response.status_code}, retrying in {wait_time}s")
                ...
                if attempt < self.max_retries - 1:
                    wait_time = self.retry_delay * (2 ** attempt)
                    logger.warning(f"Unpaywall request timeout, retrying in {wait_time}s")
                    time.sleep(wait_time)
                    continue
                else:
                ...
                if attempt < self.max_retries - 1:
                    wait_time = self.retry_delay * (2 ** attempt)
                    logger.warning(f"Unpaywall request error: {e}, retrying in {wait_time}s")
                    time.sleep(wait_time)
                    continue
                else:
    ...
    def _parse_unpaywall_response(self, response: requests.Response, doi: str, response_time_ms: float) -> APIResponse:
    ...
    def _handle_server_error(self, response: requests.Response, doi: str, response_time_ms: float) -> APIResponse:
    ...
    def _handle_client_error(self, response: requests.Response, doi: str, response_time_ms: float) -> APIResponse:
    ...
    def _handle_network_error(self, exception: Exception, doi: str, response_time_ms: float) -> APIResponse:
    ...
    def _handle_max_retries_exceeded(self, doi: str, response_time_ms: float) -> APIResponse:
