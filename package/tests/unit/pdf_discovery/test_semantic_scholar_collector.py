...
class TestSemanticScholarPDFCollector:
    ...
    def test_discover_single_paper_with_pdf(self, mock_ss_class, mock_sleep):
        ...
        assert pdf_record.confidence_score == 0.8  # Search by title has lower confidence
    ...
    def test_batch_discovery(self, mock_ss_class, mock_sleep):
        ...
        collector = SemanticScholarPDFCollector()
        papers = [
            Paper(paper_id=f"paper_{i}", title=f"Paper {i}", 
                  authors=[Author(name="Test Author")], year=2024,
                  citations=i, venue="Test") 
            for i in range(1, 4)
        ]
    ...
    def test_handle_large_batch(self, mock_ss_class, mock_sleep):
        ...
        papers = []
        for i in range(600):
            paper = Paper(paper_id=f"paper_{i}", title=f"Paper {i}",
                          authors=[], year=2024, citations=0, venue="Test")
    ...
    def test_statistics_tracking(self):
        ...
        with patch('time.sleep', return_value=None):
            with patch('src.pdf_discovery.sources.semantic_scholar_collector.semanticscholar.SemanticScholar') as mock_ss:
                mock_api = Mock()
                ...
                def search_side_effect(query, limit=5):
                    if "Paper 0" in query:
                        return {'data': [{
                            'paperId': 'ss_1',
                            'openAccessPdf': {'url': 'test1.pdf', 'status': 'GREEN'}
                        }]}
    ...
    def test_batch_error_handling(self, mock_ss_class, mock_sleep):
        ...
        papers = []
        for i in range(2):
            p = Paper(paper_id=f"paper_{i}", title=f"Paper {i}",
                      authors=[], year=2024, citations=0, venue="Test")
