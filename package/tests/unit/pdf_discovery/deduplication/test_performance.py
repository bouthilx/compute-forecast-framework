...
class TestDeduplicationPerformance:
        ...
        for i in range(500):
            paper = Paper(
                ...
                authors=[
                    ...
                    Author(name=f"Co-Author {(i+1)%100}", affiliation=f"Institute {i%30}"),
                ],
                ...
                arxiv_id=f"2{(i%2)+3}01.{i:05d}" if i % 3 == 0 else None,  # 33% have arXiv IDs
            )
            papers.append(paper)
    ...
    def test_deduplication_performance_500_papers(self, large_dataset):
        ...
        assert execution_time < 60.0, f"Deduplication took {execution_time:.2f}s, should be < 60s"
    ...
    def test_deduplication_performance_10k_papers(self):
        ...
        assert execution_time < 120.0, f"Deduplication took {execution_time:.2f}s, should be < 120s"
    ...
    def test_fuzzy_matching_performance(self):
        ...
        assert execution_time < 10.0, f"Fuzzy matching took {execution_time:.2f}s, should be < 10s"
