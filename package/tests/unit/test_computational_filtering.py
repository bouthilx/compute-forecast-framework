...
class TestComputationalAnalyzer:
    ...
    def test_high_computational_content(self):
        ...
        assert analysis.confidence_score > 0.4  # Realistic confidence for short abstract
    ...
    def test_theoretical_paper(self):
        ...
        assert 'algorithms' in analysis.keyword_matches
        assert analysis.experimental_indicators['theoretical_focus'] is True or analysis.experimental_indicators['weak_experimental'] > 0
...
class TestComputationalResearchFilter:
    ...
    def test_high_quality_ml_paper(self):
        ...
        assert result.computational_analysis.computational_richness > 0.4  # Realistic score
        assert result.venue_analysis.venue_score > 0.8
        assert any("meets computational research criteria" in reason for reason in result.reasons)
    ...
    def test_non_computational_paper(self):
        ...
        paper = Paper(
            ...
            authors=[
                Author(name="X", affiliation="University of Design")
            ],
    ...
    def test_batch_filtering(self):
        ...
        papers = [
            Paper(
                ...
                year=2024,
                citations=i * 10,
                abstract="Deep learning algorithm with neural networks and GPU training. We implement and evaluate performance." if i % 2 == 0 else "User study"
            )
            for i in range(5)
        ]
        ...
        assert len(passed) >= 1  # At least some papers should pass with lower thresholds
    
    def test_strict_mode(self):
        """Test strict filtering mode."""
        strict_config = FilteringConfig(
            min_computational_richness=0.5,
            min_venue_score=0.6,
            strict_mode=True
        )
