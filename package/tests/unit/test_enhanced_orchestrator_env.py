...
class TestEnhancedOrchestratorEnvironment:
    ...
    def test_env_vars_override_by_init_params(self):
        ...
        with patch.dict(os.environ, env_vars):
            ...
            with patch('src.data.sources.enhanced_semantic_scholar.EnhancedSemanticScholarClient') as mock_ss, \
                 patch('src.data.sources.enhanced_openalex.EnhancedOpenAlexClient') as mock_oa, \
                 patch('src.data.sources.enhanced_crossref.EnhancedCrossrefClient') as mock_cr, \
    ...
    def test_logging_env_var_count(self, caplog):
        ...
        with patch.dict(os.environ, env_vars):
            with caplog.at_level("INFO"):
                ...
                assert "Loaded 3 API configurations from environment variables" in caplog.text