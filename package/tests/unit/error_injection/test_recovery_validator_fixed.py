...
class TestRecoveryValidatorFixed:
    ...
    def test_initialization(self):
        ...
        mock_engine = Mock()
        mock_manager = Mock()
        validator2 = RecoveryValidator(recovery_engine=mock_engine, state_manager=mock_manager)
    ...
    def test_measure_data_integrity(self):
        ...
        # Test list integrity
        list_integrity = validator.measure_data_integrity(
            [1, 2, 3, 4, 5],
            [1, 2, 3]
        )
    ...
    def test_verify_graceful_degradation_healthy(self):
        ...
        mock_engine = Mock()
        mock_manager = Mock()
        validator = RecoveryValidator(recovery_engine=mock_engine, state_manager=mock_manager)
        
        # Mock healthy recovery status
        mock_engine.get_recovery_status = Mock(return_value={
            "is_recovering": False,
            "recovery_attempts": 1,
            "max_attempts": 3
        })
        
        mock_manager.get_component_status = Mock(return_value={
            "status": "running",
            "error_count": 0
        })
    ...
    def test_recommendations_generation(self):
        ...
        assert any("recovery" in r.lower() or "time" in r.lower() for r in report["recommendations"])