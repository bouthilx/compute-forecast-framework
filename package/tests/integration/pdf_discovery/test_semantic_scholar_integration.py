...
class TestSemanticScholarIntegration:
    ...
    @patch('src.pdf_discovery.sources.semantic_scholar_collector.semanticscholar.SemanticScholar')
    def test_framework_integration(self, mock_ss_class, mock_sleep):
        ...
        # Test papers
        papers = [
            Paper(paper_id="paper_1", title="Machine Learning Paper",
                  authors=[Author(name="John Doe")], year=2024,
                  citations=10, venue="ICML"),
            Paper(paper_id="paper_2", title="Deep Learning Paper",
                  authors=[Author(name="Jane Smith")], year=2024,
                  citations=20, venue="NeurIPS"),
            Paper(paper_id="paper_3", title="Unknown Paper",
                  authors=[Author(name="Bob Wilson")], year=2024,
                  citations=5, venue="Workshop")
        ]
    ...
    def test_venue_priority_integration(self, mock_ss_class, mock_sleep):
        ...
        # Test papers
        papers = [
            Paper(paper_id="neurips_paper", title="NeurIPS Paper",
                  authors=[], year=2024, citations=0, venue="NeurIPS"),
            Paper(paper_id="icml_paper", title="ICML Paper",
                  authors=[], year=2024, citations=0, venue="ICML"),
        ]
        ...
        neurips_record = next((r for r in result.records if r.paper_id == "neurips_paper"), None)
        if neurips_record:
    ...
    def test_deduplication_with_other_sources(self, mock_ss_class, mock_sleep):
        ...
        papers = [
            Paper(paper_id="ml_paper", title="Machine Learning Paper",
                  authors=[Author(name="John Doe")], year=2024,
                  citations=10, venue="ICML")
        ]
        ...
        assert "proceedings.mlr.press" in pdf_record.pdf_url or "arxiv" in pdf_record.pdf_url
    ...
    def test_mixed_identifier_batch(self, mock_ss_class, mock_sleep):
        ...
        # Paper with DOI
        p1 = Paper(paper_id="doi_paper", title="DOI Paper",
                   authors=[], year=2024, citations=0, venue="Test")
        p1.doi = "10.1234/test"
        papers.append(p1)
        
        # Paper with arXiv ID
        p2 = Paper(paper_id="arxiv_paper", title="ArXiv Paper",
                   authors=[], year=2024, citations=0, venue="Test")
        p2.arxiv_id = "2301.00001"
        papers.append(p2)
        
        # Paper with SS ID
        p3 = Paper(paper_id="ss_paper", title="SS Paper",
                   authors=[], year=2024, citations=0, venue="Test")
        p3.semantic_scholar_id = "ss_direct_123"
        papers.append(p3)
        
        # Paper with only title
        p4 = Paper(paper_id="title_paper", title="Title Only Paper",
                   authors=[], year=2024, citations=0, venue="Test")
        papers.append(p4)
