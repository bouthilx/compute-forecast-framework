...
class TestIssue5StateManager:
    ...
    def test_create_session_exact_interface(self):
        ...
        assert duration < 1.0, f"Session creation took {duration:.3f}s (>1s requirement)"
    ...
    def test_get_recovery_plan_exact_interface(self):
        ...
        assert recovery_plan.resumption_strategy in [
            "from_last_checkpoint", "from_venue_start", "partial_restart", "full_restart"
        ]
    ...
    def test_resume_session_exact_interface(self):
        ...
        assert duration < 300.0, f"Session recovery took {duration:.1f}s (>300s requirement)"
        assert abs(result.recovery_duration_seconds - duration) < 0.001  # Allow small timing differences
...
class TestIssue5PerformanceRequirements:
    ...
    def test_checkpoint_creation_2_second_requirement(self):
        ...
        for i in range(5):
            ...
            assert duration < 2.0, f"Checkpoint {i} save took {duration:.3f}s (>2s requirement)"
            assert checkpoint_id is not None
    ...
    def test_state_loading_5_second_requirement(self):
        ...
        for i in range(5):
            ...
            assert duration < 5.0, f"Checkpoint load {i} took {duration:.3f}s (>5s requirement)"
