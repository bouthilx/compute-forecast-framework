...
class TestErrorInjectionIntegration:
    ...
    def test_api_failure_recovery_scenario(self):
        ...
        for api_name, api_mock in apis.items():
            ...
            framework.register_injection_point(
                api_name,
                lambda error_type, api=api_name: collector_handler.simulate_api_timeout(api)
            )
        
        # Setup fallback order
        collector_handler.set_fallback_order(["semantic_scholar", "openalex", "crossref"])
        ...
        post_state = {
            ...
            "timestamp": datetime.now(),
            "available_apis": len([
                api for api in ["semantic_scholar", "openalex", "crossref"]
        ...
        validation = APIFailureScenarios.validate_api_recovery({
            ...
            "fallback_success_rate": 1.0 if collector_handler.get_active_api() else 0.0,
            "cascade_failures": 0
        })
    ...
    def test_data_corruption_recovery_scenario(self):
        ...
        framework.register_injection_point(
            "paper_parser",
            lambda error_type: analyzer_handler.simulate_corrupted_input("missing_fields")
        )
        ...
        validation = DataCorruptionScenarios.validate_data_integrity({
            "data_preservation_rate": total_processed / (total_processed + total_failed),
    ...
    def test_resource_exhaustion_graceful_degradation(self):
        ...
        framework.register_injection_point(
            "analyzer",
            lambda error_type: analyzer_handler.simulate_memory_pressure()
        )
        
        framework.register_injection_point(
            "report_writer",
            lambda error_type: reporter_handler.simulate_disk_full()
        )
    ...
    def test_comprehensive_error_scenario(self):
        ...
        all_scenarios.extend(DataCorruptionScenarios.get_progressive_corruption_scenario())
        all_scenarios.extend(ResourceExhaustionScenarios.get_progressive_resource_exhaustion_scenario())
        ...
        # Verify different severity levels were tested
