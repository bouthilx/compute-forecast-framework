...
class TestAllSourcesIntegration:
                    ...
                    "items": [
                        {
                            ...
                            "title": ["Computer Vision with Deep Learning: Recent Advances"],
    ...
    async def test_all_sources_collect_successfully(self, comprehensive_mock_data):
        ...
        with patch('src.data.sources.semantic_scholar.SemanticScholarSource.search') as mock_ss, \
            ...
            results = await orchestrator.collect_parallel("machine learning", max_results=20)
            
            # Verify results
            assert results["total_results"] == 8  # 2 from each source
            assert len(results["papers"]) == 8
            assert len(results["errors"]) == 0
            assert results["sources_queried"] == ["semantic_scholar", "openalex", "crossref", "google_scholar"]
        ...
        with patch('src.data.sources.semantic_scholar.SemanticScholarSource.search') as mock_ss, \
            ...
            for paper in results["papers"]:
                for field in required_fields:
                    assert field in paper, f"Paper missing required field '{field}': {paper}"
                
                # Verify year is integer
                assert isinstance(paper["year"], int), f"Year should be int: {paper['year']}"
                
                # Verify source is valid
                assert paper["source"] in ["semantic_scholar", "openalex", "crossref", "google_scholar"]
    ...
    async def test_concurrent_rate_limiting(self, comprehensive_mock_data):
        ...
        with patch('src.data.sources.semantic_scholar.SemanticScholarSource.search', side_effect=track_ss_calls), \
             patch('src.data.sources.openalex.OpenAlexSource.search', side_effect=track_oa_calls), \
             patch('src.data.sources.crossref.CrossRefSource.search', side_effect=track_cr_calls), \
             patch('src.data.sources.google_scholar.GoogleScholarSource.search', side_effect=track_gs_calls):
    ...
    async def test_partial_source_failure_handling(self, comprehensive_mock_data):
        ...
        scenarios = [
            ...
            {"failing_sources": ["semantic_scholar", "openalex", "crossref"], "expected_papers": 2}
        ]
        ...
        for scenario in scenarios:
            with patch('src.data.sources.semantic_scholar.SemanticScholarSource.search') as mock_ss, \
                ...
                # Collect and verify
                results = await orchestrator.collect_parallel("test query", max_results=20)
