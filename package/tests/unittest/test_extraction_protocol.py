...
from src.analysis.computational.extraction_protocol import (
    ExtractionProtocol,
    ExtractionResult,
    ExtractionMetadata,
    HardwareSpecs,
...
class TestExtractionProtocol:
    ...
    def test_initialization(self, extraction_protocol):
        ...
        assert extraction_protocol.extraction_result.metadata.paper_id == "test_paper_001"
    ...
    def test_phase1_preparation(self, extraction_protocol):
        ...
        assert extraction_protocol.extraction_result.metadata.phase_completed == ExtractionPhase.PREPARATION
    ...
    def test_phase2_automated_extraction(self, extraction_protocol, mock_analyzer):
        ...
        # Check metadata updates
        assert extraction_protocol.extraction_result.metadata.phase_completed == ExtractionPhase.AUTOMATED
    ...
    def test_phase3_manual_extraction(self, extraction_protocol):
        ...
        # Check metadata updates
        assert extraction_protocol.extraction_result.metadata.phase_completed == ExtractionPhase.MANUAL
    
    def test_phase4_validation(self, extraction_protocol):
        """Test Phase 4: Validation."""
        # Set up some data for validation
        extraction_protocol.extraction_result.hardware = HardwareSpecs(gpu_type="V100", gpu_count=8)
        extraction_protocol.extraction_result.training = TrainingSpecs(total_time_hours=120.0)
        extraction_protocol.extraction_result.model = ModelSpecs(parameters_count=340, architecture="Transformer")
        
        validation = extraction_protocol.phase4_validation()
        
        # Check validation results
        assert isinstance(validation, ValidationResults)
        assert validation.confidence_hardware in [ConfidenceLevel.LOW, ConfidenceLevel.MEDIUM, ConfidenceLevel.HIGH]
        assert validation.confidence_training in [ConfidenceLevel.LOW, ConfidenceLevel.MEDIUM, ConfidenceLevel.HIGH]
        assert validation.confidence_model in [ConfidenceLevel.LOW, ConfidenceLevel.MEDIUM, ConfidenceLevel.HIGH]
        assert validation.confidence_overall in [ConfidenceLevel.LOW, ConfidenceLevel.MEDIUM, ConfidenceLevel.HIGH]
        
        # Check metadata updates
        assert extraction_protocol.extraction_result.metadata.phase_completed == ExtractionPhase.VALIDATION
    ...
    def test_phase5_documentation(self, extraction_protocol):
        ...
        # Check metadata updates
        assert extraction_protocol.extraction_result.metadata.phase_completed == ExtractionPhase.DOCUMENTATION
    ...
    def test_find_section_content(self, extraction_protocol):
        ...
        experimental_content = extraction_protocol._find_section_content("experimental_setup")
    ...
    def test_run_consistency_checks(self, extraction_protocol):
        ...
        extraction_protocol.extraction_result.training = TrainingSpecs(total_time_hours=120.0)
        extraction_protocol.extraction_result.computation = ComputationSpecs(total_gpu_hours=500.0)  # Should be 960
    ...
    def test_detect_outliers(self, extraction_protocol):
        ...
        extraction_protocol.extraction_result.training = TrainingSpecs(total_time_hours=10000.0)  # > 1 year
        extraction_protocol.extraction_result.computation = ComputationSpecs(total_gpu_hours=2000000.0)  # > 1M
