...
class TestPMLRScraper:
    ...
    def test_extract_authors_various_formats(self, scraper):
        ...
        elem = BeautifulSoup("<p>John Doe (MIT), Jane Smith (Stanford)</p>", 'html.parser').p
        authors = scraper._extract_authors(elem)
        assert authors == ["John Doe", "Jane Smith"]
        
    @patch('compute_forecast.data.sources.scrapers.conference_scrapers.pmlr_scraper.PMLRScraper._make_request')
