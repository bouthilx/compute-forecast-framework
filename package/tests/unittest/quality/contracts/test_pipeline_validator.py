...
class TestAnalysisContractValidator:
    ...
    def test_validate_with_paper_cross_validation(self, validator, valid_analysis, valid_paper):
        ...
        inconsistent = [v for v in result.violations if v.violation_type == ContractViolationType.INCONSISTENT_DATA]
    ...
    def test_validate_resource_metrics(self, validator):
        ...
        resource_violations = [v for v in result.violations if "resource_metrics" in v.field_name]
    ...
    def test_performance_violation(self, validator, valid_analysis):
        ...
        with patch('time.time') as mock_time:
            ...
            perf_warnings = [v for v in result.warnings if v.violation_type == ContractViolationType.PERFORMANCE_VIOLATION]
    ...
    def test_validate_pipeline_transition_unknown(self, validator):
        ...
        assert result.violations[0].violation_type == ContractViolationType.INVALID_REFERENCE
...
class TestPipelineIntegrationValidator:
    ...
    def test_validate_full_pipeline_partial_data(self, validator):
        ...
        assert len(reports["analysis_outputs"].contract_results) == 0 if "analysis_outputs" in reports else True