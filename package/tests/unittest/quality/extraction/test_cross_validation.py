...
class TestCrossValidationFramework:
    ...
    def setup_method(self):
        ...
        for year in [2019, 2020, 2021, 2022, 2023]:
            for i in range(30):  # 30 papers per year
                paper = Paper(paper_id=f"paper_{year}_{i}",
    ...
    def test_select_validation_sample_random(self):
        ...
        sample = self.framework.select_validation_sample(self.papers, stratify_by="none")
    ...
    def test_select_validation_sample_by_year(self):
        ...
        sample = self.framework.select_validation_sample(self.papers, stratify_by="year")
    ...
    def test_compare_extractions_within_tolerance(self):
        ...
        automated = {
            "paper_1": {"gpu_hours": 1100, "parameters": 1.05e9}  # 10% and 5% difference
        }
    ...
    def test_compare_extractions_outside_tolerance(self):
        ...
        manual = {
            "paper_1": {"gpu_hours": 1000, "batch_size": 256}
        }
        automated = {
    ...
    def test_calculate_field_agreement_numeric(self):
        ...
        # Within tolerance
        agreement, accuracy = self.framework._calculate_field_agreement(
            "gpu_hours", 1000, 1100  # 10% difference
        )
        assert agreement is True
        assert accuracy > 0.8
        ...
        # Outside tolerance
        agreement, accuracy = self.framework._calculate_field_agreement(
            "gpu_hours", 1000, 2000  # 100% difference
        )
    ...
    def test_calculate_field_agreement_string(self):
        ...
        # Exact match
        agreement, accuracy = self.framework._calculate_field_agreement(
            "framework", "pytorch", "PyTorch"  # Case insensitive
        )
        assert agreement is True
        assert accuracy == 1.0
    ...
    def test_validate_extraction_quality(self):
        ...
        report = self.framework.validate_extraction_quality(manual_sample, automated_full)
    ...
    def test_stratified_sample_by_domain(self):
        ...
        sample = self.framework.select_validation_sample(self.papers, stratify_by="domain")
        
        assert len(sample) == self.framework.manual_sample_size
        
        # Check domain distribution
        nlp_count = sum(1 for p in sample if "nlp" in p.title.lower())
        cv_count = sum(1 for p in sample if "cnn" in p.title.lower() or "image" in p.title.lower())
