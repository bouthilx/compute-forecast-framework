...
class TestExtractionConsistencyChecker:
    ...
    def setup_method(self):
        ...
        for i, year in enumerate([2019, 2020, 2021, 2022, 2023]):
            paper = Paper(paper_id=f"paper_{year}",
    ...
    def test_temporal_consistency_with_outlier(self):
        ...
        # Add outlier paper
        outlier_paper = Paper(paper_id="outlier", title="Outlier", year=2022,
            authors=[],
            venue="",
            citations=0)
        ...
        result = self.checker.check_temporal_consistency(papers_with_outlier, "gpu_hours")
    ...
    def test_cross_paper_consistency_similar_values(self):
        ...
        similar_papers = []
        for i in range(5):
            paper = Paper(paper_id=f"similar_{i}", title=f"Similar Model {i}",
            year=2023,
            authors=[],
            venue="",
            citations=0)
    ...
    def test_cross_paper_consistency_high_variation(self):
        ...
        for i, val in enumerate(values):
            paper = Paper(paper_id=f"varied_{i}", title=f"Varied Model {i}",
            year=2023,
            authors=[],
            venue="",
            citations=0)
    ...
    def test_domain_consistency_nlp(self):
        ...
        nlp_paper = Paper(paper_id="nlp_1",
    ...
    def test_domain_consistency_cv(self):
        ...
        param_violation = next(v for v in result.details["violations"] if v["field"] == "parameters")
    ...
    def test_determine_domain(self):
        ...
        # NLP papers
        nlp_papers = [
            Paper(paper_id="1", title="Transformer Language Model",
            year=2023,
            authors=[],
            venue="",
            citations=0),
            Paper(paper_id="2", title="BERT: Pre-training of Deep Bidirectional Transformers",
            year=2023,
            authors=[],
            venue="",
            citations=0),
            Paper(paper_id="3", title="GPT-3: Language Models are Few-Shot Learners",
            year=2023,
            authors=[],
            venue="",
            citations=0)
        ...
        # CV papers
        cv_papers = [
            Paper(paper_id="4", title="Deep Residual Learning for Image Recognition",
            year=2023,
            authors=[],
            venue="",
            citations=0),
            Paper(paper_id="5", title="YOLO: Real-Time Object Detection",
            year=2023,
            authors=[],
            venue="",
            citations=0),
            Paper(paper_id="6", title="Vision Transformer for Image Classification",
            year=2023,
            authors=[],
            venue="",
            citations=0)
        ...
        # RL papers
        rl_papers = [
            Paper(paper_id="7", title="Deep Reinforcement Learning with Policy Gradients",
            year=2023,
            authors=[],
            venue="",
            citations=0),
            Paper(paper_id="8", title="PPO: Proximal Policy Optimization",
            year=2023,
            authors=[],
            venue="",
            citations=0),
            Paper(paper_id="9", title="Multi-Agent Reinforcement Learning",
            year=2023,
            authors=[],
            venue="",
            citations=0)
        ...
        # General papers
        general_paper = Paper(paper_id="10", title="A Study of Machine Learning Algorithms",
            year=2023,
            authors=[],
            venue="",
            citations=0)
