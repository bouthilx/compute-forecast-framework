...
class TestIntegrationWorkflow:
    ...
    def test_degraded_api_adaptive_behavior(self):
        ...
        assert degraded_usage.current_delay_seconds > initial_usage.current_delay_seconds
    ...
    def test_concurrent_api_usage_isolation(self):
        ...
        for api_name in apis:
            ...
            assert len(results[api_name]) == 15, f"Worker didn't complete for {api_name}"
            
            # Should have successfully made most requests
            success_rate = sum(results[api_name]) / len(results[api_name])
            assert success_rate > 0.8, f"Success rate too low for {api_name}: {success_rate}"
    ...
    def test_rate_limit_enforcement_under_load(self):
        ...
        assert limitation_rate > 0.5, f"Rate limiting too permissive: {limitation_rate:.2%}"
    ...
    def test_performance_requirements_under_integration(self):
        ...
        assert avg_time_per_operation < 15.0, f"Integrated operation too slow: {avg_time_per_operation:.2f}ms"
