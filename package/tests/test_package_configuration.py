...
class TestPackageConfiguration:
    ...
    def test_python_version_requirement(self, pyproject_data):
        ...
        assert requires_python == "==3.12.*", f"Expected Python 3.12 only, got {requires_python}"
    ...
    def test_no_poetry_in_precommit(self):
        ...
        assert "poetry-check" not in content, "poetry-check should be removed from pre-commit config"
        assert "python-poetry/poetry" not in content, "poetry repo should be removed from pre-commit config"
    ...
    def test_uv_is_package_manager(self, pyproject_data):
        ...
        assert "tool" not in pyproject_data or "poetry" not in pyproject_data.get("tool", {}), \
