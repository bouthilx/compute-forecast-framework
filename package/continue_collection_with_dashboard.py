...
DOMAINS = {
    ...
    "Graph Learning & Network Analysis": [
        ...
        "network topology", "complex networks", "graph mining", "knowledge graph"
    ],
    "Scientific Computing & Applications": [
        ...
        "computational chemistry", "climate modeling", "computational fluid dynamics"
    ]
}
...
class DomainProgress:
    ...
    def completion_percentage(self) -> float:
        return (self.total_papers / self.total_target) * 100 if self.total_target > 0 else 0
...
class CollectionStats:
    ...
    def overall_percentage(self) -> float:
        return (self.total_papers / self.target_papers) * 100 if self.target_papers > 0 else 0
...
class CollectionDashboard:
    ...
    def _setup_layout(self):
        ...
        self.layout["stats"].split_row(
            Layout(name="domain_stats", ratio=3),
                ...
                for domain_name, years_data in domain_stats.items():
                    if domain_name in self.domain_progress:
                        for year_str, count in years_data.items():
                            if year_str.isdigit():
                                year = int(year_str)
                                self.domain_progress[domain_name].papers_by_year[year] = count
                        
                        # Calculate total papers for domain
                        self.domain_progress[domain_name].total_papers = sum(years_data.values())
    ...
    def _initialize_progress_tasks(self):
        ...
        self.main_task_id = self.progress.add_task(
            ...
            total=self.stats.target_papers
        )
    ...
    def _update_progress_tasks(self):
        ...
        if self.main_task_id is not None:
            self.progress.update(
                self.main_task_id,
                completed=self.stats.total_papers
            )
    ...
    def create_header(self) -> Panel:
        ...
        header_text = Text()
        header_text.append("Paper Collection Dashboard - Continue Collection\n", style="bold cyan")
        header_text.append(f"Target: {self.stats.target_papers} papers | ", style="white")
        header_text.append(f"Current: {self.stats.total_papers} | ", style="green" if self.stats.total_papers >= TARGET_TOTAL else "yellow")
        header_text.append(f"Remaining: {self.stats.papers_remaining}\n", style="red" if self.stats.papers_remaining > 0 else "green")
        header_text.append(f"Runtime: {elapsed} | API Calls: {self.stats.api_calls_made} | Rate Limits: {self.stats.rate_limit_hits}", style="dim")
    ...
    def create_domain_stats_panel(self) -> Panel:
        ...
        for domain_name, progress_data in self.domain_progress.items():
            ...
            table.add_row("Papers", f"{progress_data.total_papers}/{progress_data.total_target}")
            ...
            if progress_data.current_year and progress_data.total_keywords > 0:
                kw_progress = f"{progress_data.keywords_processed}/{progress_data.total_keywords}"
                table.add_row("Keywords", kw_progress)
            
            # Recent years
            recent_years = sorted(progress_data.papers_by_year.keys())[-2:] if progress_data.papers_by_year else []
    ...
    def create_summary_panel(self) -> Panel:
        ...
        if self.stats.new_papers_collected > 0 and elapsed.total_seconds() > 0:
            rate = self.stats.new_papers_collected / (elapsed.total_seconds() / 60)  # papers per minute
        ...
        return Panel(
            table,
            title="[bold magenta]Summary Stats",
            border_style="magenta"
        )
    
    def create_logs_panel(self) -> Panel:
        """Create the logs panel."""
        recent_logs = self.log_messages[-12:] if self.log_messages else ["[dim]No logs yet...[/dim]"]
        log_text = "\n".join(recent_logs)
...
def semantic_scholar_search(query, year, limit=10, dashboard=None):
    ...
    if dashboard:
        ...
        dashboard.add_log(f"Querying Semantic Scholar: '{query[:30]}...' for {year}", "api")
    ...
    try:
        ...
        if response.status_code == 429:
            if dashboard:
                dashboard.update_stats(rate_limit_hits=dashboard.stats.rate_limit_hits + 1)
                dashboard.add_log("Rate limited by Semantic Scholar, waiting 10s...", "warning")
            time.sleep(10)
            return []
        ...
        for paper in data.get('data', []):
            papers.append({
                ...
                'venue_type': 'conference' if any(kw in paper.get('venue', '').lower() for kw in ['conference', 'proceedings', 'workshop']) else 'journal',
                'url': paper.get('url', ''),
                'source': 'semantic_scholar'
            })
        
        if dashboard:
            dashboard.add_log(f"Semantic Scholar returned {len(papers)} papers", "success")
...
def openalex_batched_search(keywords, year, limit_per_keyword=15, dashboard=None):
    ...
    if dashboard:
        dashboard.update_stats(api_calls_made=dashboard.stats.api_calls_made + 1)
        dashboard.add_log(f"Batched OpenAlex query for {len(keywords)} keywords in {year}", "api")
    ...
    params = {
        'search': or_query,
        'filter': f'publication_year:{year}',
        'per-page': min(100, limit_per_keyword * len(keywords_batch)),  # Max 100 per request
        'select': 'id,title,abstract,authorships,publication_year,cited_by_count,primary_location',
    ...
    try:
        ...
        if response.status_code == 403:
            if dashboard:
                dashboard.add_log("OpenAlex access limited, skipping batch...", "warning")
            return []
        ...
        if dashboard:
            dashboard.add_log(f"Batched OpenAlex returned {len(papers)} papers for {len(keywords_batch)} keywords", "success")
...
def collect_additional_papers_for_domain(domain_name, keywords, year, target_count, existing_titles, dashboard=None):
    ...
    if dashboard:
        ...
        dashboard.add_log(f"Starting collection for {domain_name} - {year} (target: {target_count})", "info")
    
    # Strategy 1: Batched OpenAlex search (much more efficient)
    if dashboard:
        dashboard.update_domain_progress(domain_name, current_keyword="Batched OpenAlex Search")
        dashboard.update_display()
    
    batched_oa_papers = openalex_batched_search(keywords[:8], year, limit_per_keyword=15, dashboard=dashboard)
    ...
    if len(collected_papers) < target_count:
        ...
        if dashboard:
            dashboard.add_log(f"Need {remaining_needed} more papers, doing individual Semantic Scholar searches", "info")
        ...
        for i, keyword in enumerate(keywords[:keywords_to_search]):
            ...
            ss_papers = semantic_scholar_search(keyword, year, limit=10, dashboard=dashboard)
    ...
    if len(collected_papers) < target_count:
        ...
        if dashboard:
            dashboard.add_log(f"Still need {remaining_needed} papers, doing individual OpenAlex searches", "info")
    ...
    if dashboard:
        dashboard.update_domain_progress(
            domain_name,
            new_papers_added=dashboard.domain_progress[domain_name].new_papers_added + len(final_papers),
            current_year=None,
            current_keyword=""
        )
...
def main():
    ...
    with Live(dashboard.layout, refresh_per_second=2, screen=True):
        try:
            dashboard.add_log("Starting continued collection to reach 800+ target", "info")
            dashboard.add_log(f"Current: {CURRENT_COUNT} papers | Target: {TARGET_TOTAL} | Need: {NEEDED}+", "info")
            
            # Load existing papers
            existing_papers, existing_titles = load_existing_papers()
            dashboard.add_log(f"Loaded {len(existing_papers)} existing papers", "success")
            ...
            for domain_name, keywords in DOMAINS.items():
                ...
                for year in YEARS:
                    ...
                    target_per_year = max(12, current_count + 5)  # Target at least 12 papers per domain/year
                    additional_needed = target_per_year - current_count
                    
                    if additional_needed > 0:
                        dashboard.add_log(f"{domain_name} - {year}: have {current_count}, need +{additional_needed}", "info")
                        
                        new_papers = collect_additional_papers_for_domain(
                            domain_name, keywords, year, additional_needed, existing_titles, dashboard
                        )
                        ...
                        domain_progress.papers_by_year[year] = current_count + len(new_papers)
                        domain_progress.total_papers = sum(domain_progress.papers_by_year.values())
                        
                        dashboard.add_log(f"Added {len(new_papers)} papers for {domain_name} {year}", "success")
                        dashboard.update_display()
                    ...
                    if len(all_papers) >= TARGET_TOTAL:
                        dashboard.add_log(f"🎯 Target reached! Total papers: {len(all_papers)}", "success")
                        break
            ...
            dashboard.add_log(f"Target achieved: {final_count >= TARGET_TOTAL}", "success" if final_count >= TARGET_TOTAL else "warning")
            ...
            final_stats = {
                ...
                'domain_distribution': {domain: dict(years) for domain, years in final_domain_stats.items()},
