...
def openalex_search(query, year, limit=10):
    ...
    try:
        ...
        if response.status_code == 403:
            print(f"    OpenAlex access limited, skipping...")
            return []
...
def collect_additional_papers_for_domain(domain_name, keywords, year, target_count, existing_titles):
    ...
    for domain_name, keywords in DOMAINS.items():
        ...
        for year in YEARS:
            ...
            if additional_needed > 0:
                print(f"  {domain_name} - {year}: have {current_count}, targeting {target_per_year} (+{additional_needed})")
                ...
                if new_papers_collected % 20 == 0:
                    ...
                    status['current_task'] = f'Collected {new_papers_collected} new papers - total {current_total}'
