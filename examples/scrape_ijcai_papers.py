...
class IJCAIPaperCollector:
    ...
    def save_to_json(self, papers: List[Dict[str, Any]], output_file: str):
        ...
        output_data = {"metadata": collection_metadata, "papers": papers}
...
def main():
    ...
    parser.add_argument(
        "year", type=int, help="Year to scrape IJCAI papers from (e.g., 2024)"
    )
    ...
    if args.year < 2018 or args.year > current_year:
        logger.error(
            f"Year {args.year} is out of range. Please use a year between 2018 and {current_year}"
        )
        sys.exit(1)
