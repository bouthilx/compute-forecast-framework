...
def generate_sample_paper(
    venue: str, year: int, quality_level: str = "medium"
) -> Dict[str, Any]:
...
def demonstrate_quality_filtering():
    ...
    print(
        f"Strict Filter - Passed: {len(strict_results['passed'])}, Failed: {len(strict_results['failed'])}"
    )
    print(
        f"Lenient Filter - Passed: {len(lenient_results['passed'])}, Failed: {len(lenient_results['failed'])}"
    )
...
def demonstrate_adaptive_thresholds():
    ...
    print(
        f"  Min Paper Quality Score: {initial_thresholds.min_paper_quality_score:.3f}"
    )
    print(
        f"  Min Combined Quality Score: {initial_thresholds.min_combined_quality_score:.3f}"
    )
    ...
    print(
        f"  Collection Efficiency: {poor_performance.collection_efficiency:.2%} (Target: 85%)"
    )
    ...
    print(
        f"  Min Paper Quality Score: {updated_thresholds.min_paper_quality_score:.3f}"
    )
    print(
        f"  Min Combined Quality Score: {updated_thresholds.min_combined_quality_score:.3f}"
    )
    ...
    print("\nThreshold Changes:")
    print(
        f"  Paper Quality Score: {updated_thresholds.min_paper_quality_score - initial_thresholds.min_paper_quality_score:+.3f}"
    )
    print(
        f"  Combined Quality Score: {updated_thresholds.min_combined_quality_score - initial_thresholds.min_combined_quality_score:+.3f}"
    )
...
def demonstrate_monitoring_integration():
    ...
    for venue in venues:
        ...
        for _ in range(10):
            ...
            if passes:
                ...
                integration.performance_tracker.record_rejection(
                    metrics, reasons, venue, 2024
                )
    ...
    print(
        f"  Average Collection Efficiency: {dashboard_metrics['average_collection_efficiency']:.2%}"
    )
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(
            "✓ Multiple adaptation strategies (Conservative, Balanced, Aggressive, Static)"
        )
