...
def analyze_rl_subdomains(rl_with_data, rl_without_data):
    ...
    all_subdomains = set(subdomains_with_data.keys()) | set(subdomains_without_data.keys())
    
    print("RL SUBDOMAINS - With vs Without Datasets/Environments:")
    print("=" * 90)
    print(f"{'Subdomain':<40} {'With Data':<12} {'Without Data':<12} {'Total':<8} {'% With Data':<12}")
    print("=" * 90)
    ...
    for subdomain in sorted(all_subdomains):
        ...
        print(f"{subdomain:<40} {with_count:<12} {without_count:<12} {total_count:<8} {pct_with_data:<11.1f}%")
    ...
    for i, sub in enumerate(significant_subdomains[:10]):
        print(f"  {i+1}. {sub['subdomain']}: {sub['pct_with_data']:.1f}% ({sub['with_data']}/{sub['total']} papers)")
...
def improved_theoretical_empirical_classification(rl_with_data, rl_without_data, paper_lookup):
    ...
    print("-" * 50)
    for paper_type, count in sorted(with_data_classification.items(), key=lambda x: x[1], reverse=True):
    ...
    print("-" * 50)
    for paper_type, count in sorted(without_data_classification.items(), key=lambda x: x[1], reverse=True):
    ...
    print(f"\nVERIFICATION:")
    print(f"  Papers with data classified: {total_with} (should be {len(rl_with_data)})")
    print(f"  Papers without data classified: {total_without} (should be {len(rl_without_data)})")
    ...
    all_types = set(with_data_classification.keys()) | set(without_data_classification.keys())
    
    for paper_type in sorted(all_types):
        with_pct = with_data_classification.get(paper_type, 0) / len(rl_with_data) * 100
        without_pct = without_data_classification.get(paper_type, 0) / len(rl_without_data) * 100
        diff = with_pct - without_pct
        
        print(f"{paper_type:<12} {with_pct:<14.1f}% {without_pct:<14.1f}% {diff:+<11.1f}%")
...
def main():
    ...
    with_data_classification, without_data_classification = improved_theoretical_empirical_classification(
