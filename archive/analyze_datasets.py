...
def create_dataset_taxonomy():
    ...
    dataset_taxonomy = {
        ...
        'Reinforcement Learning & Robotics': {
            'keywords': [
                # RL environments
                'atari', 'gym', 'openai gym', 'mujoco', 'dm control',
                ...
                'control', 'continuous control', 'cart pole',
                'pendulum', 'acrobot', 'mountain car'
            ]
        },
...
def extract_datasets_from_papers():
    ...
    for i, paper_json in enumerate(papers_data):
        if i % 500 == 0:
            print(f"Processing paper {i}/{len(papers_data)} ({len(papers_with_datasets)} with datasets)")
...
def compare_classifications():
    ...
    dataset_classifications, dataset_domain_stats = classify_papers_by_datasets(papers_with_datasets, dataset_taxonomy)
    
    print("\\n=== DATASET-BASED DOMAIN CLASSIFICATION ===\\n")
    
    total_classified_by_datasets = sum(dataset_domain_stats.values())
    
    for domain, count in sorted(dataset_domain_stats.items(), key=lambda x: x[1], reverse=True):
    ...
    for dataset_domain, research_domain in domain_mapping.items():
        ...
        dataset_pct = (dataset_count / total_classified_by_datasets) * 100 if total_classified_by_datasets > 0 else 0
        research_pct = (research_count / total_research_domains) * 100 if total_research_domains > 0 else 0
    ...
    for domain in ['Computer Vision & Medical Imaging', 'Natural Language Processing', 'Reinforcement Learning & Robotics']:
        domain_papers = [(pid, info) for pid, info in dataset_classifications.items() if info['domain'] == domain]
        ...
        for i, (paper_id, info) in enumerate(domain_papers[:3]):
            ...
            print(f"     Keywords matched: {', '.join(info['matched_keywords'][:5])}...")
            print()
    ...
    print(f"Results saved to dataset_domain_comparison.json")
