...
def validate_collection_results() -> Dict[str, Any]:
    ...
    print("📂 Checking collection output files:")
    ...
    if not files_exist["raw_papers"]:
        print(
            "❌ No collection data found. Collection may have failed or is incomplete."
        )
        ...
        for year in expected_years:
            if (domain, year) not in actual_combinations:
                validation["coverage_gaps"].append(
                    {"domain": domain, "year": year, "papers_found": 0}
                )
    ...
    papers_with_computational_analysis = len(
        [p for p in papers if "computational_analysis" in p]
    )

    avg_citations = (
        sum(p.get("citations", 0) for p in papers) / len(papers) if papers else 0
    )
    ...
    validation["quality_indicators"] = {
        ...
        "papers_with_computational_analysis_pct": papers_with_computational_analysis
        / len(papers),
    ...
    for domain, stats in validation["domain_analysis"].items():
        print(
            f"  {domain}: {stats['total_papers']} papers, {stats['years_covered']} years"
        )
    ...
    success_criteria = {
        ...
        "citations_90%_plus": validation["quality_indicators"][
            "papers_with_citations_pct"
        ]
        >= 0.9,
        "computational_analysis_complete": validation["quality_indicators"][
            "papers_with_computational_analysis_pct"
        ]
        >= 0.8,
    ...
    if coverage_percentage < 0.8:
        validation["recommendations"].append(
            "Significant coverage gaps - consider additional collection methods or longer collection time"
        )

    if validation["quality_indicators"]["papers_with_citations_pct"] < 0.9:
        validation["recommendations"].append(
            "High number of papers without citation data - verify API sources"
        )

    if len(validation["coverage_gaps"]) > 10:
        validation["recommendations"].append(
            "Many domain/year combinations missing - focus collection on specific gaps"
        )

    if validation["quality_indicators"]["source_diversity"] < 2:
        validation["recommendations"].append(
            "Low source diversity - ensure multiple APIs are working"
        )
    ...
    enhanced_stats = {
        ...
        "overall_assessment": {
            ...
            "success_criteria_met": sum(
                validation["success_criteria_assessment"].values()
            ),
            "success_criteria_total": len(validation["success_criteria_assessment"]),
            "ready_for_next_phase": validation["collection_completeness"][
                "coverage_percentage"
            ]
            >= 0.7,
        },
...
def main():
    ...
    print("  - status/worker6-validation.json")
    print("  - data/enhanced_collection_statistics.json")

    # Overall assessment
    overall_status = stats["overall_assessment"]["collection_status"]
    success_rate = (
        stats["overall_assessment"]["success_criteria_met"]
        / stats["overall_assessment"]["success_criteria_total"]
    )

    print("\n🎯 Overall Assessment:")
    print(f"  Collection Status: {overall_status.upper()}")
    print(
        f"  Success Criteria: {stats['overall_assessment']['success_criteria_met']}/{stats['overall_assessment']['success_criteria_total']} ({success_rate:.1%})"
    )
    print(
        f"  Ready for Next Phase: {'✅ YES' if stats['overall_assessment']['ready_for_next_phase'] else '❌ NO'}"
    )
