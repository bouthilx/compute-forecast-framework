...
def calculate_detection_rates():
    ...
    for domain, stats in overlap_data["missing_analysis"].items():
        detection_rate = (
            stats["with_datasets"] / stats["total_research_papers"]
            if stats["total_research_papers"] > 0
            else 0
        )
...
def apply_corrections_method1():
    ...
    for research_domain, dataset_domain in domain_mapping.items():
        ...
        estimated_true_count = (
            dataset_count / detection_rate if detection_rate > 0 else dataset_count
        )

        # Weighted combination
        corrected_count = (
            dataset_weight * estimated_true_count + research_weight * research_count
        )
...
def compare_all_methods():
    ...
    print("=" * 100)
    print(
        f"{'Domain':<35} {'Original':<12} {'Method 1':<12} {'Method 2':<12} {'Dataset':<12}"
    )
    print("=" * 100)
    ...
    for domain in domains:
        ...
        print(
            f"{domain:<35} {original:<12} {method1:<12.0f} {method2:<12.0f} {dataset:<12}"
        )
    ...
    for domain in domains:
        ...
        print(
            f"{domain:<35} {original_pct:<12.1f}% {method1_pct:<11.1f}% {method2_pct:<11.1f}% {dataset_pct:<11.1f}%"
        )
...
def main():
    ...
    print(
        "1. Computer Vision & Medical Imaging is significantly underestimated in research domains"
    )
