...
class CollectionProgress:
    ...
    def overall_percentage(self) -> float:
        return (
            (self.total_papers_collected / self.total_papers_target) * 100
            if self.total_papers_target > 0
            else 0
        )
...
class CollectionDashboard:
    ...
    def _setup_layout(self):
        ...
        self.layout["stats"].split_row(
            Layout(name="domain_stats", ratio=4), Layout(name="summary_stats", ratio=1)
        )
                ...
                for domain_key, stats in data.items():
                    ...
                    if full_name in self.domain_stats:
                        if isinstance(stats, dict):
                            self.domain_stats[full_name].total_papers = stats.get(
                                "total_papers", 0
                            )
                        else:
                ...
                for domain_data in data:
                    ...
                    if domain_name in self.domain_stats:
                        ...
                        for year_str, count in year_data.items():
                            if year_str.isdigit():
                                year = int(year_str)
                                self.domain_stats[domain_name].papers_by_year[year] = (
                                    count
                                )
    ...
    def create_progress_panel(self) -> Panel:
        ...
        self.progress.update(
            self.main_task_id, completed=self.collection_progress.total_papers_collected
        )
    ...
    def create_domain_stats_panel(self) -> Panel:
        ...
        for domain_name, stats in self.domain_stats.items():
            ...
            recent_years = (
                sorted(stats.papers_by_year.keys())[-3:] if stats.papers_by_year else []
            )
            ...
            short_name = domain_name.split()[0] + (
                " " + domain_name.split()[1] if len(domain_name.split()) > 1 else ""
            )
            panel = Panel(
                table,
    ...
    def create_summary_panel(self) -> Panel:
        ...
        avg_completion = sum(
            stats.completion_percentage for stats in self.domain_stats.values()
        ...
        top_domain = max(
            self.domain_stats.values(), key=lambda x: x.completion_percentage
        )
        ...
        return Panel(
            table, title="[bold magenta]Summary Statistics", border_style="magenta"
        )
