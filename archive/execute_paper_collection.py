...
def main():
    ...
    try:
        ...
        collection_results = domain_collector.execute_domain_collection(
        ...
        collection_stats = {
            "collection_summary": {
                ...
                "papers_per_second": len(collection_results["raw_papers"]) / duration
                if duration > 0
                else 0,
            },
        ...
        print("📁 Output files:")
        print(
            f"  - data/raw_collected_papers.json ({len(collection_results['raw_papers'])} papers)"
        )
        print("  - data/collection_statistics.json")
        print(
            f"  - data/failed_searches.json ({len(collection_results['failed_searches'])} failed searches)"
        )
