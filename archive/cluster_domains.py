...
def analyze_domain_clusters():
    ...
    for domain in unmapped:
        instance_count = next(
            df["instance_count"]
            for df in domain_features
            if df["domain_name"] == domain
        )
        print(f"  - {domain}: {instance_count} papers")

    # Semantic clustering approach
    print("\\n\\n2. SEMANTIC CLUSTERING")
    clusters, vectorizer, X, kmeans = cluster_domains_semantic(
        domain_features, n_clusters=6
    )
    ...
    results = {
        "manual_mapping": {
            "area_counts": {
                area: [(name, count) for name, count in domains]
                for area, domains in area_counts.items()
            },
