...
def create_enhanced_taxonomy():
    ...
    enhanced_taxonomy = {
        'Computer Vision & Medical Imaging': {
            'keywords': [
                # Image datasets
                'imagenet', 'cifar', 'mnist', 'coco', 'pascal voc', 'voc',
                'celeba', 'flickr', 'places', 'ade20k', 'cityscapes',
                'open images', 'visual genome', 'kinetics', 'ucf',
                # Medical imaging datasets
                'mimic', 'isic', 'brats', 'luna', 'lidc', 'adni',
                'ukbiobank', 'hcp', 'abide', 'oasis', 'ixi',
                'mri', 'ct scan', 'x-ray', 'ultrasound', 'mammography',
                'retinal', 'fundus', 'oct', 'pathology', 'histology',
                # Computer vision specific
                'face', 'detection', 'segmentation', 'tracking', 'recognition',
                'video', 'image', 'visual', 'photo', 'picture'
            ]
        },
        'Natural Language Processing': {
            'keywords': [
                # Text datasets
                'glue', 'superglue', 'squad', 'commonsenseqa', 'hellaswag',
                'winogrande', 'copa', 'wsc', 'rte', 'wnli', 'stsb',
                'cola', 'sst', 'mrpc', 'qqp', 'qnli', 'mnli',
                'xnli', 'xtreme', 'multilingual', 'wmt', 'opus',
                'opensubtitles', 'bookscorpus', 'common crawl', 'wikipedia',
                'reddit', 'twitter', 'news', 'pubmed', 'arxiv',
                'conll', 'penn treebank', 'ontonotes', 'wikitext',
                # NLP tasks
                'text', 'language', 'dialogue', 'conversation', 'qa',
                'translation', 'sentiment', 'classification', 'ner',
                'pos tagging', 'parsing', 'summarization', 'generation'
            ]
        },
        'Reinforcement Learning & Robotics': {
            'keywords': [
                ...
                'deepmind lab', 'starcraft', 'dota', 'minecraft',
                'procgen', 'coinrun', 'montezuma', 'breakout',
                'pong', 'pacman', 'pac-man', 'space invaders',
                'halfcheetah', 'hopper', 'walker', 'ant', 'humanoid',
                'reacher', 'pusher', 'swimmer', 'inverted pendulum',
                'cartpole', 'cart-pole', 'mountain car', 'acrobot',
                'lunar lander', 'bipedal walker', 'car racing',
                'freeway', 'seaquest', 'enduro', 'qbert', 'asterix',
                # Environment suites
                'atari 2600', 'arcade learning environment', 'ale',
                'deepmind control suite', 'dm_control', 'meta-world',
                'robotics suite', 'hand manipulation', 'shadow hand',
                'fetch', 'baxter', 'sawyer', 'calvin', 'robosuite',
                # RL-specific terms
                'environment', 'env', 'simulation', 'simulator',
                'policy', 'reward', 'agent', 'episode', 'trajectory',
                'q-learning', 'policy gradient', 'actor-critic',
                'ppo', 'sac', 'ddpg', 'td3', 'rainbow', 'dqn',
                # Robotics datasets/environments
                'robotic', 'manipulation', 'grasping', 'navigation',
                'locomotion', 'bipedal', 'quadruped', 'arm',
                'gripper', 'pick and place', 'push', 'reach',
        ...
        'Graph Learning & Network Analysis': {
            'keywords': [
                ...
                'qm7', 'qm8', 'qm9', 'zinc', 'tox21', 'toxcast',
                'muv', 'hiv', 'bace', 'bbbp', 'clintox', 'sider',
                'freesolv', 'esol', 'lipophilicity',
                # Social networks
                'facebook', 'twitter', 'social network', 'citation',
                'collaboration', 'friendship', 'interaction',
                # Molecular/chemical
                'molecular', 'chemical', 'drug', 'protein', 'compound',
                'smiles', 'graph', 'network', 'node', 'edge'
            ]
        },
        'Scientific Computing & Applications': {
            'keywords': [
                ...
                'atmospheric', 'oceanographic', 'satellite', 'remote sensing',
                'astronomy', 'astrophysics', 'cosmology', 'galaxy',
                'star', 'planet', 'telescope', 'hubble', 'sloan',
                # Biology/chemistry
                'protein', 'dna', 'rna', 'genome', 'genetic',
                'molecular dynamics', 'simulation', 'quantum',
        ...
        'Speech & Audio': {
            'keywords': [
                # Audio datasets
                'librispeech', 'common voice', 'voxceleb', 'timit',
                'switchboard', 'fisher', 'wsj', 'tedlium',
                'musicnet', 'nsynth', 'gtzan', 'fma', 'million song',
                'speech', 'audio', 'voice', 'sound', 'acoustic',
                'music', 'song', 'instrument', 'phoneme', 'asr'
            ]
        },
        'Machine Learning Benchmarks': {
            'keywords': [
                # General ML datasets
                'uci', 'adult', 'housing', 'wine', 'iris', 'diabetes',
                'breast cancer', 'titanic', 'heart disease',
                'mushroom', 'abalone', 'car evaluation',
                # Benchmark suites
                'openml', 'sklearn', 'benchmark', 'synthetic',
...
def extract_datasets_and_environments():
    ...
    for i, paper_json in enumerate(papers_data):
        if i % 500 == 0:
            print(f"Processing paper {i}/{len(papers_data)} ({len(papers_with_data)} with data)")
        ...
        try:
            ...
            env_keywords = [
                'atari', 'gym', 'mujoco', 'dm control', 'openai gym',
                'environment', 'simulator', 'cartpole', 'mountain car',
                'halfcheetah', 'hopper', 'walker', 'ant', 'humanoid',
                'procgen', 'deepmind lab', 'starcraft', 'minecraft'
            ]
...
def compare_original_vs_enhanced():
    ...
    enhanced_classifications, enhanced_stats = classify_papers_enhanced(papers_with_data)
    
    print("Detailed comparison:")
    print("=" * 80)
    print(f"{'Domain':<35} {'Original':<12} {'Enhanced':<12} {'Difference':<12} {'% Change':<10}")
    print("=" * 80)
    ...
    for domain in sorted(all_domains):
        ...
        print(f"{domain:<35} {original_count:<12} {enhanced_count:<12} {difference:+<12} {pct_change:+<10.1f}%")
    
    print("=" * 80)
    print(f"{'TOTAL':<35} {sum(original_stats.values()):<12} {sum(enhanced_stats.values()):<12}")
    ...
    print(f"Improvement: +{rl_improvement} papers ({(rl_improvement/rl_original)*100:+.1f}%)")
    
    # Show sample RL papers with environments
    rl_papers = [info for info in enhanced_classifications.values() 
...
def analyze_environment_vs_dataset_split():
    ...
    rl_papers = {pid: info for pid, info in enhanced_classifications.items() 
                if info['domain'] == 'Reinforcement Learning & Robotics'}
    ...
    print(f"  RL papers with environments: {rl_with_envs} ({rl_with_envs/len(rl_papers)*100:.1f}%)")
    print(f"  RL papers with datasets: {rl_with_datasets} ({rl_with_datasets/len(rl_papers)*100:.1f}%)")
...
def final_enhanced_recommendation():
    ...
    print("=" * 90)
    print(f"{'Domain':<35} {'Research':<12} {'Enhanced':<12} {'Agreement':<15} {'Conclusion':<15}")
    print("=" * 90)
    
    total_research = sum(research_stats.values())
    total_enhanced = sum(enhanced_stats.values())
    
    for domain in ['Computer Vision & Medical Imaging', 'Natural Language Processing', 
        ...
        agreement = min(research_pct, enhanced_pct) / max(research_pct, enhanced_pct) * 100
        ...
        print(f"{domain:<35} {research_pct:<12.1f}% {enhanced_pct:<11.1f}% {agreement:<14.1f}% {conclusion:<15}")
    
    print("=" * 90)
    
    # Final verdict
    rl_research_pct = research_stats.get('Reinforcement Learning & Robotics', 0) / total_research * 100
    rl_enhanced_pct = enhanced_stats.get('Reinforcement Learning & Robotics', 0) / total_enhanced * 100
