#!/usr/bin/env python3
"""
COMPLETELY FIXED domain extraction that checks BOTH old and new analysis formats.
Extracts from both 'analysis' and 'extractions' fields when available.
"""
    ...
    try:
        ...
        basic_taxonomy = {
            "classification": {
                ...
                "Natural Language Processing": {"category": "Natural Language Processing"},
                "NLP": {"category": "Natural Language Processing"},
                "Reinforcement Learning": {"category": "Reinforcement Learning & Robotics"},
...
def extract_domains_from_new_format(extractions):
    """Extract research domains from new format (extractions field)."""
...
def extract_all_domains_completely_fixed():
    """Extract all research domains with COMPLETELY FIXED logic - check both formats."""
    
    print("COMPLETELY FIXED DOMAIN EXTRACTION")
    print("="*50)
    ...
    for i, paper_json in enumerate(papers_data):
        ...
        try:
            ...
            if paper_formats:
                ...
                if 'both' in paper_formats or ('old' in paper_formats and 'new' in paper_formats):
    ...
    print(f"\nCOMPLETELY FIXED EXTRACTION STATISTICS:")
    ...
    # Save results
    with open('all_domains_completely_fixed.json', 'w') as f:
        json.dump(all_domains, f, indent=2)
    
    print(f"\nResults saved to all_domains_completely_fixed.json")
    print(f"Found {len(all_domains)} domain entries from {len(set(d['paper_id'] for d in all_domains))} unique papers")
    
    return all_domains, stats

def main():
    """Run the completely fixed domain extraction."""
    
    domains, stats = extract_all_domains_completely_fixed()
