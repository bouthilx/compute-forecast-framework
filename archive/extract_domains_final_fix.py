#!/usr/bin/env python3
"""
FINAL FIX: Extract domains from BOTH 'analysis' AND 'extractions' fields.
Fixed the typo: it's 'extractions' not 'extraction'.
"""
...
def extract_all_domains_from_query(query_file_path):
    ...
    try:
        ...
        # Try new format (extractions field) - FIXED: was 'extraction' now 'extractions'
...
def extract_all_domains_final_fix():
    """Extract all research domains with FINAL FIX - correct field name 'extractions'."""
    
    print("FINAL FIX DOMAIN EXTRACTION")
    print("="*50)
    ...
    print(f"\nFINAL FIX EXTRACTION STATISTICS:")
    ...
    # Save results
    with open('all_domains_final_fix.json', 'w') as f:
        json.dump(all_domains, f, indent=2)
    
    print(f"\nResults saved to all_domains_final_fix.json")
...
def main():
    """Run the final fixed domain extraction."""
    
    domains, stats = extract_all_domains_final_fix()
