...
class MilaVenueStatisticsGenerator:
    ...
    def __init__(self, input_file: str = "all_domains_full.json"):
        ...
        self.venue_patterns = [
            ...
            # ICML variations
            (
                r"Proceedings of the \d+th International Conference on Machine Learning",
                "ICML",
            ),
            (
                r"Proceedings of the \d+st International Conference on Machine Learning",
                "ICML",
            ),
            (
                r"Proceedings of the \d+nd International Conference on Machine Learning",
                "ICML",
            ),
            (
                r"Proceedings of the \d+rd International Conference on Machine Learning",
                "ICML",
            ),
            # CVPR variations
            (
                r"Proceedings of the IEEE/CVF Conference on Computer Vision and Pattern Recognition",
                "CVPR",
            ),
            (r"IEEE/CVF Conference on Computer Vision and Pattern Recognition", "CVPR"),
            # ICCV variations
            (
                r"Proceedings of the IEEE/CVF International Conference on Computer Vision",
                "ICCV",
            ),
        ...
        self.citation_averages = {
            ...
            "JMLR": 32.1,
            "Nature MI": 42.5,
            "Science Robotics": 38.7,
            "IJRR": 26.8,
            "Neural Networks": 22.1,
    ...
    def extract_venue_counts(self) -> Dict[str, Dict]:
        ...
        for venue, data in venue_data.items():
            final_venue_data[venue] = {
                ...
                "paper_count": len(
                    data["papers"]
                ),  # Total records (may have duplicates)
    ...
    def create_domain_venue_mapping(self) -> Dict[str, Dict[str, int]]:
        ...
        for domain, venues in domain_venues.items():
            final_mapping[domain] = {
                venue: len(paper_ids) for venue, paper_ids in venues.items()
            }
    ...
    def calculate_venue_metadata(self, venue_counts: Dict[str, Dict]) -> Dict[str, Any]:
        ...
        for venue, data in venue_counts.items():
            ...
            venue_metadata["computational_scores"][venue] = (
                self.computational_scores.get(venue, 0.5)
            )

            # Citation averages
            venue_metadata["citation_averages"][venue] = self.citation_averages.get(
                venue, 15.0
            )
            ...
            venue_metadata["domain_diversity"][venue] = {
                ...
                "diversity_score": min(
                    domain_count / 5.0, 1.0
                ),  # Normalize to max 5 domains
            }
    ...
    def generate_analysis_summary(
        self, venue_counts: Dict, domain_mapping: Dict
    ) -> Dict[str, Any]:
    ...
    def save_results(self, results: Dict[str, Any]) -> None:
        ...
        logger.info(
            f"Results saved successfully ({self.output_file.stat().st_size} bytes)"
        )
