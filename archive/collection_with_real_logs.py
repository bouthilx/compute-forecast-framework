...
def main():
    ...
    try:
        print(f"🚀 Starting Worker 6 paper collection continuation...")
        ...
        with Live(create_full_layout(), refresh_per_second=2) as live:
            ...
            for domain_name, keywords in DOMAINS.items():
                ...
                for year in YEARS:
                    ...
                    additional_needed = max(8, current_domain_count + 3) - current_domain_count
                    
                    if additional_needed > 0:
                        print(f"📊 {domain_name} {year}: need {additional_needed} more papers")
                        ...
                        for i, keyword in enumerate(keywords[:3]):
                            if len(year_papers) >= additional_needed:
                                print(f"🎯 Target reached with {len(year_papers)} papers")
                                break
                            ...
                            for paper in ss_papers:
                                ...
                                if title and title not in existing_titles and len(year_papers) < additional_needed:
                                    ...
                                    new_papers += 1
                                    print(f"➕ Added: \"{paper.get('title', '')[:40]}...\"")
                                elif title in existing_titles:
                                    print(f"🔄 Duplicate: \"{paper.get('title', '')[:40]}...\"")
                            
                            print(f"📈 Semantic Scholar: {new_papers} new papers")
                            
                            # Rate limiting
                            print(f"⏱️ Rate limiting: 3 seconds...")
                            for countdown in range(3, 0, -1):
                                print(f"⏳ {countdown}s...")
                                time.sleep(1)
                            
                            # OpenAlex
                            oa_papers = collector.openalex_search(keyword, year, limit=5)
                        ...
                        print(f"✅ {domain_name} {year}: {len(year_papers)} papers collected")
                        ...
                        print(f"🎯 Total: {current_total}/800 ({(current_total/800)*100:.1f}%)")
                ...
                if len(all_papers) >= TARGET_TOTAL:
                    print(f"🎉 COLLECTION TARGET ACHIEVED!")
                    break
