        ...
        if len(venues) > 1:
            ...
            if not already_handled:
                ...
                for venue, data in venues:
                    ...
                    merged_data["total"] += data.get("total", 0)
                    merged_data["paper_count"] += data.get("paper_count", 0)
                    merged_data["unique_paper_count"] += data.get(
                        "unique_paper_count", 0
                    )
...
def print_analysis(duplicates, pattern_groups, merged_venues):
    ...
    other_duplicates = {
        ...
        if not any(
            venue[0] in [v2[0] for venues in pattern_groups.values() for v2 in venues]
            for venue in v
        )
    }
    ...
    total_original_venues = sum(
        len(info["original_venues"]) for info in merged_venues.values()
    )
...
def main():
    ...
    output_data = {
        ...
        "analysis_summary": {
            ...
            "venues_after_merge": len(venue_counts)
            - sum(len(info["original_venues"]) - 1 for info in merged_venues.values()),
        },
    }
