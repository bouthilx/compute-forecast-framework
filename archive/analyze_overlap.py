...
def analyze_domain_agreement(overlap_analysis):
    ...
    agreement_rate = (
        agreements / total_comparisons * 100 if total_comparisons > 0 else 0
    )
    ...
    for domain, count in sorted(
        agreement_details.items(), key=lambda x: x[1], reverse=True
    ):
        print(f"  {domain}: {count} papers")

    print("\\nTop disagreements:")
    for disagreement, count in sorted(
        disagreement_details.items(), key=lambda x: x[1], reverse=True
    )[:10]:
...
def calculate_missing_proportions(overlap_analysis):
    ...
    for research_domain, dataset_domain in domain_mapping.items():
        ...
        research_with_datasets = (
            research_papers & overlap_analysis["papers_with_datasets"]
        )
        research_missing_datasets = (
            research_papers - overlap_analysis["papers_with_datasets"]
        )

        missing_rate = (
            len(research_missing_datasets) / len(research_papers) * 100
            if research_papers
            else 0
        )
...
def main():
    ...
    print(
        "Starting overlap analysis between dataset and research domain classifications...\\n"
    )
    ...
    print(
        f"Dataset classification coverage: {overlap_analysis['dataset_coverage']:.1f}%"
    )
    print(
        f"Research domain classification coverage: {overlap_analysis['research_coverage']:.1f}%"
    )
