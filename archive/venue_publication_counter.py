...
def count_publications_by_venue(data):
    ...
    return {
        ...
        "venue_coverage_rate": papers_with_venues / len(unique_papers)
        if unique_papers
        else 0,
    }
...
def analyze_venue_patterns(venue_stats):
    ...
    for i, (venue, count) in enumerate(venue_counts.most_common(20), 1):
        ...
        print(
            f"{i:2d}. {venue:40s} | {count:3d} entries | Top domain: {top_domain[0]} ({top_domain[1]})"
        )
    print()
    ...
    # Multi-domain venues
    multi_domain_venues = {
        venue: domains for venue, domains in venue_by_domain.items() if len(domains) > 1
    }

    print("=== TOP MULTI-DOMAIN VENUES ===")
    multi_domain_sorted = sorted(
        multi_domain_venues.items(), key=lambda x: len(x[1]), reverse=True
    )
    ...
    for venue, domains in multi_domain_sorted[:10]:
        ...
        print(
            f"{venue:40s} | {len(domains)} domains, {total_count} total | {domain_list}"
        )
    print()
...
def analyze_venue_categories():
    ...
    print(
        f"Conferences: {len(conferences)} venues, {sum(c for _, c in conferences)} entries"
    )
...
def main():
    ...
    try:
        ...
        print(f"Error: {e}")
        print(
            "Make sure you have the 'all_domains_full.json' file in the current directory."
        )
        print(
            "This file should contain the processed Mila paper domain data with venue information."
        )
