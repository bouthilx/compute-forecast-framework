...
def analyze_empirical_coverage_by_domain():
    ...
    print("=" * 80)
    print(
        f"{'Category':<35} {'Research':<8} {'Dataset':<8} {'Empirical':<8} {'Coverage':<8} {'Miss Rate':<10}"
    )
    print("=" * 80)
    ...
    for category in main_categories:
        ...
        research_empirical = sum(
            1
            for pid in research_papers
            if paper_classifications.get(pid) == "empirical"
        )

        dataset_empirical = sum(
            1 for pid in dataset_papers if paper_classifications.get(pid) == "empirical"
        )

        # Calculate coverage of empirical papers
        if research_empirical > 0:
            empirical_in_both = sum(
                1
                for pid in (research_papers & dataset_papers)
                if paper_classifications.get(pid) == "empirical"
            )

            coverage_rate = empirical_in_both / research_empirical * 100
            miss_rate = (
                (research_empirical - empirical_in_both) / research_empirical * 100
            )
        else:
            coverage_rate = 0
            miss_rate = 0
        ...
        print(
            f"{category:<35} {len(research_papers):<8} {len(dataset_papers):<8} {research_empirical:<8} {coverage_rate:<7.1f}% {miss_rate:<9.1f}%"
        )
...
def calculate_correction_factors():
    ...
    print("-" * 50)
    print(
        "1. If datasets/env capture most empirical work (>80% coverage): Use dataset counts"
    )
    print(
        "2. If datasets/env miss significant empirical work (>20% miss): Adjust upward"
    )
    ...
    for category, stats in coverage_analysis.items():
        ...
        empirical_rate_in_research = (
            research_empirical / research_total * 100 if research_total > 0 else 0
        )
        ...
        print(
            f"  Empirical papers in research domains: {research_empirical} ({empirical_rate_in_research:.1f}%)"
        )
        ...
        if coverage_rate >= 80:
            ...
            if empirical_rate_in_research < 70:
                ...
                recommendation = (
                    f"Use dataset count ({dataset_total}) - good empirical alignment"
                )
                corrected_count = dataset_total
            ...
            corrected_count = int(
                dataset_total * weight_dataset + research_empirical * weight_research
            )
...
def analyze_specific_domains_empirical_gaps():
    ...
    print("=== SPECIFIC EMPIRICAL GAPS ANALYSIS ===\n")
    ...
    for domain_entry in raw_domains:
        ...
        for class_domain, info in research_data["classification"].items():
            if (
                class_domain == domain_name
                and info["category"] == "Reinforcement Learning & Robotics"
            ):
    ...
    rl_research_empirical = {
        pid
        for pid in rl_research_papers
        if paper_classifications.get(pid) == "empirical"
    }

    rl_dataset_empirical = {
        pid
        for pid in rl_dataset_papers
        if paper_classifications.get(pid) == "empirical"
    }
    ...
    if len(rl_research_empirical) > 0:
        coverage = (
            len(rl_research_empirical & rl_dataset_papers)
            / len(rl_research_empirical)
            * 100
        )
...
def main():
    ...
    print(
        "Methodology: Include all works, but correct based on empirical work coverage\n"
    )
