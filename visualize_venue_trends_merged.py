...
def create_merged_venue_trends_plot(venue_trends, years):
    ...
    sorted_venues = sorted(venue_trends.items(), 
    ...
    for i, (venue, year_counts) in enumerate(sorted_venues):
        ...
        ax.plot(years, counts, marker='o', linewidth=2.5, markersize=6, 
               label=venue, color=colors[i], alpha=0.8)
...
def create_merged_venue_heatmap(venue_trends, years):
    ...
    ax.set_title("Venue Publication Heatmap (2019-2024)\n[After Merging Duplicates]", 
...
def print_merged_summary(venue_trends, years, mapping_data):
    ...
    print(f"Original venues eliminated: {mapping_data['analysis_summary']['venues_after_merge']}")
    
    # Show major mergers
    print(f"\n=== MAJOR VENUE MERGERS ===")
    merged_venues = mapping_data['merged_venues']
    major_mergers = [(k, v) for k, v in merged_venues.items() 
                    if v['merged_data']['total'] > 10]
    ...
    for canonical, info in major_mergers[:10]:
        ...
        print(f"{canonical:30s} | {total_papers:3d} papers from {original_count} venues")
...
def main():
    ...
    try:
        ...
        venue_trends, years = get_venue_year_data_merged(primary_venues, merged_venue_counts)
        ...
        if venue_trends:
            ...
            fig1.savefig('venue_trends_merged_line_plot.png', dpi=300, bbox_inches='tight')
            plt.close(fig1)
            
            # Create heatmap
            print("Creating merged venue trends heatmap...")
            fig2 = create_merged_venue_heatmap(venue_trends, years)
            fig2.savefig('venue_trends_merged_heatmap.png', dpi=300, bbox_inches='tight')
            plt.close(fig2)
