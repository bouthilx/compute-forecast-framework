...
def create_multi_label_visualizations():
    ...
    years, categories, year_data, multi_label_stats, total_papers_by_year = (
        create_multi_label_temporal_analysis()
    )
    ...
    for i, year in enumerate(years):
        ...
        total_research_assignments = sum(
            year_data[year][cat] for cat in research_categories
        )
        ...
        for j, category in enumerate(research_categories):
            ...
            research_props[j, i] = (
                count / total_research_assignments * 100
                if total_research_assignments > 0
                else 0
            )
    ...
    ax2.set_title(
        "Research Domain Assignment Counts Over Time (MULTI-LABEL)\n(Papers can contribute to multiple domains)",
        fontsize=14,
        fontweight="bold",
    )
    ax2.set_xlabel("Year")
    ...
    for i, year in enumerate(years):
        ...
        print("  Domain assignment breakdown:")
        ...
        for num_labels, paper_count in sorted(multi_label_stats[year].items()):
            if paper_count > 0:
                pct = (
                    paper_count / papers_with_domains * 100
                    if papers_with_domains > 0
                    else 0
                )
...
def create_paper_classification_chart():
    ...
    ax1.set_title(
        "Paper Classification Proportions Over Time (MULTI-LABEL AWARE)",
        fontsize=14,
        fontweight="bold",
    )
    ...
    ax2.set_title(
        "Paper Classification Counts Over Time (MULTI-LABEL AWARE)",
        fontsize=14,
        fontweight="bold",
    )
    ax2.set_xlabel("Year")
...
def main():
    ...
    print("- research_domains_temporal_analysis_MULTI_LABEL.png")
    print("- paper_classification_temporal_analysis_MULTI_LABEL.png")
    print("- temporal_analysis_data_MULTI_LABEL.json")

    print("\nKey differences from single-label analysis:")
    print("- Papers can now contribute to multiple research domains")
    print("- Domain assignment counts may exceed paper counts")
    print("- Proportions are calculated based on total domain assignments")
    print("- Multi-label statistics show how many domains papers span")
