...
def create_fixed_multi_label_visualizations():
    ...
    (
        ...
        total_papers_by_year,
        other_only_papers,
    ) = create_fixed_multi_label_temporal_analysis()
    ...
    for i, year in enumerate(years):
        ...
        total_research_papers = sum(
            year_data[year][cat] for cat in all_research_categories
        )
        ...
        for j, category in enumerate(all_research_categories):
            ...
            research_props[j, i] = (
                count / total_research_papers * 100 if total_research_papers > 0 else 0
            )
    ...
    ax1.stackplot(
        years,
        research_props,
        labels=all_research_categories,
        colors=research_colors,
        alpha=0.8,
    )
    ax1.set_title(
        "Research Domain Proportions Over Time (MULTI-LABEL)\n(All research domains including Other)",
        fontsize=14,
        fontweight="bold",
    )
    ...
    ax2.set_title(
        "Research Domain Paper Counts Over Time (MULTI-LABEL)\n(Main domains can have multiple assignments, Other is mutually exclusive)",
        fontsize=14,
        fontweight="bold",
    )
    ax2.set_xlabel("Year")
    ...
    plt.tight_layout()
    plt.savefig(
        "all_research_domains_temporal_analysis_FIXED.png", dpi=300, bbox_inches="tight"
    )
    plt.show()
    ...
    for i, year in enumerate(years):
        ...
        papers_with_domains = (
            papers_with_analysis - year_data[year]["NO domain classification"]
        )
        ...
        if papers_with_main_domains > 0:
            ...
            print(
                f"  Average main domains per paper (with main domains): {avg_main_domains:.2f}"
            )

        print("  Main domain assignment breakdown:")
        ...
        print("  Multi-label distribution (main domains only):")
        # Separate numeric and string keys, sort only the numeric ones
        numeric_items = [
            (k, v) for k, v in multi_label_stats[year].items() if isinstance(k, int)
        ]
        for num_labels, paper_count in sorted(numeric_items):
            if paper_count > 0:
                pct = (
                    paper_count / papers_with_main_domains * 100
                    if papers_with_main_domains > 0
                    else 0
                )
                print(
                    f"    {num_labels} main domain(s): {paper_count} papers ({pct:.1f}%)"
                )
    ...
    main_assignments_2019 = sum(
        year_data[2019][cat] for cat in main_research_categories
    )
    main_assignments_2024 = sum(
    ...
    if other_2019 > 0:
        ...
        print(
            f"Other-only papers growth: {other_2019} → {other_2024} ({other_growth:+.1f}%)"
        )
    else:
...
def create_complete_paper_classification_chart():
    ...
    for i, year in enumerate(years):
        ...
        for j, count in enumerate(counts):
            ...
            classification_props[j, i] = (
                count / total_papers * 100 if total_papers > 0 else 0
            )
    ...
    ax1.stackplot(
        ...
        labels=classification_categories,
        colors=classification_colors,
        alpha=0.8,
    )
    ax1.set_title(
        "Paper Classification Proportions Over Time (FIXED MULTI-LABEL)",
        fontsize=14,
        fontweight="bold",
    )
    ...
    ax2.set_title(
        "Paper Classification Counts Over Time (FIXED MULTI-LABEL)",
        fontsize=14,
        fontweight="bold",
    )
    ax2.set_xlabel("Year")
...
def main():
    ...
    print("- all_research_domains_temporal_analysis_FIXED.png")
    print("- paper_classification_temporal_analysis_FIXED.png")
    print("- temporal_analysis_data_FIXED_MULTI_LABEL.json")

    print("\nKey corrections:")
    print("- 'Other research domains' is now mutually exclusive with main categories")
    print("- Papers with main domains can have multiple main domain labels")
    print("- Papers with ONLY other domains go to 'Other research domains'")
    print("- Other research domains now included in stacked area charts")
    print("- Statistics properly separate main vs other domain distributions")
