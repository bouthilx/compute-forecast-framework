...
class PaperCollector:
    ...
    def semantic_scholar_search(self, query, year, limit=10):
        ...
        try:
            ...
            print(f"✅ API response received - processing {len(data.get('data', []))} results")
            
            papers = []
            for i, paper in enumerate(data.get('data', [])):
                title = paper.get('title', 'No title')[:50] + ('...' if len(paper.get('title', '')) > 50 else '')
                citations = paper.get('citationCount', 0)
                venue = paper.get('venue', 'Unknown venue')[:30] + ('...' if len(paper.get('venue', '')) > 30 else '')
            ...
            print(f"✅ Semantic Scholar completed: {len(papers)} papers extracted for '{query}' ({year})")
            return papers
    ...
    def openalex_search(self, query, year, limit=10):
        ...
        try:
            ...
            print(f"✅ API response received - processing {len(data.get('results', []))} results")
            
            papers = []
            for i, work in enumerate(data.get('results', [])):
                title = work.get('title', 'No title')[:50] + ('...' if len(work.get('title', '')) > 50 else '')
                ...
                if work.get('primary_location'):
                    ...
                    if source_info:
                        venue = source_info.get('display_name', 'Unknown venue')[:30] + ('...' if len(source_info.get('display_name', '')) > 30 else '')
            ...
            print(f"✅ OpenAlex completed: {len(papers)} papers extracted for '{query}' ({year})")
            return papers
...
def create_layout(tracker):
    ...
    log_text = Text("\\n".join(recent_lines[-25:]) if recent_lines else "Waiting for collection activity...")
...
def main():
    ...
    print(f"🚀 Starting Worker 6 paper collection with real-time streaming...")
    ...
    try:
        ...
        with Live(create_full_layout(), refresh_per_second=6) as live:  # High refresh rate for real-time feel
            ...
            for domain_name, keywords in DOMAINS.items():
                ...
                for year in YEARS:
                    year_count += 1
                    current_domain_count = domain_stats.get(domain_name, {}).get(str(year), 0)
                    target_per_year = max(15, current_domain_count + 8)  # Target 15+ papers per domain/year
                    additional_needed = target_per_year - current_domain_count
                    
                    if additional_needed > 0:
                        print(f"📊 {domain_name} {year}: have {current_domain_count}, targeting {additional_needed} more papers")
                        ...
                        for i, keyword in enumerate(keywords[:6]):  # Use top 6 keywords
                            if len(year_papers) >= additional_needed:
                                print(f"🎯 Target reached for {domain_name} {year} with {len(year_papers)} papers")
                                break
                                
                            print(f"🔍 Keyword {i+1}/6: Searching for '{keyword}' in {domain_name} {year}")
                            
                            # Semantic Scholar
                            ss_papers = collector.semantic_scholar_search(keyword, year, limit=10)
                            
                            # Process Semantic Scholar results
                            new_papers_from_ss = 0
                            for paper in ss_papers:
                                title = paper.get('title', '').lower().strip()
                                if title and title not in existing_titles and len(year_papers) < additional_needed:
                                    existing_titles.add(title)
                                    ...
                                    print(f"➕ Added paper: \"{paper.get('title', 'No title')[:40]}...\" ({paper.get('citations', 0)} citations)")
                                elif title in existing_titles:
                                    print(f"🔄 Duplicate skipped: \"{paper.get('title', 'No title')[:40]}...\"")
                            
                            print(f"📈 Semantic Scholar: {new_papers_from_ss} new papers added from {len(ss_papers)} results")
                            ...
                            # OpenAlex
                            oa_papers = collector.openalex_search(keyword, year, limit=10)
                            ...
                            print(f"📈 OpenAlex: {new_papers_from_oa} new papers added from {len(oa_papers)} results")
                        ...
                        print(f"✅ {domain_name} {year} completed: {len(year_papers)} papers collected this year")
                        
                        # Update domain stats
                        update_domain_stats(tracker, all_papers)
                        print(f"📊 Domain statistics updated")
                        ...
                        print(f"🎯 Session total: {len(year_papers)} new papers | Overall total: {current_total}/800 ({(current_total/800)*100:.1f}%)")
                        
                        # Save progress periodically
                        if new_papers_collected % 25 == 0 and new_papers_collected > 0:
                            print(f"💾 Saving progress: {current_total} papers collected so far...")
                            with open('data/raw_collected_papers.json', 'w') as f:
                                json.dump(all_papers, f, indent=2)
                            print(f"✅ Progress saved to data/raw_collected_papers.json")
                    else:
                        print(f"✅ {domain_name} {year}: Already has {current_domain_count} papers (target met)")
                ...
                if len(all_papers) >= TARGET_TOTAL:
                    print(f"🎯 TARGET ACHIEVED! Collected {len(all_papers)} papers - stopping collection")
                    break
            ...
            print(f"🎯 Target achievement: {(final_count/TARGET_TOTAL)*100:.1f}% ({final_count}/{TARGET_TOTAL})")
            
            # Save final results
            print(f"💾 Saving final results to data/raw_collected_papers.json...")
            with open('data/raw_collected_papers.json', 'w') as f:
                json.dump(all_papers, f, indent=2)
            print(f"✅ Papers saved successfully")
            
            # Update statistics
            print(f"📊 Generating final collection statistics...")
