...
def extract_venue_data(
    primary_venues: List[str], venue_stats: Dict, years: List[int]
            ...
            for stat_venue, stats in venue_stats.items():
                ...
                if normalized_stat_venue == venue:
                    ...
                    for year_str, count in by_year.items():
                        year = int(year_str)
                        if year in years:
                            venue_data[venue][year] += (
                                count  # Add in case of multiple matches
                            )
                    found = True
...
def create_venue_visualization(
    venue_data: Dict[str, Dict[int, int]], years: List[int], output_path: Path
):
    """
    ...
    active_venues = {
        venue: data for venue, data in venue_data.items() if sum(data.values()) > 0
    }
    ...
    sorted_venues = sorted(
        active_venues.items(), key=lambda x: sum(x[1].values()), reverse=True
    )
    ...
    for i, (venue, data) in enumerate(sorted_venues):
        ...
        if total_papers > 0:  # Only plot venues with data
            plt.plot(
                ...
                markersize=6,
                color=colors[i],
                label=f"{venue} (total: {total_papers})",
            )

    # Customize the plot
    plt.title(
        "Mila Papers by Primary Venue (2019-2024)",
        fontsize=16,
        fontweight="bold",
        pad=20,
    )
...
def create_top_venues_bar_chart(
    venue_data: Dict[str, Dict[int, int]], output_path: Path, top_n: int = 15
):
    """
    ...
    # Customize the plot
    plt.title(
        f"Top {top_n} Primary Venues by Total Mila Papers (2019-2024)",
        fontsize=16,
        fontweight="bold",
        pad=20,
    )
