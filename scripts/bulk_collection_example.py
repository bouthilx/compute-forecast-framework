...
def bulk_collect_by_venues(venue_ids: List[str], years: List[int], email: str = None):
    ...
    for venue_id in venue_ids:
        for year in years:
            ...
            # Construct filter
            filter_str = (
                f"primary_location.source.id:{venue_id},publication_year:{year}"
            )
            ...
            while cursor:
                ...
                if response.status_code == 200:
                    ...
                    page_count += 1
                    print(
                        f"  Page {page_count}: {len(results)} papers (total so far: {len(all_papers)})"
                    )
...
def bulk_collect_by_institution(
    institution_id: str, years: List[int], email: str = None
):
    """
    ...
    # Construct filter
    filter_str = (
        f"authorships.institutions.id:{institution_id},publication_year:{year_range}"
    )
