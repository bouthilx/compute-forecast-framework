...
def test_configuration():
    ...
    try:
        ...
        if not items:
            ...
            print("   This means the service account hasn't been shared any folders yet")
        else:
    ...
    if folder_id:
        ...
        try:
            ...
            if not has_access:
                print(f"   ❌ Service account does NOT have access to this folder!")
                print(f"   Please share the folder with: {service_account_email}")
                
        except HttpError as e:
            if e.resp.status == 404:
                print(f"   ❌ Folder not found or not accessible!")
    ...
    try:
        ...
        print(f"   You can use this folder ID in your configuration")
        ...
        print(f"   This is normal if using restricted scopes")
    ...
    if folder_id and not has_access:
        ...
        print(f"6. Wait a few seconds and run this test again")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--folder-id",
        help="Test access to a specific folder ID (overrides .env)"
    )
