...
def categorize_venue_type(venue, venue_type_hint=None):
    ...
    if any(
        kw in venue_lower
        for kw in ["conference", "proceedings", "international", "acm", "ieee"]
    ):
        return "conference"
    ...
    if any(
        kw in venue_lower
        for kw in ["journal", "transactions", "letters", "review", "quarterly"]
    ):
        return "journal"
...
def generate_venue_analysis_report(venue_stats):
    ...
    top_venues = sorted(
        venue_stats.items(), key=lambda x: x[1]["total_papers"], reverse=True
    )[:20]
