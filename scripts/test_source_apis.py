...
class SemanticScholarAPI:
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        self.fields = "paperId,title,abstract,authors,venue,year,citationCount,fieldsOfStudy,url"
    ...
    def search_by_title(self, title: str) -> Optional[Dict]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logging.error(f"S2 title search failed: {response.status_code} - {response.text}")
...
def main():
    ...
    print(f"  Title search: {s2_result.papers_found_by_title}/{s2_result.total_papers_tested} found")
    print(f"  DOI lookup: {s2_result.papers_found_by_doi}/{s2_result.total_papers_tested} found")
    print(f"  ArXiv lookup: {s2_result.papers_found_by_arxiv}/{s2_result.total_papers_tested} found")
    print(f"  Avg response times: Title={s2_result.avg_title_search_time:.3f}s, DOI={s2_result.avg_doi_search_time:.3f}s, ArXiv={s2_result.avg_arxiv_search_time:.3f}s\n")
    ...
    print(f"  Title search: {oa_result.papers_found_by_title}/{oa_result.total_papers_tested} found")
    print(f"  DOI lookup: {oa_result.papers_found_by_doi}/{oa_result.total_papers_tested} found")
    print(f"  Avg response times: Title={oa_result.avg_title_search_time:.3f}s, DOI={oa_result.avg_doi_search_time:.3f}s\n")
    ...
    print(f"  Title search: {cr_result.papers_found_by_title}/{cr_result.total_papers_tested} found")
    print(f"  DOI lookup: {cr_result.papers_found_by_doi}/{cr_result.total_papers_tested} found")
    print(f"  Avg response times: Title={cr_result.avg_title_search_time:.3f}s, DOI={cr_result.avg_doi_search_time:.3f}s\n")
    
    # Summary table
    print("\n=== COVERAGE SUMMARY ===")
    print(f"{'Source':<20} {'Title':<10} {'DOI':<10} {'ArXiv':<10} {'Abstract':<10} {'Citations':<10} {'Venue':<10} {'Affiliations':<15}")
    print("-" * 105)
    
    for r in results:
        print(f"{r.source_name:<20} "
    ...
    print(f"{'Source':<20} {'Title Search':<15} {'DOI Lookup':<15} {'ArXiv Lookup':<15}")
    print("-" * 65)
    
    for r in results:
        arxiv_str = f"{r.avg_arxiv_search_time:.3f}" if r.avg_arxiv_search_time > 0 else "N/A"
        print(f"{r.source_name:<20} {r.avg_title_search_time:<15.3f} {r.avg_doi_search_time:<15.3f} {arxiv_str:<15}")
    ...
    print("\n=== ZETA ALPHA ===")
    print("Zeta Alpha is a commercial service focused on AI/ML literature search and discovery.")
    print("Key features:")
    ...
    print("  - Not suitable for bulk metadata collection due to commercial restrictions")
    print("  - Better suited as a discovery/search tool rather than a consolidation source")
