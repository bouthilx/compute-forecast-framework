...
class MilaExtractionPipeline:
    ...
    def extract_from_paper(self, paper: Dict) -> Dict:
        ...
        if suppression_dict:
            suppression_score = SuppressionTemplates.calculate_suppression_score(
                suppression
            )
        else:
    ...
    def _extract_computational_requirements(self, text: str) -> Dict:
        ...
        if time_info:
            ...
            requirements["training_time_hours"] = self._convert_time_to_hours(
                value, unit
            )
        ...
        if param_info:
            ...
            requirements["parameters_millions"] = self._convert_params_to_millions(
                value, unit
            )
...
def main():
    ...
    parser.add_argument(
        "--limit", "-l", type=int, help="Limit number of papers to process"
    )
            ...
            if results:
                ...
                for result in results:
                    row = {
                        ...
                        "gpu_type": result["computational_requirements"].get(
                            "gpu_type"
                        ),
                        "gpu_count": result["computational_requirements"].get(
                            "gpu_count"
                        ),
                        "training_hours": result["computational_requirements"].get(
                            "training_time_hours"
                        ),
                        "parameters_millions": result["computational_requirements"].get(
                            "parameters_millions"
                        ),
                        "estimated_gpu_hours": result["computational_requirements"].get(
                            "estimated_gpu_hours"
                        ),
                        "confidence": result["extraction_metadata"]["confidence"],
                        "suppression_score": result.get("suppression_score"),
                        "num_ablations": result["suppression_indicators"]
                        .get("experimental_scope", {})
                        .get("num_ablations"),
                        "num_seeds": result["suppression_indicators"]
                        .get("experimental_scope", {})
                        .get("num_seeds"),
                        "constraints_mentioned": result["suppression_indicators"]
                        .get("explicit_constraints", {})
                        .get("mentions_constraints"),
                    }
                    writer.writerow(row)
