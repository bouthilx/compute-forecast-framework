...
def main():
    ...
    for year in [2023, 2024]:
        ...
        try:
            year_papers = domain_collector.collect_domain_year_papers(
                test_domain, year, target_count=4
            )
    ...
    for i, paper in enumerate(all_papers[:3], 1):
        ...
        print(
            f"     Citations: {paper.get('citations', 0)}, Source: {paper.get('source', 'unknown')}"
        )
    ...
    print("📁 Proof of concept files created:")
    ...
    for paper in all_papers:
        ...
        collection_stats["papers_by_source"][source] = (
            collection_stats["papers_by_source"].get(source, 0) + 1
        )
