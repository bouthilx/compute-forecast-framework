...
class GoogleDriveStore:
    ...
    def upload_pdf(
        ...
        media = MediaFileUpload(
            str(pdf_path), mimetype=self.PDF_MIME_TYPE, resumable=True
        )
    ...
    def batch_upload(
        self, files: Dict[str, Path], show_progress: bool = True
    ) -> Dict[str, str]:
    ...
    def upload_file(self, file_path: Path, filename: str, metadata: Optional[Dict] = None) -> Optional[str]:
        ...
        try:
            ...
            try:
                ...
                if folder.get("mimeType") != "application/vnd.google-apps.folder":
                    logger.error(
                        f"ID {self.folder_id} is not a folder, it's a {folder.get('mimeType')}"
                    )
                    return False

                logger.info(
                    f"Successfully connected to Google Drive folder: {folder.get('name')}"
                )
                return True
                ...
                if e.resp.status == 404:
                    ...
                    logger.error(
                        "3. The service account has at least 'Viewer' permission"
                    )
                else:
            ...
            logger.error(
                "Please ensure the Google Drive API is enabled in your project"
            )
            return False
        except Exception as e:
