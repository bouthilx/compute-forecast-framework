...
class AlertSuppressionManager:
    ...
    def should_suppress_alert(self, alert: Alert) -> bool:
        ...
        with self._lock:
            ...
            if self.global_suppression:
                logger.debug(
                    f"Alert {alert.alert_id} suppressed: global suppression enabled"
                )
            ...
            for rule_id, rule in self.suppression_rules.items():
                ...
                if self._should_suppress_by_rule(alert, rule):
                    logger.debug(
                        f"Alert {alert.alert_id} suppressed by rule: {rule_id}"
                    )
                    self._suppression_stats["suppressed_alerts"] += 1
    ...
    def suppress_pattern(self, pattern: str, duration_minutes: int = 30) -> None:
        ...
        with self._lock:
            ...
            logger.info(
                f"Manually suppressed pattern '{pattern}' for {duration_minutes} minutes"
            )
    ...
    def get_suppression_status(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            return {
                ...
                "active_rules": len(
                    [r for r in self.suppression_rules.values() if r.enabled]
                ),
    ...
    def _load_built_in_rules(self) -> None:
        ...
        logger.info(
            f"Loaded {len(BUILT_IN_SUPPRESSION_RULES)} built-in suppression rules"
        )
    ...
    def _should_suppress_by_rule(self, alert: Alert, rule: SuppressionRule) -> bool:
        ...
        try:
            ...
            if (
                ...
                and rule.escalation_override
            ):
                pass  # Allow non-critical alerts to be evaluated
            elif self._severity_exceeds_threshold(
                alert.severity, rule.severity_threshold
            ):
    ...
    def _rule_matches_alert(self, alert: Alert, rule: SuppressionRule) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Invalid regex pattern in suppression rule {rule.rule_id}: {e}"
            )
            return False

    def _severity_exceeds_threshold(
        ...
        return severity_levels.get(alert_severity, 0) > severity_levels.get(
            threshold, 0
        )
    ...
    def _exceeds_rate_limit(self, alert: Alert, rule: SuppressionRule) -> bool:
        ...
        recent_alerts = [
            timestamp
            for timestamp in self.alert_counts[rule_key]
    ...
    def _exceeds_burst_threshold(self, alert: Alert, rule: SuppressionRule) -> bool:
        ...
        burst_window_start = datetime.now() - timedelta(
            seconds=rule.burst_window_seconds
        )
        rule_key = f"{rule.rule_id}:{alert.rule_id}"
        ...
        recent_burst = [
            timestamp
            for timestamp in self.burst_tracking[rule_key]
    ...
    def _cleanup_expired_patterns(self) -> None:
        ...
        expired_patterns = [
            pattern
            for pattern, expiry in self.suppressed_patterns.items()
        ...
        with self._lock:
            ...
            return {
                ...
                "recommendations": self._generate_rule_recommendations(
                    pattern_frequency, burst_candidates
                ),
            }

    def track_rule_effectiveness(
        self, rule_id: str, suppressed_count: int, total_evaluated: int
    ) -> None:
