...
from .dashboard_metrics import SystemMetrics, DashboardStatus
...
class CollectionDashboard:
    ...
    def start_dashboard(self, metrics_collector: MetricsCollector) -> None:
        ...
        with self._lock:
            ...
            self._broadcast_thread = threading.Thread(
                target=self._broadcast_loop, name="DashboardBroadcast", daemon=True
            )
    ...
    def broadcast_metrics(self, metrics: SystemMetrics) -> None:
        ...
        try:
            ...
            if broadcast_time > 100:
                logger.warning(
                    f"Metrics broadcast took {broadcast_time:.1f}ms (>100ms limit)"
                )
    ...
    def _setup_routes(self):
        ...
        def get_current_metrics():
            ...
            try:
                ...
                current_metrics = (
                    self.metrics_collector.metrics_buffer.get_current_metrics()
                )
        ...
        def get_dashboard_status():
            ...
            with self._lock:
                uptime = (
                    datetime.now() - self.status.server_start_time
                ).total_seconds()
                ...
                return jsonify(
                    {
                        ...
                        "last_broadcast": self.status.last_broadcast_time.isoformat()
                        if self.status.last_broadcast_time
                        else None,
                    }
                )
        ...
        def get_metrics_history():
            ...
            try:
                ...
                return jsonify(
                    {"history": formatted_history, "count": len(formatted_history)}
                )
    ...
    def _setup_socketio_events(self):
        ...
        def handle_connect():
            ...
            logger.info(
                f"Client connected. Total clients: {self.status.connected_clients}"
            )
            ...
            if self.metrics_collector:
                ...
                if current_metrics:
                    emit(
                        ...
                        {
                            "timestamp": current_metrics.timestamp.isoformat(),
                            "system_metrics": self._format_metrics_for_frontend(
                                current_metrics
                            ),
                        },
                    )
        ...
        def handle_disconnect():
            ...
            with self._lock:
                self.status.connected_clients = max(
                    0, self.status.connected_clients - 1
                )

            logger.info(
                f"Client disconnected. Total clients: {self.status.connected_clients}"
            )
        ...
        return {
            "collection_progress": {
                ...
                "papers_per_minute": round(
                    metrics.collection_progress.papers_per_minute, 2
                ),
                "completion_percentage": round(
                    metrics.collection_progress.completion_percentage, 2
                ),
                "estimated_remaining_minutes": round(
                    metrics.collection_progress.estimated_remaining_minutes, 1
                ),
            },
            "api_health": {
                api_name: {
