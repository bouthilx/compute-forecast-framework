...
class AlertRuleFactory:
    ...
    def _condition_api_down(metrics: Dict[str, Any]) -> bool:
        ...
        for api_name, api_data in api_metrics.items():
            if (
                hasattr(api_data, "health_status")
                and api_data.health_status == "unhealthy"
            ):
                return True
    ...
    def _condition_api_response_time_high(metrics: Dict[str, Any]) -> bool:
        ...
        for api_name, api_data in api_metrics.items():
            if (
                hasattr(api_data, "avg_response_time_ms")
                and api_data.avg_response_time_ms > 5000
            ):
                return True
        return False
    ...
    def _condition_collection_stalled(metrics: Dict[str, Any]) -> bool:
        ...
        if (
            hasattr(collection_progress, "papers_per_minute")
            and collection_progress.papers_per_minute == 0
            and hasattr(collection_progress, "session_duration_minutes")
            and collection_progress.session_duration_minutes > 30
        ):
            return True
        return False
    ...
    def _condition_collection_rate_low(metrics: Dict[str, Any]) -> bool:
        ...
        if (
            hasattr(collection_progress, "papers_per_minute")
            and collection_progress.papers_per_minute < 20
            and hasattr(collection_progress, "session_duration_minutes")
            and collection_progress.session_duration_minutes > 5
        ):  # Give it some time to ramp up
            return True
        return False
    ...
    def _condition_venue_completed(metrics: Dict[str, Any]) -> bool:
        ...
        for venue_key, venue_data in venue_progress.items():
            if (
                hasattr(venue_data, "status")
                and venue_data.status == "completed"
                and hasattr(venue_data, "last_activity")
            ):
                # Check if completed recently (within last 2 minutes)
                if venue_data.last_activity:
                    age_minutes = (
                        datetime.now() - venue_data.last_activity
                    ).total_seconds() / 60
                    if age_minutes <= 2:
                        return True
        return False
    ...
    def _condition_disk_space_low(metrics: Dict[str, Any]) -> bool:
        ...
        return (
            hasattr(system_metrics, "disk_free_mb")
            and system_metrics.disk_free_mb < 1024
        )  # Less than 1GB
    ...
    def _condition_processing_errors_high(metrics: Dict[str, Any]) -> bool:
        ...
        if hasattr(processing_metrics, "processing_errors") and hasattr(
    ...
    def _condition_state_validation_errors(metrics: Dict[str, Any]) -> bool:
        ...
        return (
            hasattr(state_metrics, "state_validation_errors")
            and state_metrics.state_validation_errors > 0
        )
    ...
    def _condition_checkpoint_size_large(metrics: Dict[str, Any]) -> bool:
        ...
        return (
            hasattr(state_metrics, "checkpoint_size_mb")
            and state_metrics.checkpoint_size_mb > 100
        )
