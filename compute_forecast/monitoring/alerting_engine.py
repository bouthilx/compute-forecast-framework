...
class EscalationRule:
    ...
    def should_escalate(self, alert: Alert) -> bool:
        ...
        if alert.last_escalation is None:
            ...
            time_since_last = (datetime.now() - alert.last_escalation).total_seconds() / 60
...
class AlertingEngine:
            ...
            if channel:
                try:
                    ...
                    if success:
                        ...
                        logger.warning(f"Failed to send alert to {channel_name}: {alert.title}")
                except Exception as e:
    ...
    def _escalate_alert(self, alert: Alert, rule: AlertRule) -> None:
        ...
        logger.warning(f"Alert escalated: {alert.title} (escalation #{alert.escalation_count})")
    ...
    def _format_alert_message(self, rule: AlertRule, system_metrics: Dict[str, Any]) -> str:
        ...
        if 'api_metrics' in system_metrics:
            ...
            for api_name, metrics in system_metrics['api_metrics'].items():
                if hasattr(metrics, 'success_rate') and metrics.success_rate < 0.8:
                    api_info.append(f"{api_name}: {metrics.success_rate:.1%} success rate")
        ...
        if 'system_metrics' in system_metrics:
            ...
            if hasattr(sys_metrics, 'memory_usage_percent') and sys_metrics.memory_usage_percent > 80:
                base_message += f"\nMemory usage: {sys_metrics.memory_usage_percent:.1f}%"
            if hasattr(sys_metrics, 'cpu_usage_percent') and sys_metrics.cpu_usage_percent > 90:
