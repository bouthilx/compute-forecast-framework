...
class SimpleCheckpointManager:
    ...
    def load_latest_checkpoint(self, session_id: str) -> Optional[CheckpointData]:
        ...
        latest_checkpoint = max(
            session_checkpoints.values(), key=lambda cp: cp.timestamp
        )
...
class StateManager:
    ...
    def create_session(
        ...
        checkpoint_id = self.checkpoint_manager.create_checkpoint(
            ...
            venues_in_progress=[],
            venues_not_started=[
                (v.venue_name, year) for v in target_venues for year in target_years
            ],
            papers_collected=0,
            papers_by_venue={},
    ...
    def save_checkpoint(self, session_id: str, checkpoint_data: CheckpointData) -> str:
        ...
        if not checkpoint_data.validate_integrity():
            raise ValueError("Checkpoint data integrity validation failed")
        ...
        try:
            ...
            if checkpoint is not None:
                ...
                if not checkpoint.validate_integrity():
                    logger.warning(
                        f"Latest checkpoint for session {session_id} failed integrity check"
                    )
                    return None
            ...
            if duration > 5.0:
                logger.warning(
                    f"Checkpoint load took {duration:.3f}s (>5s requirement)"
                )
            else:
            ...
            logger.error(
                f"Failed to load latest checkpoint for session {session_id}: {e}"
            )
    ...
    def get_recovery_plan(self, session_id: str) -> RecoveryPlan:
        ...
        recovery_plan = RecoveryPlan(
            ...
            optimal_checkpoint_id=latest_checkpoint.checkpoint_id
            if latest_checkpoint
            else None,
            ...
            venues_to_restart=[],
            venues_to_validate=[],
            checkpoints_to_restore=[latest_checkpoint.checkpoint_id]
            if latest_checkpoint
            else [],
            ...
            if latest_checkpoint
            else ["Medium risk - no checkpoints"],
        )

        return recovery_plan

    def resume_session(
        self, session_id: str, recovery_plan: RecoveryPlan
    ) -> SessionResumeResult:
        """
        ...
        try:
            ...
            result.state_consistency_validated = all(
                v.passed for v in validation_results
            )

            # Update result
            result.success = len(result.resume_errors) == 0
            result.ready_for_continuation = (
                result.success and result.state_consistency_validated
            )
            ...
            if result.recovery_duration_seconds > 300.0:  # 5 minutes = 300 seconds
                logger.warning(
                    f"Session recovery took {result.recovery_duration_seconds:.1f}s (>300s requirement)"
                )
                result.resume_warnings.append("Recovery exceeded 5-minute requirement")
            else:
                logger.info(
                    f"Session {session_id} recovered in {result.recovery_duration_seconds:.1f}s"
                )
            ...
            if result.recovery_duration_seconds > 300.0:
                result.resume_warnings.append(
                    "Recovery attempt exceeded 5-minute requirement"
                )
    ...
    def _validate_session_state(
        ...
        all_venues = set(
            session.venues_completed
            + session.venues_in_progress
            + session.venues_failed
        )
        expected_venues = set(
            (v.venue_name, year)
            for v in session.target_venues
            for year in session.target_years
        )
        ...
        results.append(
            ValidationResult(
                ...
                details="All venues match configuration"
                if venue_consistency
                else "Venue mismatch detected",
                recommendations=[]
                if venue_consistency
                else ["Review venue configuration"],
            )
        )
        ...
        papers_sum = sum(
            ...
            for year_counts in session.papers_by_venue.values()
        )
        papers_consistency = abs(papers_sum - session.total_papers_collected) <= (
            session.total_papers_collected * 0.1
        )
        results.append(
            ValidationResult(
                ...
                recommendations=[]
                if papers_consistency
                else ["Recalculate paper counts"],
            )
        )
        return results
    ...
    def _cleanup_old_checkpoints(self, session_id: str):
        ...
        checkpoint_files = sorted(
            checkpoint_dir.glob("*.json"), key=lambda f: f.stat().st_mtime
        )

        # Calculate how many to delete
        to_delete = (
            total_checkpoints - self.max_checkpoints_per_session + 10
        )  # Keep buffer of 10
        ...
        if to_delete > 0 and len(checkpoint_files) > to_delete:
            for i in range(to_delete):
                try:
                    ...
                    logger.warning(
                        f"Failed to delete checkpoint {checkpoint_files[i]}: {e}"
                    )
...
def create_session_flexible(self, *args, **kwargs):
    ...
    if len(args) >= 3 or (
        "target_venues" in kwargs
        and "target_years" in kwargs
        and "collection_config" in kwargs
    ):
        ...
        logger.warning(
            "Using legacy create_session interface. "
