...
class DomainCollector:
    ...
    def execute_domain_collection(
        self, target_per_domain_year: int = 8
    ) -> Dict[str, Any]:
        """Execute collection for each domain and year combination"""
        logger.info(
            f"Starting domain collection with target {target_per_domain_year} papers per domain/year"
        )
        ...
        for domain_name in domains:
            ...
            for year in range(2019, 2025):
                ...
                try:
                    ...
                    self.collection_results["collection_stats"][domain_name][year] = (
                        len(enriched_papers)
                    )
                    ...
                    logger.info(
                        f"  ✓ Collected {len(enriched_papers)} papers for {domain_name} {year}"
                    )

                except Exception as e:
                    logger.error(
                        f"  ✗ Failed to collect papers for {domain_name} {year}: {e}"
                    )
    ...
    def collect_domain_year_papers(
        ...
        if (
            hasattr(self.executor, "collection_strategy_optimizer")
            and self.executor.collection_strategy_optimizer
        ):
            try:
                venue_papers = self.collect_from_domain_venues(
                    domain_name, year, target_count
                )
        ...
        try:
            keyword_papers = self.collect_from_major_venues_with_keywords(
                domain_name, year, target_count
            )
            collected_papers.extend(keyword_papers)
            logger.debug(
                f"    Major venues with keywords: {len(keyword_papers)} papers"
            )
        except Exception as e:
        ...
        if len(collected_papers) < target_count:
            try:
                direct_papers = self.collect_from_direct_keyword_search(
                    domain_name, year, target_count
                )
    ...
    def collect_from_domain_venues(
        self, domain_name: str, year: int, target_count: int
    ) -> List[Dict[str, Any]]:
        ...
        try:
            strategy = self.executor.collection_strategy_optimizer.generate_collection_strategy(
                domain_name
            )
            primary_venues = [
                venue.venue_name for venue in strategy.primary_venues[:3]
            ]  # Top 3 venues per domain

            for venue in primary_venues:
                try:
                    citation_threshold = self.executor.get_citation_threshold(year)
                    venue_papers = (
                        self.executor.paper_collector.collect_from_venue_year(
                            venue,
                            year,
                            citation_threshold,
                            working_apis=getattr(self.executor, "working_apis", None),
                        )
                    )
            ...
            logger.warning(
                f"Failed to generate collection strategy for {domain_name}: {e}"
            )

        return papers

    def collect_from_major_venues_with_keywords(
        self, domain_name: str, year: int, target_count: int
    ) -> List[Dict[str, Any]]:
        ...
        for venue in major_venues:
            try:
                keyword_papers = (
                    self.executor.paper_collector.collect_from_venue_year_with_keywords(
                        venue,
                        year,
                        domain_keywords[:5],
                        domain_name,
    ...
    def collect_from_direct_keyword_search(
        self, domain_name: str, year: int, target_count: int
    ) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(
                f"    Direct keyword search failed for {domain_name} {year}: {e}"
            )
    ...
    def enrich_papers_with_analysis(
        self, papers: List[Dict[str, Any]], domain_name: str, year: int
    ) -> List[Dict[str, Any]]:
        ...
        for paper in papers:
            ...
            try:
                computational_analysis = (
                    self.executor.computational_analyzer.analyze_paper_content(paper)
                )
                paper["computational_analysis"] = computational_analysis
            except Exception as e:
                logger.warning(
                    f"Computational analysis failed for paper '{paper.get('title', 'Unknown')}': {e}"
                )
            ...
            try:
                ...
                venue_score = (
                    self.executor.venue_classifier.get_venue_computational_score(venue)
                )
