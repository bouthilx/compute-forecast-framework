...
class VenueCollectionEngine:
    ...
    def collect_venue_batch(
        self, venues: List[str], year: int, working_apis: Optional[List[str]] = None
    ) -> BatchCollectionResult:
        """
        ...
        if len(venues) > self.config.max_venues_per_batch:
            logger.warning(
                f"Batch size {len(venues)} exceeds max {self.config.max_venues_per_batch}, splitting"
            )
            ...
            try:
                ...
                if not self.rate_limiter.can_make_request(api_name, len(venues)):
                    ...
                    if wait_time > 60:  # Don't wait more than 1 minute
                        logger.warning(
                            f"API {api_name} requires {wait_time}s wait, skipping"
                        )
                        continue
        ...
        logger.info(
            f"Batch collection completed: {len(result.papers)} papers in {result.total_duration_seconds:.2f}s"
        )
        return result

    def collect_single_venue(
        self, venue: str, year: int, working_apis: Optional[List[str]] = None
    ) -> VenueCollectionResult:
        """
        ...
        for api_name in available_apis:
            try:
                if self._check_timeout(
                    start_time, self.config.single_venue_timeout_seconds
                ):
                    logger.warning("Single venue collection timeout reached")
                    break

                # Perform paginated collection for this venue
                venue_papers, metadata = self._collect_venue_with_pagination(
                    api_name, venue, year
                )
                ...
                if venue_papers:
                    ...
                    result.success = True
                    logger.info(
                        f"Single venue collection successful via {api_name}: {len(venue_papers)} papers"
                    )
                    break
                ...
                result.errors.append(error)
                logger.error(
                    f"Exception during single venue collection with {api_name}: {e}"
                )
        ...
        logger.info(
            f"Single venue collection completed: {len(result.papers)} papers in {result.total_duration_seconds:.2f}s"
        )
        return result
    ...
    def estimate_collection_time(
        self, venues: List[str], years: List[int]
    ) -> CollectionEstimate:
        """
        ...
        if len(years) > 1:
            # We can batch multiple venue-year combinations in a single API call
            # e.g., (venue1 OR venue2) AND (year:2022 OR year:2023)
        ...
        estimated_api_calls = total_batches
    ...
    def _collect_large_batch(
        self, venues: List[str], year: int, working_apis: Optional[List[str]]
    ) -> BatchCollectionResult:
    ...
    def _perform_batch_collection(
        self, api_name: str, venues: List[str], year: int
    ) -> APIResponse:
        ...
        try:
            ...
            response_time_ms = (time.time() - start_time) * 1000
            error = APIError(
                error_type="api_request_error", message=str(e), timestamp=datetime.now()
            )
    ...
    def _collect_venue_with_pagination(
        self, api_name: str, venue: str, year: int
    ) -> tuple[List[Paper], ResponseMetadata]:
        ...
        while True:
            ...
            try:
                ...
                response = client.search_papers(
                    f'venue:"{venue}" year:{year}', year, limit=limit, offset=offset
                )
                ...
                self.rate_limiter.record_request(
                    api_name, response.success, int(response_time)
                )
                ...
                if len(all_papers) > 10000:  # Max 10k papers per venue
                    logger.warning(
                        f"Venue {venue} exceeded 10k papers, stopping pagination"
                    )
                    break
    ...
    def _get_api_client(self, api_name: str):
        ...
        if api_name not in self._api_clients:
            if api_name == "semantic_scholar":
                from ..sources.enhanced_semantic_scholar import (
    ...
    def _check_timeout(
        self, start_time: datetime, timeout_seconds: Optional[int] = None
    ) -> bool:
    ...
    def _estimate_average_delay(self) -> float:
        ...
        degraded_apis = [
            s for s in api_statuses.values() if s.status in ["degraded", "critical"]
        ]
        if degraded_apis:
