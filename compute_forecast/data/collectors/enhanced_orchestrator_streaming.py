...
class EnhancedCollectionOrchestratorStreaming:
    ...
    def stream_papers_from_all_sources(
        ...
        for source_name in active_sources:
            try:
                ...
                for paper_batch in self._stream_from_source_paginated(source_name, query):
    ...
    async def async_stream_papers_from_all_sources(
        ...
        async def stream_from_source(source_name: str):
            ...
            try:
                async for paper_batch in self._async_stream_from_source(source_name, query):
                    for paper in paper_batch:
                        if process_callback:
                            paper = await process_callback(paper) if asyncio.iscoroutinefunction(process_callback) else process_callback(paper)
                        yield paper
            except Exception as e:
    ...
    def collect_with_memory_limit(
        ...
        for paper in self.stream_papers_from_all_sources(query, sources_to_use):
            # Update source count
            source = paper.metadata.get("source", "unknown") if hasattr(paper, "metadata") else "unknown"
            ...
            if len(current_batch) >= self.batch_size:
                try:
                    ...
                    for p in current_batch:
                        source = p.metadata.get("source", "unknown") if hasattr(p, "metadata") else "unknown"
                        source_counts[source] = source_counts.get(source, 0) + 1
