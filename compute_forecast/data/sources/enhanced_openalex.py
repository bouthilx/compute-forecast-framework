            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if attempt < self.max_retries - 1:
                        ...
                        logger.warning(
                            f"OpenAlex server error {response.status_code}, retrying in {wait_time}s"
                        )
                ...
                if attempt < self.max_retries - 1:
                    wait_time = self.retry_delay * (2**attempt)
                    logger.warning(
                        f"OpenAlex request timeout, retrying in {wait_time}s"
                    )
                    time.sleep(wait_time)
                    continue
                else:
        ...
        try:
            ...
            for item in data.get("results", []):
                try:
                    ...
                    # Extract OpenAlex ID
                    openalex_id = (
                        item.get("id", "").replace("https://openalex.org/", "")
                        if item.get("id")
                        else None
                    )
