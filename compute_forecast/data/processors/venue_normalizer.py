...
from .fuzzy_venue_matcher import FuzzyVenueMatcher
...
class VenueNormalizer:
            ...
            if fuzzy_result.matched_venue:
                ...
                if fuzzy_result.similarity_score >= 0.95:
                    ...
                    if self.update_mappings_live:
                        self._persist_new_mapping(raw_venue, fuzzy_result.matched_venue, fuzzy_result.similarity_score)
    ...
    def batch_normalize_venues(self, papers: List[Paper]) -> BatchNormalizationResult:
        ...
        self.logger.info(f"Batch normalized {len(papers)} papers in {processing_time:.2f}s")
    ...
    def update_mapping(self, raw_venue: str, normalized_venue: str, confidence: float) -> bool:
        """
        ...
        if normalized_venue not in self._canonical_venues:
            self.logger.warning(f"Normalized venue '{normalized_venue}' not in canonical venues")
            return False
    ...
    def get_mapping_statistics(self) -> VenueMappingStats:
        ...
        with self._lock:
            ...
            for tier in ["tier1", "tier2", "tier3", "tier4"]:
                tier_venues = [v for v in self._venue_configs.values() if v.venue_tier == tier]
                if tier_venues:
                    mapped_count = sum(1 for v in tier_venues if v.venue_name in self._venue_mappings)
    ...
    def _persist_new_mapping(self, raw_venue: str, normalized_venue: str, confidence: float) -> None:
        ...
        try:
            ...
            self.logger.info(f"Persisted new mapping: {raw_venue} -> {normalized_venue}")
