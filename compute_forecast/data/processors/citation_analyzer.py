...
class CitationAnalyzer:
    ...
    def analyze_citation_distributions(self, papers: List[Paper]) -> CitationAnalysisReport:
        ...
        for venue, venue_paper_list in venue_papers.items():
            venue_analysis[venue] = self._analyze_venue_citations(venue, venue_paper_list)
    ...
    def filter_papers_by_citations(self, papers: List[Paper], preserve_breakthroughs: bool = True) -> CitationFilterResult:
        """
        ...
        for paper in valid_papers:
            ...
            is_breakthrough = paper.paper_id in breakthrough_papers if paper.paper_id else False
            ...
            if is_breakthrough and preserve_breakthroughs:
                ...
                threshold = venue_year_thresholds.get((venue, year), 5)  # Default threshold
        ...
        breakthrough_preservation_rate = len(breakthrough_papers_preserved) / len(breakthrough_papers) if breakthrough_papers else 1.0
    ...
    def calculate_adaptive_threshold(self, venue: str, year: int, papers: List[Paper]) -> AdaptiveThreshold:
        """
        ...
        """
        venue_tier = self._get_venue_tier(venue)
        return self.threshold_calculator.calculate_adaptive_threshold(venue, year, papers, venue_tier)
    
    def validate_filtering_quality(self, original_papers: List[Paper], filtered_papers: List[Paper]) -> FilteringQualityReport:
        """Validate that filtering preserves important papers."""
        original_count = len(original_papers)
        filtered_count = len(filtered_papers)
        
        # Venue coverage
        original_venues = set(p.normalized_venue or p.venue for p in original_papers if p.normalized_venue or p.venue)
        preserved_venues = set(p.normalized_venue or p.venue for p in filtered_papers if p.normalized_venue or p.venue)
        ...
        breakthrough_original = self.breakthrough_detector.detect_breakthrough_papers(original_papers)
        breakthrough_ids = {bp.paper.paper_id for bp in breakthrough_original if bp.paper.paper_id}
        breakthrough_preserved = [p for p in filtered_papers if p.paper_id in breakthrough_ids]
        breakthrough_preservation_rate = len(breakthrough_preserved) / len(breakthrough_original) if breakthrough_original else 1.0
        
        # Quality indicators
        avg_citations_original = np.mean([p.citations for p in original_papers if p.citations is not None]) if original_papers else 0
        avg_citations_filtered = np.mean([p.citations for p in filtered_papers if p.citations is not None]) if filtered_papers else 0
        citation_improvement_ratio = avg_citations_filtered / avg_citations_original if avg_citations_original > 0 else 0.0
        ...
        if venue_coverage_rate < 0.8:
            warnings.append(f"Low venue coverage: {venue_coverage_rate:.1%} of venues preserved")
            recommendations.append("Consider lowering thresholds for underrepresented venues")
        
        if impact_preservation_rate < 0.9:
            warnings.append(f"High-impact paper loss: {1-impact_preservation_rate:.1%} of high-impact papers filtered out")
            recommendations.append("Review threshold settings for high-impact papers")
        
        if breakthrough_preservation_rate < 0.95:
            warnings.append(f"Breakthrough paper loss: {1-breakthrough_preservation_rate:.1%} of breakthrough papers filtered out")
            recommendations.append("Enable breakthrough preservation in filtering")
        
        if filtered_count < original_count * 0.1:
            warnings.append(f"Aggressive filtering: only {filtered_count/original_count:.1%} of papers retained")
    ...
    def _analyze_venue_citations(self, venue: str, papers: List[Paper]) -> VenueCitationStats:
        ...
        breakthrough_papers = self.breakthrough_detector.detect_breakthrough_papers(papers)
        venue_breakthrough_papers = [bp for bp in breakthrough_papers if (bp.paper.normalized_venue or bp.paper.venue) == venue]
        
        # Calculate recommended threshold
        venue_tier = self._get_venue_tier(venue)
        recent_year = max(papers_by_year.keys()) if papers_by_year else self.current_year - 1
    ...
    def _analyze_year_citations(self, year: int, papers: List[Paper]) -> YearCitationStats:
    ...
    def _calculate_quality_indicators(self, papers: List[Paper], 
                                    venue_analysis: Dict[str, VenueCitationStats]) -> Dict[str, float]:
        ...
        if citations:
            ...
            indicators["zero_citation_rate"] = sum(1 for c in citations if c == 0) / len(citations)
        
        # Venue diversity
        indicators["venue_count"] = len(venue_analysis)
        indicators["papers_per_venue"] = len(papers) / len(venue_analysis) if venue_analysis else 0
        
        # Breakthrough potential
        breakthrough_papers = self.breakthrough_detector.detect_breakthrough_papers(papers)
        indicators["breakthrough_rate"] = len(breakthrough_papers) / len(papers) if papers else 0
        indicators["avg_breakthrough_score"] = np.mean([bp.breakthrough_score for bp in breakthrough_papers]) if breakthrough_papers else 0
        
        # Time relevance
        current_year = self.current_year
        recent_papers = [p for p in papers if p.year and p.year >= current_year - 3]
        indicators["recent_paper_rate"] = len(recent_papers) / len(papers) if papers else 0
        
        return indicators
    
    def _generate_filtering_recommendations(self, venue_analysis: Dict[str, VenueCitationStats]) -> List[str]:
        """Generate specific recommendations for filtering strategy."""
        recommendations = []
        
        # Check venue representation
        low_paper_venues = [v for v, stats in venue_analysis.items() if stats.total_papers < 20]
        if low_paper_venues:
            recommendations.append(f"Consider lower thresholds for venues with few papers: {', '.join(low_paper_venues[:3])}")
        
        # Check breakthrough paper distribution
        total_breakthroughs = sum(len(stats.breakthrough_papers) for stats in venue_analysis.values())
        if total_breakthroughs < len(venue_analysis) * 2:
            recommendations.append("Enable breakthrough preservation to retain high-potential papers with lower citations")
        ...
        if high_variance_venues:
            recommendations.append(f"Use adaptive thresholds for high-variance venues: {', '.join(high_variance_venues[:3])}")
        
        # General recommendations
        recommendations.append("Consider venue-specific and year-specific thresholds for optimal filtering")
