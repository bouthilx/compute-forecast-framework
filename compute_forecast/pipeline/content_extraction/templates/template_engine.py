...
class ExtractionTemplate:
    ...
    # Validation rules
    validation_rules: Dict[ExtractionField, Dict[str, Any]] = field(
        ...
        for field_list in [self.required_fields, self.optional_fields]:
            for field_item in field_list:
                if not isinstance(field_item, ExtractionField):
                    raise TypeError(
                        f"Field must be ExtractionField enum, got {type(field_item)}"
                    )

    def calculate_completeness(
        self, extracted_fields: Dict[ExtractionField, Any]
    ) -> float:
        ...
        present_required = sum(
            1 for field in self.required_fields if field in extracted_fields
        )
...
class ExtractionTemplateEngine:
    ...
    def extract_to_template(self, paper: Paper, template_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "confidence_scores": self.calculate_field_confidence(
    ...
    def map_to_template(
        self, analysis: Dict[str, Any], template: ExtractionTemplate
    ) -> Dict[ExtractionField, Any]:
    ...
    def validate_extraction(
        self, extracted: Dict[ExtractionField, Any], template: ExtractionTemplate
    ) -> Dict[str, Any]:
    ...
    def calculate_field_confidence(
        self, extracted: Dict[ExtractionField, Any], raw_analysis: Dict[str, Any]
    ) -> Dict[ExtractionField, float]:
