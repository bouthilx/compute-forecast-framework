...
class CrossValidationFramework:
    ...
    def select_validation_sample(
        self, papers: List[Paper], stratify_by: str = "year"
    ) -> List[Paper]:
        """
    ...
    def compare_extractions(
        self, manual: Dict[str, Any], automated: Dict[str, Any]
    ) -> Dict[str, float]:
        """
    ...
    def generate_calibration_model(self, comparisons: List[Dict[str, Any]]) -> Any:
        """
        ...
        for field, data in field_data.items():
            ...
            manual_vals = [
                d["manual"] for d in data if isinstance(d["manual"], (int, float))
            ]
            auto_vals = [
                d["automated"] for d in data if isinstance(d["automated"], (int, float))
            ]
            ...
            covariance = sum(
                (m - manual_mean) * (a - auto_mean)
                for m, a in zip(manual_vals, auto_vals)
            ) / len(manual_vals)
            auto_variance = sum((a - auto_mean) ** 2 for a in auto_vals) / len(
                auto_vals
            )
    ...
    def apply_calibration(
        self, field: str, automated_value: float
    ) -> Tuple[float, bool]:
        """
    ...
    def validate_extraction_quality(
        self, manual_sample: Dict[str, Any], automated_full: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        for paper_id in sample_ids:
            if paper_id in manual_sample and paper_id in automated_sample:
                comparison = {}
                for field in set(manual_sample[paper_id].keys()) & set(
                    automated_sample[paper_id].keys()
                ):
                    comparison[field] = [
                        {
    ...
    def _calculate_field_agreement(
        self, field: str, manual_val: Any, auto_val: Any
    ) -> Tuple[bool, float]:
        """
    ...
    def _stratified_sample_by_domain(self, papers: List[Paper]) -> List[Paper]:
        ...
        for paper in papers:
            ...
            if hasattr(paper, "title") and paper.title:
                ...
                if any(
                    term in title_lower
                    for term in ["language", "nlp", "text", "transformer"]
                ):
                    domain = "nlp"
                elif any(
                    term in title_lower for term in ["image", "vision", "visual", "cnn"]
                ):
                    domain = "cv"
                elif any(
                    term in title_lower
                    for term in ["reinforcement", "rl", "agent", "reward"]
                ):
                    domain = "rl"
    ...
    def _generate_recommendations(
        self, field_accuracies: Dict[str, float], problematic_fields: List[str]
    ) -> List[str]:
        ...
        if overall_accuracy >= 0.9:
            recommendations.append(
                "Extraction quality is excellent. Continue with current approach."
            )
        elif overall_accuracy >= 0.8:
            recommendations.append(
                "Extraction quality is good. Minor improvements recommended."
            )
        else:
            recommendations.append(
                "Extraction quality needs improvement. Consider manual review."
            )
        ...
        for field in problematic_fields:
            ...
            if accuracy < 0.5:
                recommendations.append(
                    f"Critical: {field} extraction is unreliable (accuracy: {accuracy:.2f})"
                )
            elif accuracy < 0.8:
                recommendations.append(
                    f"Warning: {field} extraction needs improvement (accuracy: {accuracy:.2f})"
                )

        # Calibration recommendations
        if any(acc < 0.9 for acc in field_accuracies.values()):
            recommendations.append(
                "Consider applying calibration models to improve accuracy"
            )
