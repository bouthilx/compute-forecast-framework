...
from compute_forecast.pipeline.content_extraction.parser.core.cost_tracker import (
    CostTracker,
)
...
class OptimizedPDFProcessor:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.extractors: Dict[
            str, Dict[str, Any]
        ] = {}  # Will store {name: {'extractor': BaseExtractor, 'level': int}}
        self.config = config
        self.validator = AffiliationValidator()
        self.cost_tracker = CostTracker()

    def register_extractor(
        self, name: str, extractor: BaseExtractor, level: int
    ) -> None:
        ...
        """
        self.extractors[name] = {"extractor": extractor, "level": level}
            ...
            try:
                ...
                if self.validator.validate_affiliations(result, metadata):
                    ...
                    extraction_cost = self._calculate_extraction_cost(
                        name, "affiliation_extraction", result
                    )
                    self.cost_tracker.record_extraction_cost(
                        name, "affiliation_extraction", extraction_cost
                    )
    ...
    def _extract_full_text(self, pdf_path: Path) -> str:
        ...
        for level, name, extractor in available_extractors:
            try:
                ...
                extraction_cost = self._calculate_extraction_cost(
                    name, "full_text", {"text": full_text}
                )
                self.cost_tracker.record_extraction_cost(
                    name, "full_text", extraction_cost
                )
    ...
    def _extract_computational_specs(self, full_text: str) -> Dict:
        ...
        found_keywords = [
            kw for kw in computational_keywords if kw.lower() in full_text.lower()
        ]
    ...
    def _calculate_extraction_cost(
        self, extractor_name: str, operation: str, result: Dict
    ) -> float:
