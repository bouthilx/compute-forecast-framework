...
class PyMuPDFExtractor(BaseExtractor):
        ...
        try:
            ...
            for page_num in pages:
                if page_num < len(doc):
                    ...
                    logger.warning(
                        f"Page {page_num} out of range for PDF with {len(doc)} pages"
                    )

            full_text = "\n".join(text_parts)
            confidence = self._calculate_confidence(full_text)

            return {"text": full_text, "method": "pymupdf", "confidence": confidence}

        except Exception as e:
            logger.error(
                f"PyMuPDF extraction failed for {pdf_path} (pages: {pages}): {type(e).__name__}: {str(e)}"
            )
            raise
        finally:
            if doc:
    ...
    def extract_full_text(self, pdf_path: Path) -> str:
        ...
        try:
            ...
            logger.error(
                f"PyMuPDF full text extraction failed for {pdf_path}: {type(e).__name__}: {str(e)}"
            )
            raise
        finally:
            if doc:
    ...
    def _calculate_confidence(self, text: str) -> float:
        ...
        marker_bonus = min(
            marker_count * self.ACADEMIC_MARKER_BONUS, self.MAX_ACADEMIC_BONUS
        )
        ...
        garbage_penalty = min(
            garbage_ratio * self.GARBAGE_PENALTY_FACTOR, self.MAX_GARBAGE_PENALTY
        )
        ...
        if words:
            ...
            short_word_penalty = min(
                short_word_ratio * self.SHORT_WORD_PENALTY_FACTOR,
                self.MAX_SHORT_WORD_PENALTY,
            )
        else:
        ...
        penalty = min(
            garbage_penalty + short_word_penalty,
            positive_score * self.MAX_PENALTY_RATIO,
        )
