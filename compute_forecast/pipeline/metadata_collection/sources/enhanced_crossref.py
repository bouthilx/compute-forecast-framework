            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if attempt < self.max_retries - 1:
                        ...
                        logger.warning(
                            f"Crossref server error {response.status_code}, retrying in {wait_time}s"
                        )
                ...
                if attempt < self.max_retries - 1:
                    wait_time = self.retry_delay * (2**attempt)
                    logger.warning(
                        f"Crossref request timeout, retrying in {wait_time}s"
                    )
                    time.sleep(wait_time)
                    continue
                else:
    ...
    def lookup_doi(self, doi: str) -> APIResponse:
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if response.status_code == 200:
                    return self._parse_doi_response(
                ...
                if attempt < self.max_retries - 1:
                    wait_time = self.retry_delay * (2**attempt)
                    logger.warning(
                        f"Crossref request error: {e}, retrying in {wait_time}s"
                    )
                    time.sleep(wait_time)
                    continue
                else:
