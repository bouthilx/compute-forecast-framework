...
class AAAIScraper(BaseScraper):
    ...
    def __init__(self, config: Optional[ScrapingConfig] = None):
        ...
        if self.config.batch_size > 50:
            self.logger.warning(
                f"Reducing batch size from {self.config.batch_size} to 50 for AAAI to avoid timeouts"
            )
        ...
        try:
            ...
            conference_months = {
                "aaai": [
                    ...
                    3,
                    4,
                ],  # AAAI is usually in Feb-March, papers appear Jan-Apr
            ...
            for from_date, until_date in date_ranges:
                ...
                use_weekly_ranges = (
                    len(errors) > 0
                )  # Switch to weekly after first error
                ...
                if use_weekly_ranges and from_date != until_date:
                    ...
                    for week_start in range(from_day, until_day + 1, 7):
                        ...
                        week_ranges.append(
                            (
                                f"{year_month}-{week_start:02d}",
                                f"{year_month}-{week_end:02d}",
                            )
                        )
                else:
                ...
                for week_from, week_until in week_ranges:
                    ...
                    while len(papers) < self.config.batch_size:
                        ...
                        self.logger.info(
                            f"Querying OAI-PMH for {venue} papers from {week_from} to {week_until}"
                        )
                        ...
                        while retry_count < max_retries:
                            try:
                                ...
                                response = self.session.get(
                                    oai_url, params=params, timeout=5
                                )
                                if response.status_code == 503:
                                    ...
                                    self.logger.warning(
                                        f"Got 503 error, retrying after {retry_after} seconds"
                                    )
                                ...
                                if retry_count < max_retries - 1:
                                    ...
                                    self.logger.warning(
                                        f"Request failed for {week_from} to {week_until}, retrying in {wait_time}s: {str(e)[:100]}"
                                    )
                                    ...
                                    self.logger.error(
                                        f"Skipping {week_from} to {week_until} after {max_retries} retries: {str(e)[:100]}"
                                    )
                                    errors.append(
                                        f"Timeout for {week_from} to {week_until}"
                                    )
                        ...
                        if "response" not in locals():
                            self.logger.error(
                                f"No response received for {week_from} to {week_until}"
                            )
                            continue
                        ...
                        if error_elem is not None:
                            ...
                            if error_code == "noRecordsMatch":
                                self.logger.info(
                                    f"No records found for {venue} from {week_from} to {week_until}"
                                )
                                break
                            else:
                                raise Exception(
                                    f"OAI-PMH error {error_code}: {error_msg}"
                                )
                        ...
                        for record in records:
                            ...
                            try:
                                paper = self._parse_oai_record(
                                    record, venue_lower, year
                                )
                                if paper:
                                    papers.append(paper)
                            except Exception as e:
    ...
    def _parse_oai_record(
        self, record: ET.Element, venue: str, year: int
        ...
        try:
            ...
            for relation in metadata.findall("dc:relation", self.namespaces):
                if relation.text and (
                    relation.text.endswith(".pdf") or "/view/" in relation.text
                ):
    ...
    def estimate_paper_count(self, venue: str, year: int) -> Optional[int]:
        ...
        try:
            ...
            self.logger.debug(
                f"Estimating papers for {venue} {year} using sample month {start_month}"
            )
            self.logger.debug(f"Request URL: {oai_url} with params: {params}")

            request_start = timer.time()
            response = self.session.get(
                oai_url, params=params, timeout=3
            )  # Very short timeout for estimation
            request_time = timer.time() - request_start
            self.logger.debug(
                f"OAI-PMH request took {request_time:.2f}s, status: {response.status_code}"
            )
            ...
            if resumption is not None:
                ...
                if complete_size:
                    try:
                        ...
                        self.logger.debug(
                            f"Got completeListSize from OAI-PMH: {sample_count}"
                        )
                    except (ValueError, TypeError):
                        pass
            ...
            self.logger.info(
                f"Estimated {estimated_total} papers for {venue} {year} (sample: {sample_count}) in {total_time:.2f}s"
            )
            ...
            total_time = timer.time() - start_time
            self.logger.debug(
                f"Failed to estimate count for {venue} {year} after {total_time:.2f}s: {e}"
            )
