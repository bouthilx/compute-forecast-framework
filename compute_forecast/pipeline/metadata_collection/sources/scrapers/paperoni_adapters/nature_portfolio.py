...
class NaturePortfolioAdapter(BasePaperoniAdapter):
    ...
    def _call_paperoni_scraper(
        self, scraper: Any, venue: str, year: int
    ) -> List[SimplePaper]:
        ...
        try:
            ...
            self.logger.info(
                f"Found {total_results} total papers, processing {len(items)}"
            )
            ...
            for item in items:
                try:
                    ...
                    self.logger.warning(
                        f"Failed to parse paper {item.get('DOI', 'unknown')}: {e}"
                    )
                    continue
            ...
            self.logger.error(
                f"Error fetching Nature Portfolio papers for {venue} {year}: {e}"
            )
            raise
    ...
    def _convert_to_simple_paper(
        self, item: dict, venue: str, year: int
