...
class OpenReviewAdapter(BasePaperoniAdapter):
    ...
    def _get_conference_submissions_v1(
        self, venue_id: str, venue_lower: str
    ) -> List[Any]:
        ...
        for invitation in invitation_formats:
            try:
                ...
                submissions = self.client_v1.get_notes(
                    invitation=invitation, details="replyCount"
                )
                if submissions:
                    self.logger.info(
                        f"Found {len(submissions)} submissions using v1 API with {invitation}"
                    )
                    break
            except Exception as e:
    ...
    def _get_tmlr_papers(self, year: int) -> List[SimplePaper]:
        ...
        try:
            ...
            submissions = list(
                self.client.get_all_notes(
                    invitation="TMLR/-/Accepted", details="directReplies"
                )
            )
