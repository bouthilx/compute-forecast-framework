...
class OpenReviewAdapter(BasePaperoniAdapter):
    ...
    def _call_paperoni_scraper(self, scraper: Any, venue: str, year: int) -> List[Any]:
        ...
        try:
            ...
            if venue_lower == "iclr" and year <= 2023:
                submissions = self._get_conference_submissions_v1(
                    venue_id, venue_lower, year
                )
            ...
            for submission in submissions:
                try:
                    ...
                    if decision:
                        ...
                        venue_info = submission.content.get("venue", {})
                        ...
                        self.logger.debug(
                            f"No decision found for {submission.id}, defaulting to poster"
                        )
                        acceptance_type = "poster"
                    else:
        ...
        self.logger.info(
            f"After filtering: {len(papers)} accepted papers from {len(submissions)} total submissions"
        )
        return papers
    ...
    def _get_conference_submissions_v1(
        self, venue_id: str, venue_lower: str, year: int
    ) -> List[Any]:
        ...
        for invitation in invitation_formats:
            try:
                ...
                if all_submissions:
                    self.logger.info(
                        f"Found {len(all_submissions)} total submissions using v1 API with {invitation}"
                    )
                    break
            except Exception as e:
        ...
        if venue_lower == "iclr" and all_submissions:
            ...
            sample_venues = [
                sub.content.get("venue", "") for sub in all_submissions[:20]
            ]
            ...
            if venues_with_data > 10:  # More than 50% have venue data
                ...
                for submission in all_submissions:
                    ...
                    if year == 2022 and venue == f"ICLR {year} Submitted":
                        ...
                        continue  # Skip submitted papers
                    elif f"ICLR {year}" in venue and any(
                        t in venue.lower()
                        for t in ["poster", "oral", "spotlight", "notable"]
                    ):
            ...
            if has_venue_info and len(venue_filtered) > 100:  # Sanity check
                ...
                for i in range(0, len(all_submissions), batch_size):
                    ...
                    self.logger.debug(
                        f"Processed {processed}/{len(all_submissions)} papers, found {len(accepted_submissions)} accepted"
                    )

                submissions = accepted_submissions
                self.logger.info(
                    f"Found {len(submissions)} accepted papers out of {len(all_submissions)} total"
                )
        else:
            # For other venues, include all submissions
            self.logger.warning(
                f"Cannot filter decisions for {venue_lower}, including all submissions"
            )
    ...
    def _get_accepted_papers_by_venueid(self, venue_id: str) -> List[Any]:
        ...
        try:
            ...
            submissions = list(self.client.get_all_notes(content={"venueid": venue_id}))
