...
class NeurIPSAdapter(BasePaperoniAdapter):
    ...
    def estimate_paper_count(self, venue: str, year: int) -> Optional[int]:
        ...
        try:
            ...
            self.logger.warning(f"Could not estimate paper count for NeurIPS {year}: {e}")
            return None
    ...
    def _call_paperoni_scraper(self, scraper: Any, venue: str, year: int) -> List[Any]:
        ...
        try:
            ...
            limit = self.config.batch_size if self.config.batch_size < 10000 else len(paper_entries)
            ...
            self.logger.info(f"Successfully extracted {len(papers)} papers from {len(paper_entries)} entries")
