...
class OpenReviewAdapterV2(BasePaperoniAdapter):
    ...
    def _venues_from_wildcard(self, pattern: str, client: Any) -> List[str]:
        ...
        if isinstance(pattern, list):
            return reduce(
                list.__add__, 
                [self._venues_from_wildcard(p, client) for p in pattern]
            )
        elif "*" not in pattern:
            return [pattern]
        else:
    ...
    def _call_paperoni_scraper(self, scraper: Any, venue: str, year: int) -> List[Any]:
        ...
        try:
            ...
            self.logger.info(f"Successfully collected {len(papers)} papers for {venue} {year}")
            ...
            self.logger.error(f"Unexpected error fetching papers for {venue} {year}: {e}")
            import traceback
    ...
    def _get_tmlr_papers(self, year: int) -> List[SimplePaper]:
        ...
        try:
            ...
            submissions = list(self.client_v2.get_all_notes(
            ...
            for submission in submissions:
                try:
                    ...
                    paper = self._convert_to_simple_paper(submission, "accepted", "TMLR", year)
                    if paper:
                        papers.append(paper)
                    ...
                    self.logger.debug(f"Failed to parse TMLR submission {getattr(submission, 'id', 'unknown')}: {e}")
                    continue
    ...
    def _get_accepted_by_venueid(self, venue_id: str, venue: str, year: int) -> List[SimplePaper]:
        ...
        try:
            ...
            for submission in submissions:
                try:
                    ...
                    self.logger.debug(f"Failed to parse submission {getattr(submission, 'id', 'unknown')}: {e}")
                    continue
    ...
    def _get_conference_submissions_v1(self, venue_id: str, venue: str, year: int) -> List[SimplePaper]:
        ...
        for invitation in invitation_formats:
            try:
                ...
                if all_submissions:
                    self.logger.info(f"Found {len(all_submissions)} submissions using {invitation}")
                    break
            except Exception as e:
        ...
        self.logger.info(f"Filtering {len(all_submissions)} submissions for accepted papers...")
        ...
        if len(venue_filtered) > 100:  # Sanity check
            ...
            self.logger.info("Venue field unreliable, extracting decisions for all papers...")
        ...
        self.logger.info(f"Found {len(papers)} accepted papers out of {len(all_submissions)} total")
        
        # Final safety check
        if len(papers) == 0 and len(all_submissions) > 100:
            self.logger.warning(f"No accepted papers found out of {len(all_submissions)} submissions - this might indicate a parsing issue")
        
        return papers
    
    def _get_conference_submissions_v2(self, venue_id: str, venue: str, year: int) -> List[SimplePaper]:
        ...
        self.logger.info(f"Processed {len(submissions)} submissions: {accepted_count} accepted, {rejected_count} rejected")
        return papers
    ...
    def _convert_to_simple_paper(self, submission: Any, decision: Optional[str], venue: str, year: int) -> Optional[SimplePaper]:
            ...
            if pdf_field:
                if isinstance(pdf_field, str):
                    if not pdf_field.startswith('http'):
                        pdf_field = f"https://openreview.net{pdf_field}" if pdf_field.startswith('/') else f"https://openreview.net/{pdf_field}"
    ...
    def _extract_decision_smart(self, submission: Any, venue_lower: str, year: int) -> Optional[str]:
        """
