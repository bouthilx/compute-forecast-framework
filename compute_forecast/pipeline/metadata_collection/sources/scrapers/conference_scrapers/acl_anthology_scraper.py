...
class ACLAnthologyScraper(ConferenceProceedingsScraper):
    ...
    def scrape_venue_year(self, venue: str, year: int) -> ScrapingResult:
        ...
        # Handle case-insensitive venue lookup
        if (
            venue not in self.venue_mappings
            and venue.upper() not in self.venue_mappings
        ):
        ...
        try:
            ...
            try:
                ...
                volume_urls = self._extract_volume_urls(
                    event_response.text, venue, year
                )
                ...
                if volume_urls:
                    for vol_name, vol_url in volume_urls.items():
                        try:
                            self.logger.info(
                                f"Scraping {venue} {year} {vol_name} from {vol_url}"
                            )
                            papers = self._scrape_volume_page(vol_url, venue, year)
                            all_papers.extend(papers)
                        except Exception as e:
                            self.logger.warning(
                                f"Failed to scrape volume {vol_name}: {e}"
                            )
                            continue
                else:
                    # No volume links found, try to parse papers directly from event page
                    all_papers = self._parse_event_page(
                        event_response.text, venue, year, event_url
                    )

            except requests.exceptions.RequestException as e:
                self.logger.warning(
                    f"Event page not found, trying volume URLs directly: {e}"
                )
            ...
            return ScrapingResult.success_result(
                ...
                metadata={"venue": venue, "year": year, "papers": all_papers},
            )
    ...
    def parse_proceedings_page(
        ...
        return self._parse_event_page(
            html, venue, year, self.get_proceedings_url(venue, year)
        )

    def _parse_event_page(
        self, html: str, venue: str, year: int, source_url: str
    ) -> List[SimplePaper]:
        ...
        if not paper_entries:
            ...
            for entry in paper_entries:
                try:
                    paper = self._extract_paper_from_entry(
                        entry, venue, year, source_url
                    )
                    if paper:
                        papers.append(paper)
                except Exception as e:
    ...
    def _extract_paper_from_entry(
        self, entry, venue: str, year: int, source_url: str
        ...
        if hasattr(entry, "name") and entry.name == "div":
            ...
            paper_link = entry.find(
                "a", href=re.compile(r"/\d{4}\.\w+-\w+\.\d+/?$|/[A-Z]\d{2}-\d+/?$")
            )
        else:
            paper_link = entry.find(
                "a", href=re.compile(r"/\d{4}\.\w+-\w+\.\d+/?$|/[A-Z]\d{2}-\d+/?$")
    ...
    def _extract_paper_from_link(
        self, link, venue: str, year: int, source_url: str
    ) -> Optional[SimplePaper]:
    ...
    def _extract_authors_from_entry(self, entry) -> List[str]:
        ...
        author_container = entry.find(
            ["span", "div"], class_=re.compile(r"(author|by)")
        )
        if author_container:
            ...
            if italic:
                ...
                for text in text_nodes:
                    ...
                    if (
                        ...
                        and text != entry.find("a").get_text(strip=True)
                        if entry.find("a")
                        else True
                    ):
                        author_text = text
