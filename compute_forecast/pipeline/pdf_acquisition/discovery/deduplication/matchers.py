...
class IdentifierNormalizer:
    ...
    def normalize_arxiv_id(self, arxiv_id: str) -> Optional[str]:
        ...
        if self.arxiv_new_pattern.match(normalized) or self.arxiv_old_pattern.match(
            normalized
        ):
            return normalized
...
class PaperFuzzyMatcher:
    ...
    def calculate_venue_year_match(self, paper1: Paper, paper2: Paper) -> bool:
        ...
        return (
            norm1.normalized_venue == norm2.normalized_venue
            and norm1.confidence > 0.8
    ...
    def find_duplicates_exact(
        ...
        for record in records:
            ...
            if paper:
                ...
                # Group by arXiv ID
                if paper.arxiv_id:
                    normalized_arxiv = self.identifier_normalizer.normalize_arxiv_id(
                        paper.arxiv_id
                    )
                    if normalized_arxiv:
    ...
    def find_duplicates_fuzzy(
        ...
        if len(records) > 5000:
            logger.warning(
                f"Skipping fuzzy matching for {len(records)} records (too large)"
            )
            return matches
