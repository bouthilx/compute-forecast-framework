...
class AAICollector(BasePDFCollector):
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if attempt < self.max_retries - 1:
                    ...
                    logger.warning(
                        f"Request failed (attempt {attempt + 1}/{self.max_retries}), retrying in {delay}s: {e}"
                    )
                    time.sleep(delay)
                else:
                    logger.error(
    ...
    def _search_by_title(self, title: str, year: int) -> Optional[Tuple[str, str]]:
        ...
        try:
            ...
            if best_match:
                ...
                logger.info(
                    f"Found paper '{title}' as '{matched_title}' (score: {score}) -> article {article_id}"
                )
    ...
    def _get_pdf_id_from_article(self, article_id: str) -> Optional[str]:
        ...
        try:
            article_url = (
                f"{self.base_url}{self.proceedings_path}/article/view/{article_id}"
            )
    ...
    def _search_by_authors(
        self, authors: List[str], title: str, year: int
