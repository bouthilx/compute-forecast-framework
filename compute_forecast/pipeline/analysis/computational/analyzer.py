...
from compute_forecast.pipeline.analysis.base import BaseAnalyzer
...
class ComputationalAnalyzer(BaseAnalyzer):
    ...
    def get_confidence_score(self, analysis_result: Dict[str, Any]) -> float:
        ...
        is_experimental = analysis_result["experimental_indicators"][
    ...
    def extract_resource_metrics(self, text: str) -> Dict[str, Dict[str, Any]]:
        ...
        for metric_type, pattern in self.patterns.items():
            ...
            if matches:
                metrics[metric_type] = {
                    "raw_matches": matches,
                    "normalized_values": self.normalize_metric_values(
                        metric_type, matches
                    ),
                    "count": len(matches),
                }
    ...
    def normalize_metric_values(
        self, metric_type: str, matches: List[tuple]
    ) -> List[Dict[str, Any]]:
            ...
            try:
                if metric_type == "gpu_count":
                    ...
                    if isinstance(match, tuple) and len(match) >= 2:
                        ...
                        normalized.append(
                            {
                                "value": float(value),
                                "unit": unit.lower(),
                                "hours_equivalent": self._convert_to_hours(
                                    float(value), unit
                                ),
                            }
                        )
                        ...
                        normalized.append(
                            {"value": match, "unit": "unknown", "raw": match}
                        )
                    ...
                    if isinstance(match, tuple) and len(match) >= 2:
                        ...
                        # Fallback for unexpected format
                        normalized.append(
                            {"value": match, "scale": "unknown", "raw": match}
                        )
                    ...
                    if isinstance(match, tuple) and len(match) >= 2:
                        value, unit = match[0], match[1]
                        normalized.append(
                            {"value": float(value), "unit": unit, "raw": match}
                        )
                    else:
                        normalized.append(
                            {"value": match, "unit": "unknown", "raw": match}
                    ...
                    if isinstance(match, tuple) and len(match) >= 2:
                        ...
                        normalized.append(
                            {"value": float(value), "scale": scale, "raw": match}
                        )
                    else:
                    ...
                    if isinstance(match, tuple):
                        normalized.append(
                            {"value": match[0] if match else None, "raw": match}
                        )
                    else:
    ...
    def detect_experimental_content(self, text: str) -> Dict[str, Any]:
        ...
        experimental_score = sum(
            1 for indicator in experimental_indicators if indicator in text_lower
