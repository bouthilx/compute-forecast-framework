...
class ComputationalContentFilter:
    ...
    def __init__(self):
        ...
        self.compute_pattern = re.compile(
            "|".join(self.compute_keywords), re.IGNORECASE
        )
    ...
    def compute_richness_score(self, paper: Dict) -> float:
        ...
        if not abstract.strip():
            ...
            if any(kw in title_lower for kw in ml_keywords):
                ...
                if any(
                    kw in title_lower
                    for kw in ["survey", "review", "analysis of", "study of"]
                ):
                    return 0.0
    ...
    def filter_by_richness(
        self, papers: List[Dict], min_score: float = 0.4
    ) -> List[Dict]:
        ...
        scored_papers.sort(
            key=lambda p: p["computational_richness_score"], reverse=True
        )
...
class MilaPaperSelector:
    ...
    def select_papers(
        self, papers: List[Dict], criteria: PaperSelectionCriteria
    ) -> List[Dict]:
        ...
        for year in sorted(papers_by_year.keys()):
            ...
            for domain in criteria.domains:
                ...
                n_select = min(
                    len(domain_papers), criteria.papers_per_domain_per_year_max
                )
            ...
            if len(year_papers) < criteria.papers_per_year_min:
                ...
                remaining_papers = [
                    p for p in papers_by_year[year] if p["paper_id"] not in selected_ids
                ]
