...
from .base import BaseConsolidationSource
...
class LoggingSourceWrapper(BaseConsolidationSource):
    ...
    def find_papers(self, papers: List[Paper]) -> Dict[str, str]:
        ...
        self.logger.info("→ Sending paper ID lookup request...")

        try:
            result = self.wrapped_source.find_papers(papers)
            duration = time.time() - start_time
            self.logger.info(
                f"← Received response in {duration:.2f}s - found {len(result)} matches"
            )
            return result
    ...
    def fetch_all_fields(self, source_ids: List[str]) -> Dict[str, Dict[str, Any]]:
        ...
        self.logger.info(
            f"Preparing to fetch enrichment data for {len(source_ids)} papers"
        )

        start_time = time.time()
        self.logger.info("→ Sending enrichment data request...")
        ...
        try:
            ...
            citations = sum(
                1 for r in result.values() if r.get("citations") is not None
            )
            ...
            self.logger.info(
                f"← Received response in {duration:.2f}s - "
    ...
    def enrich_papers(
        ...
        def logging_progress_callback(result):
            ...
            if self._papers_processed % 50 == 0:
                self.logger.debug(
                    f"Progress: {self._papers_processed}/{len(papers)} papers processed"
                )
            ...
            if (
                ...
                if counts and self._papers_processed <= 5:  # First 5 papers only
                    self.logger.debug(
                        f"Paper {result.paper_id}: found {', '.join(counts)}"
                    )
        ...
        self.logger.info(
            f"Batch completed in {duration:.2f}s - Total enrichments: "
