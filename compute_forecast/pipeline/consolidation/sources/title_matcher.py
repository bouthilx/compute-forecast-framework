...
class TitleMatcher:
    ...
    def _normalize_authors(self, authors1: List, authors2: List) -> Tuple[set, set]:
            ...
            for a1_first, a1_last in authors1_normalized:
                for i, (a2_first, a2_last) in enumerate(authors2_normalized):
                    ...
                    if a1_last == a2_last:
                        ...
                        if (
                            not a1_first
                            or not a2_first  # One is missing
                            or a1_first == a2_first
                        ):  # Same initial
                            matches += 1
                            used_indices.add(i)
                            break
            ...
            for a2_first, a2_last in authors2_normalized:
                for i, (a1_first, a1_last) in enumerate(authors1_normalized):
                    if i in used_indices:
                        continue
                    if a2_last == a1_last:
                        if not a1_first or not a2_first or a1_first == a2_first:
                            matches += 1
                            used_indices.add(i)
                            break
        ...
        set2 = set(
            range(
                len(authors1_normalized),
                len(authors1_normalized) + len(authors2_normalized),
            )
        )
    ...
    def calculate_similarity(
        ...
        if norm1 == norm2:
            ...
            if self.require_safety_checks:
                ...
                if authors1 and authors2:
                    authors1_normalized, authors2_normalized = self._normalize_authors(
                        authors1, authors2
                    )
                    ...
                    if min_authors > 0:
                        ...
                        logger.debug(
                            f"Author overlap: {overlap}/{min_authors} = {overlap_ratio:.2f}, penalty factor: {author_penalty:.2f}"
                        )
        ...
        similarity = token_sort * 0.4 + token_set * 0.3 + partial * 0.2 + ratio * 0.1
