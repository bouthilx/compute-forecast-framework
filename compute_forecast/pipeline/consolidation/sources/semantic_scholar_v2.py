...
class SemanticScholarSource(BaseConsolidationSource):
    ...
    def find_papers(self, papers: List[Paper]) -> Dict[str, str]:
        ...
        if papers_needing_title_search:
            self.logger.info(f"Batch title search for {len(papers_needing_title_search)} papers")
            ...
            with profile_operation('title_searches', source=self.name, count=len(papers_needing_title_search)):
                for i, paper in enumerate(papers_needing_title_search):
                    ...
                    if i > 0 and i % 10 == 0:
                        self.logger.info(f"Title search progress: {i}/{len(papers_needing_title_search)}")
