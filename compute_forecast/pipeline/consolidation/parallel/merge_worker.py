...
from compute_forecast.pipeline.consolidation.models import (
    CitationData, CitationRecord,
    AbstractData, AbstractRecord,
    URLData, URLRecord,
    IdentifierData, IdentifierRecord
)
...
class MergeWorker(threading.Thread):
    ...
    def _merge_enrichment(self, result: Dict[str, Any]):
        ...
        if enrichment.get('semantic_scholar_id'):
            ...
            consolidated.external_ids['semantic_scholar'] = enrichment['semantic_scholar_id']
        ...
        for id_field in ['pmid', 'pmcid', 'mag_id']:
            if enrichment.get(id_field):
                if not hasattr(consolidated, 'external_ids'):
                    consolidated.external_ids = {}
                consolidated.external_ids[id_field.replace('_id', '')] = enrichment[id_field]
        
        # 2. Citations - append CitationRecord
        if enrichment.get('citations') is not None:
            consolidated.citations.append(CitationRecord(
                source=source,
                timestamp=timestamp,
                original=False,
        ...
        if enrichment.get('abstract'):
            consolidated.abstracts.append(AbstractRecord(
                source=source,
                timestamp=timestamp,
                original=False,
        ...
        for url in enrichment.get('urls', []):
            consolidated.urls.append(URLRecord(
                source=source,
                timestamp=timestamp,
                original=False,
        ...
        for identifier in enrichment.get('identifiers', []):
            consolidated.identifiers.append(IdentifierRecord(
                source=source,
                timestamp=timestamp,
                original=False,
                data=IdentifierData(
