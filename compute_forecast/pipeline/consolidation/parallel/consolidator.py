...
from compute_forecast.pipeline.consolidation.parallel.openalex_worker import OpenAlexWorker
from compute_forecast.pipeline.consolidation.parallel.semantic_scholar_worker import SemanticScholarWorker
...
class ParallelConsolidator:
    ...
    def process_papers(self, papers: List[Paper], progress_update_callback: Optional[Callable[[str, int], None]] = None, input_file: str = "", checkpoint_papers: Optional[List[Paper]] = None, checkpoint_stats: Optional[Dict[str, Any]] = None) -> List[Paper]:
        ...
        logger.debug(f"All papers added. OpenAlex queue size: {self.openalex_input_queue.qsize()}, SS queue size: {self.ss_input_queue.qsize()}")
    ...
    def _monitor_progress(self, total_papers: int, progress_callback: Optional[Callable[[str, int, int, int], None]]):
        ...
        while self.merge_worker.papers_merged < expected_enrichments:
            ...
            try:
                ...
                current_merged = self.merge_worker.papers_merged
                
                # Update progress based on what sources have completed
                # This is approximate since we can't peek into the queue
                if progress_callback and time.time() - last_progress_update > 0.1:  # Update every 100ms
