...
class ConsolidationCheckpointManager:
    ...
    def __init__(
        ...
        logger.info(
            f"ConsolidationCheckpointManager initialized: session={self.session_id}, interval={checkpoint_interval_minutes}min"
        )
    ...
    def _generate_session_id(self) -> str:
        ...
        return (
            f"consolidate_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{uuid4().hex[:8]}"
        )
    ...
    def save_checkpoint(
        ...
        try:
            ...
            logger.info(
                f"Checkpoint saved in {duration:.2f}s: {len(papers)} papers, sources={list(sources_state.keys())}"
            )
        ...
        try:
            ...
            if data.get("session_id") != self.session_id:
                ...
                if self.session_id != data.get("session_id"):
                    ...
                    logger.info(
                        f"Updated session ID to match checkpoint: {self.session_id}"
                    )
            ...
            if self.papers_file.exists():
                try:
                    ...
                    if isinstance(data, dict) and "papers" in data:
                        ...
                        logger.info(
                            f"Loading papers: {metadata.get('total_papers', 'unknown')} total, "
            ...
            logger.info(
                f"Loaded checkpoint: {len(papers)} papers, sources={list(checkpoint.sources.keys())}"
            )
    ...
    def _save_checkpoint_atomic(self, checkpoint: ConsolidationCheckpoint):
        ...
        if self.checkpoint_file.exists():
            ...
            try:
                ...
                if self.checkpoint_meta_file.exists():
                    shutil.copy2(
                        self.checkpoint_meta_file, backup_file.with_suffix(".bak.meta")
                    )
            except Exception as e:
        ...
        with open(meta_temp, "w") as f:
            json.dump(
                {"checksum": checksum, "timestamp": datetime.now().isoformat()}, f
            )
    ...
    def find_resumable_sessions(
        cls, checkpoint_dir: Path = Path(".cf_state/consolidate")
    ) -> List[Dict[str, Any]]:
        """
            ...
            if checkpoint_file.exists():
                try:
                    ...
                    if isinstance(sources_status, str):
                        logger.warning(
                            f"Unexpected string value for sources in {session_dir.name}: {sources_status}"
                        )
                    ...
                    if "phase" in sources_status:
                        ...
                        if phase == "completed":
                            ...
                            "id_harvesting_complete",
                            "semantic_scholar_complete",
                        ]:
                            status = "interrupted"  # Between phases
                        else:
                            status = "pending"
                    elif any(
                        "papers_processed" in s
                        for s in sources_status.values()
                    ...
                    if isinstance(sources_status, dict):
                        for source_name, source_data in sources_status.items():
                            if (
                                ...
                                and "papers_processed" in source_data
                            ):
                                source_stats[source_name] = {
                                    "papers_processed": source_data.get(
                                        "papers_processed", 0
                                    ),
                                    "papers_enriched": source_data.get(
                                        "papers_enriched", 0
                                    ),
                                    "citations_found": source_data.get(
                                        "citations_found", 0
                                    ),
                                    "abstracts_found": source_data.get(
                                        "abstracts_found", 0
                                    ),
    ...
    def get_latest_resumable_session(
        cls, input_file: str, checkpoint_dir: Path = Path(".cf_state/consolidate")
    ) -> Optional[str]:
        """
        ...
        for session in sessions:
            if session["input_file"] == input_file and session["status"] in [
                "interrupted",
                "failed",
            ]:
    ...
    def merge_enrichments(paper: Paper, new_enrichments: Any):
        ...
        def record_exists(records, new_record):
            for r in records:
                if r.source == new_record.source and r.data == new_record.data:
                    return True
            return False
