...
class CollectionPhaseValidator(PhaseValidator):
    ...
    def validate(self, data: List[Paper]) -> ValidationResult:
        ...
        if valid_papers < self.min_papers:
            result.add_error(
                f"Insufficient valid papers: {valid_papers} < {self.min_papers}"
            )
        ...
        for analysis in data:
            ...
            if hasattr(analysis, "has_computational_content"):
                if analysis.has_computational_content:
                    ...
                    if (
                        not hasattr(analysis, "resource_metrics")
                        or not analysis.resource_metrics
                    ):
                        missing_metrics += 1
                        result.add_error(
                            f"Paper {analysis.paper_id} missing resource metrics"
                        )
                    else:
                        valid_analyses += 1
            else:
                result.add_error(
                    f"Analysis {analysis.paper_id} missing computational content flag"
                )
...
class ProjectionPhaseValidator(PhaseValidator):
    ...
    def validate(self, data: Dict[str, Any]) -> ValidationResult:
            ...
            for year in projection_years:
                ...
                if year_str in ci_data:
                    ...
                    if "lower" not in ci or "upper" not in ci:
                        result.add_warning(
                            f"Incomplete confidence interval for year {year}"
                        )
        ...
        result.metrics = {
            ...
            "resource_types": len(
                resource_projections.get(str(projection_years[0]), {})
            )
            if projection_years
            else 0,
        }
...
class TransitionValidator:
    ...
    def validate_transition(
        ...
        result = ValidationResult(phase=to_phase, is_valid=True)
        ...
        if (
            from_phase == PipelinePhase.COLLECTION
            and to_phase == PipelinePhase.EXTRACTION
        ):
            self._validate_collection_to_extraction(from_data, to_data, result)
        elif (
            from_phase == PipelinePhase.EXTRACTION
            and to_phase == PipelinePhase.ANALYSIS
        ):
            self._validate_extraction_to_analysis(from_data, to_data, result)
        elif (
            from_phase == PipelinePhase.ANALYSIS
            and to_phase == PipelinePhase.PROJECTION
        ):
            self._validate_analysis_to_projection(from_data, to_data, result)
        elif (
            from_phase == PipelinePhase.PROJECTION
            and to_phase == PipelinePhase.REPORTING
        ):
            self._validate_projection_to_reporting(from_data, to_data, result)

        return result

    def _validate_collection_to_extraction(
        self, from_data: List[Paper], to_data: List[Paper], result: ValidationResult
    ) -> None:
        ...
        if isinstance(from_data, list) and isinstance(to_data, list):
            ...
            if to_count < from_count:
                ...
                result.add_warning(
                    f"Data loss in transition: {loss_percent:.1f}% papers lost"
                )
    ...
    def _validate_extraction_to_analysis(
        self, from_data: List[Paper], to_data: Any, result: ValidationResult
    ) -> None:
    ...
    def _validate_analysis_to_projection(
        self, from_data: Any, to_data: Dict[str, Any], result: ValidationResult
    ) -> None:
    ...
    def _validate_projection_to_reporting(
        self,
        ...
        if isinstance(from_data, dict) and isinstance(to_data, dict):
            if "summary" in to_data and isinstance(to_data["summary"], dict):
                ...
                if "total_papers" in from_data and "total_analyzed" not in summary:
                    result.add_warning(
                        "Projection data not reflected in report summary"
                    )
...
class DataIntegrityChecker:
    ...
    def check_unique_ids(
        self, items: List[Dict[str, Any]], id_field: str = "id"
    ) -> Tuple[bool, Set[str]]:
