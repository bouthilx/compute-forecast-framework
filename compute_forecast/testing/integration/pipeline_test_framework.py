...
class EndToEndTestFramework:
    ...
    def register_phase_validator(
        self, phase: PipelinePhase, validator: Callable[[Any], bool]
    ) -> None:
    ...
    def _run_phase_logic(
        self, phase: PipelinePhase, data: Any, metrics: PhaseMetrics
    ) -> Any:
    ...
    def _execute_collection(
        self, papers: List[Paper], metrics: PhaseMetrics
    ) -> List[Paper]:
        ...
        if metrics.errors_encountered:
            raise ValueError(
                f"Collection phase failed with {len(metrics.errors_encountered)} errors"
            )
    ...
    def _execute_extraction(
        ...
        for paper in papers:
            ...
            if hasattr(paper, "abstract") and paper.abstract:
                ...
                metrics.errors_encountered.append(
                    f"No abstract for paper {paper.paper_id}"
                )
    ...
    def _execute_projection(
        self, analyses: List[Any], metrics: PhaseMetrics
    ) -> Dict[str, Any]:
        ...
        projections = {
            ...
            "average_score": sum(a.get("score", 0) for a in analyses) / len(analyses)
            if analyses
            else 0,
    ...
    def _execute_reporting(
        self, projections: Dict[str, Any], metrics: PhaseMetrics
    ) -> Dict[str, Any]:
    ...
    def validate_phase_transition(
        self, from_phase: PipelinePhase, to_phase: PipelinePhase, data: Any
    ) -> bool:
        ...
        if (
            ...
            return isinstance(data, list) and all(
                isinstance(p, (Paper, dict)) for p in data
            )
        ...
        while not self._stop_monitoring.is_set():
            try:
                ...
                for phase, profile in self.performance_profiles.items():
                    if (
                        phase.value in self.phase_metrics
                        and not self.phase_metrics[phase.value].end_time
                    ):
            ...
            if phase.value in self.phase_metrics:
                ...
                if metrics.execution_time_seconds > expected_time * 0.8:
                    phase_bottlenecks.append(
                        f"Slow execution: {metrics.execution_time_seconds:.1f}s"
                    )
