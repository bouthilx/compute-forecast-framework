...
from compute_forecast.testing.integration.performance_monitor import PerformanceMonitor
...
class ErrorRecoveryTestScenario:
            ...
            def error_injector(data: Any) -> bool:
                ...
                for config in phase_configs:
                    if random.random() < config.failure_rate:
                        ...
                        print(
                            f"   💥 Injected {config.error_type.value} in {phase.value} phase"
                        )
                        ...
                        if config.recovery_possible:
                            ...
                            if recovery_success:
                                ...
                                print(
                                    f"   ❌ Failed to recover from {config.error_type.value}"
                                )
                                self.recovery_attempts.append(
                                    {
                                        "error": error_info,
    ...
    def _analyze_recovery_results(
        ...
        graceful_degradation = (
            pipeline_result["success"] or len(pipeline_result["phases_completed"]) > 0
        )
        ...
        phases_completed_ratio = len(pipeline_result["phases_completed"]) / len(
        ...
        success = (
            graceful_degradation
            and error_handling_quality > 0.5
            and resilience_score > 0.6
        )
        ...
        return ErrorRecoveryResult(
            ...
            recovery_metrics={
                ...
                "recovery_success_rate": errors_recovered / len(self.recovery_attempts)
    ...
    def _generate_recovery_recommendations(
        ...
        for phase, errors in failures_by_phase.items():
            if len(errors) > 2:
                recommendations.append(
                    f"Implement better error handling for {phase} phase"
                )
        ...
        all_error_types = [
            error for errors in failures_by_phase.values() for error in errors
        ]
    ...
    def _print_results(self, result: ErrorRecoveryResult) -> None:
        ...
        print(
            f"Graceful Degradation: {'✅ Yes' if result.graceful_degradation else '❌ No'}"
        )
        print(
            f"Partial Results Preserved: {'✅ Yes' if result.partial_results_preserved else '❌ No'}"
        )
