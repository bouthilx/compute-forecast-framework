...
class NormalFlowTestScenario:
    ...
    def _setup_validators(self) -> None:
        ...
        self.framework.register_phase_validator(PipelinePhase.COLLECTION, validate_collection)
        self.framework.register_phase_validator(PipelinePhase.EXTRACTION, validate_extraction)
        self.framework.register_phase_validator(PipelinePhase.ANALYSIS, validate_analysis)
        self.framework.register_phase_validator(PipelinePhase.PROJECTION, validate_projection)
        self.framework.register_phase_validator(PipelinePhase.REPORTING, validate_reporting)
    ...
    def run_test(self) -> NormalFlowResult:
        ...
        try:
            ...
            result = NormalFlowResult(
                success=pipeline_result["success"] and self._meets_success_criteria(
                    execution_time, performance_analysis
                ),
    ...
    def _validate_results(self, pipeline_result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        validation_results = {
            ...
            "completion_rate": len(pipeline_result["phases_completed"]) / len(self.config.phases_to_test)
        }
        ...
        for phase_name, metrics in pipeline_result["phase_metrics"].items():
            ...
            validation_results[f"{phase_name}_duration"] = metrics.execution_time_seconds
    ...
    def _meets_success_criteria(self, execution_time: float, 
        ...
        for phase_name, phase_data in performance_analysis.items():
            if isinstance(phase_data, dict):
                ...
                if phase_data.get("peak_cpu", 0) > 80:
                    bottlenecks.append(f"{phase_name}: High CPU usage ({phase_data['peak_cpu']:.1f}%)")
                ...
                if phase_data.get("duration", 0) > 60:  # More than 1 minute per phase
                    bottlenecks.append(f"{phase_name}: Slow execution ({phase_data['duration']:.1f}s)")
    ...
    def _generate_recommendations(self, performance_analysis: Dict[str, Any]) -> List[str]:
        ...
        if total_duration > 240:  # 4 minutes, close to limit
            recommendations.append("Consider optimizing slow phases to improve overall execution time")
            
        peak_memory = self._get_peak_memory(performance_analysis)
        if peak_memory > 3000:  # 75% of limit
            recommendations.append("Implement memory optimization to prevent hitting 4GB limit")
            
        # Phase-specific recommendations
        for phase_name, phase_data in performance_analysis.items():
            if isinstance(phase_data, dict):
                if phase_data.get("duration", 0) > 60:
                    recommendations.append(f"Optimize {phase_name} phase execution time")
