...
class PerformanceRegressionTestScenario:
    ...
    def run_test(self, save_as_baseline: bool = False) -> PerformanceRegressionResult:
        ...
        if self.baseline:
            print(
                f"   📊 Comparing against baseline: {self.baseline.version} ({self.baseline.date})"
            )
        else:
        ...
        try:
            ...
            if self.baseline:
                ...
                result = self._create_baseline_result(
                    current_performance, execution_time
                )
    ...
    def _analyze_regression(
        self, current: PerformanceBaseline, execution_time: float
            ...
            if analysis.is_regression:
                if analysis.severity == "critical":
                    critical_issues.append(
                        f"Critical regression in {analysis.metric_name}: "
                        f"{analysis.change_percent:+.1%}"
                    )
                else:
                    regressions.append(
                        f"{analysis.metric_name}: {analysis.change_percent:+.1%}"
                    )
        ...
        # Determine success
        success = len(critical_issues) == 0 and regression_score > 0.8
    ...
    def _analyze_metric(
        ...
        return RegressionAnalysis(
            ...
            and abs_change > 0.05,  # Only count >5% as regression
    ...
    def _generate_regression_recommendations(
        self, analyses: List[RegressionAnalysis], critical_issues: List[str]
    ) -> List[str]:
        ...
        if critical_issues:
            recommendations.append(
                "Immediately investigate critical performance regressions"
            )
            recommendations.append(
    ...
    def _create_baseline_result(
        ...
        return PerformanceRegressionResult(
            ...
            critical_issues=[],
            recommendations=[
                "Baseline established - future runs will compare against this"
            ],
            execution_time_seconds=execution_time,
            errors=[],
        )
...
def run_performance_regression_test(
    baseline_file: Optional[str] = None, save_as_baseline: bool = False
) -> PerformanceRegressionResult:
