...
from compute_forecast.pipeline.consolidation.parallel.consolidator import (
    ParallelConsolidator,
)
...
def main(
    ...
    if resume:
        ...
        if checkpoint_result:
            ...
            if hasattr(checkpoint_data, "phase_state") and checkpoint_data.phase_state:
                ...
                console.print(
                    f"  OpenAlex processed: {len(phase_state.openalex_processed_hashes)}"
                )
                console.print(
                    f"  Semantic Scholar processed: {len(phase_state.semantic_scholar_processed_hashes)}"
                )

            # Count actual enriched papers from the loaded file
            actual_stats = {
                "openalex": {
                    "papers_processed": checkpoint_data.sources.get("openalex", {}).get(
                        "papers_processed", 0
                    ),
                    "papers_enriched": 0,
                    "citations_found": 0,
                ...
                "semantic_scholar": {
                    "papers_processed": checkpoint_data.sources.get(
                        "semantic_scholar", {}
                    ).get("papers_processed", 0),
                    "papers_enriched": 0,
                    "citations_found": 0,
                    "abstracts_found": 0,
                },
            ...
            if enriched_file.exists():
                try:
                    ...
                    console.print(
                        f"  OpenAlex: {actual_stats['openalex']['papers_enriched']} enriched, "
                        f"{actual_stats['openalex']['citations_found']} citations, "
                        f"{actual_stats['openalex']['abstracts_found']} abstracts"
                    )
                    console.print(
                        f"  Semantic Scholar: {actual_stats['semantic_scholar']['papers_enriched']} enriched, "
                    ...
                    console.print(
                        f"[yellow]Warning: Could not read enriched papers file: {e}[/yellow]"
                    )
                    actual_stats = None
    ...
    with Live(
        ...
        openalex_task = progress.add_task("[cyan]OpenAlex[/cyan]", total=len(papers))

        ss_task = progress.add_task(
            "[green]Semantic Scholar[/green]", total=len(papers)
        )
        ...
        if resume and (
            ...
            console.print(
                f"  OpenAlex: {oa_processed} processed, {oa_stats.get('papers_enriched', 0)} enriched"
            )
            console.print(
                f"  Semantic Scholar: {ss_processed} processed, {ss_stats.get('papers_enriched', 0)} enriched"
            )

        # Define progress callback
        def update_progress(source: str, count: int, citations: int, abstracts: int):
            logger.debug(
                f"Progress update: {source} - count:{count}, citations:{citations}, abstracts:{abstracts}"
            )
            if source == "openalex":
    ...
    console.print(
        f"  Papers with citations: {citation_count} ({citation_count / len(enriched_papers) * 100:.1f}%)"
    )
    console.print(
        f"  Papers with abstracts: {abstract_count} ({abstract_count / len(enriched_papers) * 100:.1f}%)"
    )
    console.print(
        f"  Papers with DOIs: {doi_count} ({doi_count / len(enriched_papers) * 100:.1f}%)"
    )
    console.print(
        f"  Papers with ArXiv IDs: {arxiv_count} ({arxiv_count / len(enriched_papers) * 100:.1f}%)"
    )
