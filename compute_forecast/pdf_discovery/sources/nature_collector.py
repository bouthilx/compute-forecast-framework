...
from compute_forecast.core.exceptions import UnsupportedSourceError, PDFNotAvailableError, PDFNetworkError
...
class NaturePDFCollector(BasePDFCollector):
    ...
    def __init__(self, email: str):
        ...
        self.nature_doi_pattern = re.compile(r'10\.1038/(?:s41467|srep|ncomms)-?\d+(?:-\d+)?(?:-\w+)?')
    ...
    def is_nature_paper(self, paper: Paper) -> bool:
        ...
        if paper.venue:
            ...
            if any(journal in venue_lower for journal in self.supported_journals.keys()):
                return True
    ...
    def _check_open_access_availability(self, article_id: str) -> Optional[str]:
        ...
        try:
            ...
            response = requests.head(
                ...
                headers={
                    "User-Agent": f"Academic PDF Discovery Bot ({self.email})"
                }
            )
            ...
            if response.status_code in [302, 303] and 'idp.nature.com' in str(response.headers.get('Location', '')):
