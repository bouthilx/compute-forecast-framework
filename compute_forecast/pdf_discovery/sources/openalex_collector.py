...
class OpenAlexPDFCollector(BasePDFCollector):
    ...
    def __init__(
        self, email: Optional[str] = None, mila_institution_id: Optional[str] = None
    ):
        ...
        self.headers = {"User-Agent": "PDF-Discovery-Framework/1.0"}
        if self.email:
        ...
        logger.info(
            f"Initialized OpenAlex PDF collector "
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if retries >= self.max_retries:
                        raise Exception(
                            f"OpenAlex server error: {response.status_code}"
                        )
                ...
                if retries >= self.max_retries:
                    logger.error(
                        f"OpenAlex request failed after {self.max_retries} retries: {e}"
                    )
                    raise

                wait_time = self.retry_delay * (2 ** (retries - 1))
                logger.warning(
                    f"OpenAlex request error (retry {retries}/{self.max_retries}): {e}"
                )
                time.sleep(wait_time)
                            ...
                            if work_doi in doi_to_paper:
                                ...
                                if pdf_url:
                                    pdf_record = PDFRecord(
                                        ...
                                        version_info={
                                            "openalex_id": work.get("id", ""),
                                            "has_mila_author": self._has_mila_author(
                                                work
                                            ),
                                        },
                                        validation_status="available",
