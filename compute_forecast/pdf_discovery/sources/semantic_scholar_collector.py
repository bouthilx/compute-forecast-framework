...
class SemanticScholarPDFCollector(BasePDFCollector):
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        logger.info(
            f"Initialized Semantic Scholar PDF collector "
    ...
    def _match_paper_to_response(
        self, ss_paper: dict, paper_map: Dict[str, Paper]
    ) -> Optional[Paper]:
    ...
    def _discover_single(self, paper: Paper) -> PDFRecord:
        ...
        while retries < self.max_retries:
            try:
                ...
                if not ss_paper:
                    ...
                    search_results = self.api.search_paper(query=paper.title, limit=5)

                    if search_results:
                        # Handle both dict and list responses
                        if (
                            isinstance(search_results, dict)
                            and "data" in search_results
                        ):
                ...
                if retries >= self.max_retries:
                    logger.error(
                        f"Failed to query Semantic Scholar after {self.max_retries} retries: {e}"
                    )
                    raise

                logger.warning(
                    f"Semantic Scholar API error (retry {retries}/{self.max_retries}): {e}"
                )
        ...
        has_identifier = (
            (hasattr(paper, "doi") and paper.doi)
            or (hasattr(paper, "arxiv_id") and paper.arxiv_id)
            or (hasattr(paper, "semantic_scholar_id") and paper.semantic_scholar_id)
        )
        confidence_score = (
    ...
    def discover_pdfs_batch(self, papers: List[Paper]) -> Dict[str, PDFRecord]:
        ...
        for i in range(0, len(papers), self.batch_size):
            ...
            try:
                ...
                for paper in batch:
                    ...
                    if (
                        hasattr(paper, "semantic_scholar_id")
                        and paper.semantic_scholar_id
                    ):
                    ...
                    for ss_paper in ss_papers:
                        ...
                        if (
                            ...
                            pdf_record = PDFRecord(
                                ...
                                version_info={
                                    "ss_paper_id": ss_paper.get("paperId"),
                                    "pdf_status": open_access_pdf.get(
                                        "status", "unknown"
                                    ),
                                },
                                validation_status=open_access_pdf.get(
                                    "status", "unknown"
                                ),
                ...
                for paper in batch:
                    if (
                        ...
                        try:
                            ...
                            logger.debug(
                                f"Failed to discover PDF for {paper.paper_id}: {e}"
                            )
        ...
        logger.info(
            f"Batch discovery complete: found {len(results)}/{len(papers)} PDFs"
        )
        return results
