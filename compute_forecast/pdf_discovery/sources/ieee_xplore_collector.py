...
class IEEEXplorePDFCollector(BasePDFCollector):
    ...
    def _build_api_url(self, doi: Optional[str] = None, title: Optional[str] = None) -> str:
    ...
    def _parse_api_response(self, response_data: dict, paper: Paper) -> PDFRecord:
        ...
        if not pdf_url:
            ...
            if article_number:
                ...
                logger.warning(f"No pdf_url in response, article may not have open PDF access")
        ...
        confidence_score = CONFIDENCE_SCORE_WITH_DOI if hasattr(paper, 'doi') and paper.doi else CONFIDENCE_SCORE_TITLE_SEARCH
