...
class BasePDFCollector(ABC):
    ...
    def discover_pdfs(self, papers: List[Paper]) -> Dict[str, PDFRecord]:
        ...
        logger.info(f"Starting PDF discovery for {len(papers)} papers using {self.source_name}")
        ...
        with ThreadPoolExecutor(max_workers=1) as executor:
            for paper in papers:
                try:
                    ...
                    logger.warning(f"Timeout discovering PDF for {paper.paper_id} from {self.source_name}")
                    self._stats["failed"] += 1
                    
                except Exception as e:
                    logger.error(f"Error discovering PDF for {paper.paper_id} from {self.source_name}: {e}")
                    self._stats["failed"] += 1
        
        logger.info(f"Discovered {len(results)}/{len(papers)} PDFs from {self.source_name}")
