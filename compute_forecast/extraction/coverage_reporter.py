...
class TemplateCoverageReport:
    ...
    validation_failures: int = 0
    field_coverage: Dict[ExtractionField, FieldCoverageStats] = field(
    ...
    def get_required_field_coverage(
...
class CoverageReporter:
    ...
    def add_extraction_result(
        self, result: Dict[str, Any], template: ExtractionTemplate
    ):
        ...
        for template_field in all_fields:
            ...
            for error in validation_errors:
                if (
                    ...
                    and error.get("field") == template_field.value
                ):
    ...
    def get_field_insights(self, field: ExtractionField) -> Dict[str, Any]:
        ...
        if total_papers > 0:
            insights["overall_coverage"] = (
                f"{(papers_with_field / total_papers * 100):.1f}%"
            )

        if all_confidence_scores:
            insights["overall_confidence"] = (
                f"{statistics.mean(all_confidence_scores):.2f}"
            )

        # Get most common values
        if value_frequency:
            sorted_values = sorted(
                value_frequency.items(), key=lambda x: x[1], reverse=True
            )
