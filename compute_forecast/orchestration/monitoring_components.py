...
class SimpleMetricsCollector:
    ...
    def collect_current_metrics(self, session_id: str = "default") -> SystemMetrics:
        ...
        if session_id in self.start_times:
            ...
            if duration_minutes > 0:
                total_papers = sum(
                    m.papers_collected for m in self.session_metrics[session_id]
                )
    ...
    def get_session_summary(self, session_id: str) -> Dict[str, Any]:
        ...
        avg_papers_per_minute = sum(m.papers_per_minute for m in metrics_list) / len(
            metrics_list
        )
        ...
        return {
            ...
            "session_duration_minutes": (
                (metrics_list[-1].timestamp - metrics_list[0].timestamp).total_seconds()
                / 60
...
class SimpleDashboard:
    ...
    def list_active_dashboards(self) -> List[str]:
        ...
        return [
            session_id
            for session_id, data in self.active_dashboards.items()
...
class SimpleAlertSystem:
    ...
    def check_alerts(
        ...
        # Low collection rate
        if (
            self.alert_rules["low_collection_rate"]["enabled"]
            and metrics.papers_per_minute
            < self.alert_rules["low_collection_rate"]["threshold"]
        ):
            alert = {
        ...
        # High error rate
        if self.alert_rules["high_error_rate"]["enabled"] and metrics.errors_count > 0:
        ...
        # API failure
        if (
            self.alert_rules["api_failure"]["enabled"]
            and metrics.api_success_rate < self.alert_rules["api_failure"]["threshold"]
        ):
            alert = {
                "type": "api_failure",
                "severity": "critical",
