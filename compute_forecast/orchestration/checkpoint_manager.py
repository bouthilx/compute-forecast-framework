...
class CheckpointManager:
        ...
        with session_lock:
            try:
                ...
                if self.persistence.save_checkpoint(checkpoint):
                    ...
                    logger.info(
                        f"Created checkpoint {checkpoint_id} for session {session.session_id}"
                    )
                    return checkpoint
                else:
                ...
                logger.error(
                    f"Failed to create checkpoint for session {session.session_id}: {e}"
                )
                return None
    ...
    def create_venue_completion_checkpoint(
        ...
        venues_in_progress = [
            (v, y)
            for (v, y) in session.venues_in_progress
    ...
    def validate_checkpoint_chain(
        self, session_id: str
    ) -> List[CheckpointValidationResult]:
        """
    ...
    def find_best_recovery_checkpoint(
        self, session_id: str
    ) -> Optional[CheckpointData]:
        """
        ...
        try:
            ...
            if not recovery_candidates:
                logger.warning(
                    f"No suitable recovery checkpoints found for session {session_id}"
                )
                return None
            ...
            if checkpoint and checkpoint.validate_integrity():
                logger.info(
                    f"Best recovery checkpoint for {session_id}: {best_checkpoint_id}"
                )
                return checkpoint
            else:
                logger.error(
                    f"Failed to load or validate best recovery checkpoint {best_checkpoint_id}"
                )
                return None

        except Exception as e:
            logger.error(
                f"Failed to find best recovery checkpoint for session {session_id}: {e}"
            )
            return None
    ...
    def get_checkpoint_summary(self, session_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            recoverable_checkpoints = sum(
                1 for result in validation_results if result.can_be_used_for_recovery
            )
            ...
            if validation_results:
                avg_integrity = sum(
                    result.integrity_score for result in validation_results
            ...
            logger.error(
                f"Failed to get checkpoint summary for session {session_id}: {e}"
            )
            return {
                "session_id": session_id,
                "error": str(e),
    ...
    def cleanup_old_checkpoints(self, session_id: str, keep_count: int = 10) -> int:
        ...
        try:
            ...
            for checkpoint_id in checkpoints_to_remove:
                checkpoint_file = (
                    self.persistence.checkpoints_dir / f"{checkpoint_id}.json"
                )
            ...
            logger.info(
                f"Cleaned up {removed_count} old checkpoints for session {session_id}"
            )
