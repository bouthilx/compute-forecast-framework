...
class InterruptionRecoverySystem:
    ...
    def create_recovery_plan(
        ...
        estimated_time = self._estimate_recovery_time(
            len(venues_to_recover),
            strategy
        )
    ...
    def execute_recovery(
        ...
        try:
            ...
            if recovery_plan.strategy == RecoveryStrategy.RESUME_FROM_CHECKPOINT:
                ...
                result = self._skip_and_continue(
                    session,
                    recovery_plan,
                    recovery_state
                )
            else:
                result = self._full_restart(
                    session,
                    recovery_plan,
                    recovery_state
                )
            ...
            return RecoveryResult(
                success=False,
                recovered_data={},
                recovery_time=recovery_state.recovery_end_time - recovery_state.recovery_start_time,
