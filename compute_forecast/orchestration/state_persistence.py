        ...
        with session_lock:
            try:
                ...
                if not checkpoint.validate_integrity():
                    logger.error(
                        f"Checkpoint integrity validation failed: {checkpoint.checkpoint_id}"
                    )
                    return False

                checkpoint_file = (
                    self.checkpoints_dir / f"{checkpoint.checkpoint_id}.json"
                )
                ...
                logger.info(
                    f"Saved checkpoint: {checkpoint.checkpoint_id} for session {checkpoint.session_id}"
                )
                return True

            except Exception as e:
                logger.error(
                    f"Failed to save checkpoint {checkpoint.checkpoint_id}: {e}"
                )
                return False
    ...
    def check_data_integrity(self, session_id: str) -> List[IntegrityCheckResult]:
        ...
        if session_file.exists():
            integrity_results.append(
                self._check_file_integrity(session_file, "session")
            )
        ...
        for checkpoint_id in self.list_session_checkpoints(session_id):
            ...
            if checkpoint_file.exists():
                integrity_results.append(
                    self._check_file_integrity(checkpoint_file, "checkpoint")
                )
