...
class CollectionReportFormatter:
    ...
    def format_report(self, report: QualityReport, metrics: CollectionQualityMetrics) -> str:
...
class TextReportFormatter(CollectionReportFormatter):
    ...
    def format_report(self, report: QualityReport, metrics: CollectionQualityMetrics) -> str:
        ...
        lines.append(f"  Papers with all required fields: {metrics.papers_with_all_required_fields}/{metrics.total_papers_collected}")
        lines.append(f"  Papers with abstracts: {metrics.papers_with_abstracts}/{metrics.total_papers_collected}")
        lines.append(f"  Papers with PDFs: {metrics.papers_with_pdfs}/{metrics.total_papers_collected}")
        lines.append(f"  Papers with DOIs: {metrics.papers_with_dois}/{metrics.total_papers_collected}")
        lines.append("")
        
        # Consistency
        lines.append("Consistency:")
        lines.append(f"  Duplicate rate: {metrics.duplicate_rate:.1%} ({metrics.duplicate_count} papers)")
        ...
        lines.append(f"  Valid years: {metrics.valid_years_count}/{metrics.total_papers_collected}")
        lines.append(f"  Valid authors: {metrics.valid_authors_count}/{metrics.total_papers_collected}")
        lines.append(f"  Valid URLs: {metrics.valid_urls_count}/{metrics.total_papers_collected}")
        lines.append("")
    ...
    def _generate_recommendations(self, report: QualityReport, metrics: CollectionQualityMetrics) -> List[str]:
        """Generate recommendations based on quality results."""
        recommendations = []
        
        # Completeness recommendations
        if metrics.papers_with_all_required_fields < metrics.total_papers_collected * 0.9:
            recommendations.append("Improve extraction of required fields (title, authors, venue, year)")
        ...
        if len(metrics.papers_by_scraper) == 1:
            recommendations.append("Consider using multiple scrapers for better coverage")
        ...
        if not recommendations:
            recommendations.append("Quality looks good! Consider minor optimizations based on warnings.")
