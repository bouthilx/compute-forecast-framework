...
class QualityMonitoringIntegration:
    ...
    def __init__(self, 
        ...
        self.quality_alert_rules = {
            'quality_threshold_performance_degradation': {
                'condition': lambda metrics: metrics.get('collection_efficiency', 1.0) < 0.7,
                'severity': 'WARNING',
            ...
            'quality_trend_declining': {
                'condition': lambda metrics: metrics.get('quality_trend_direction') == 'declining',
                'severity': 'WARNING',
        ...
        try:
            ...
            if original_collect_paper:
                def quality_aware_collect_paper(paper_data):
                    ...
                    quality_metrics = self.quality_analyzer.assess_paper_quality(paper_data)
                    ...
                    if passes:
                        ...
                        # Reject the paper
                        self._record_quality_rejection(quality_metrics, reasons, venue, year)
                        logger.debug(f"Paper {paper_data.get('paper_id', 'unknown')} rejected: {reasons}")
                        return None
    ...
    def update_quality_performance(self, venue: str, year: Optional[int] = None) -> None:
        """
        Update quality performance metrics and adapt thresholds if needed.
        """
        try:
            # Calculate performance metrics
            performance_data = self.performance_tracker.calculate_performance_metrics(venue, year)
    ...
    def get_quality_dashboard_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            metrics['average_collection_efficiency'] = (
                ...
                if venue_efficiencies else 0.0
            )
    ...
    def _get_quality_filter(self, venue: str, year: Optional[int], 
                           thresholds: QualityThresholds) -> QualityFilter:
    ...
    def _record_quality_success(self, metrics: QualityMetrics, venue: str, year: Optional[int]) -> None:
        """Record successful quality assessment."""
        self.performance_tracker.record_success(metrics, venue, year)
    
    def _record_quality_rejection(self, metrics: QualityMetrics, reasons: List[str], 
                                 venue: str, year: Optional[int]) -> None:
        """Record quality rejection."""
        self.performance_tracker.record_rejection(metrics, reasons, venue, year)
    
    def _record_performance_update(self, venue: str, year: Optional[int], 
                                  performance_data: QualityPerformanceMetrics) -> None:
...
class QualityPerformanceTracker:
    ...
    def record_success(self, metrics: QualityMetrics, venue: str, year: Optional[int]) -> None:
        """Record successful quality assessment."""
    ...
    def record_rejection(self, metrics: QualityMetrics, reasons: List[str], 
                        venue: str, year: Optional[int]) -> None:
        """Record quality rejection."""
        venue_key = f"{venue}_{year}" if year else venue
    ...
    def calculate_performance_metrics(self, venue: str, year: Optional[int]) -> Optional[QualityPerformanceMetrics]:
    ...
    def _cleanup_old_records(self, venue_key: str) -> None:
        ...
        self.venue_performance[venue_key] = [
            record for record in self.venue_performance[venue_key]
