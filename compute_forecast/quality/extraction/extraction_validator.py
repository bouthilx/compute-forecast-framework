...
class ExtractionQualityValidator(QualityAnalyzer):
    ...
    def _calculate_validity_score(self, extraction: ComputationalAnalysis) -> float:
        ...
        # Check parameters
        if (
            "parameters" in extraction_dict
            and extraction_dict["parameters"] is not None
        ):
            total_fields += 1
        ...
        # Check training time
        if (
            "training_time" in extraction_dict
            and extraction_dict["training_time"] is not None
        ):
            total_fields += 1
        ...
        # Check batch size
        if (
            "batch_size" in extraction_dict
            and extraction_dict["batch_size"] is not None
        ):
            total_fields += 1
    ...
    def _check_paper_consistency(
        ...
        if hasattr(paper, "title") and paper.title:
            ...
            if any(
                term in title_lower
                for term in ["language model", "transformer", "gpt", "bert"]
            ):
                if (
                    "parameters" in extraction_dict
                    and extraction_dict["parameters"]
                    and extraction_dict["parameters"] > 1e6
                ):
                    consistency_checks.append(1.0)
                else:
            ...
            if any(
                term in title_lower for term in ["vision", "image", "visual", "cnn"]
            ):
    ...
    def cross_validate_extractions(
        ...
        for field in all_fields:
            ...
            if manual_value == auto_value:
                ...
                if isinstance(manual_value, (int, float)) and isinstance(
                    auto_value, (int, float)
                ):
                    relative_diff = abs(manual_value - auto_value) / max(
                        manual_value, auto_value
                    )
