...
class FormatterRegistry:
    ...
    def register(
        cls,
        format_name: str,
        formatter_class: Type[ReportFormatter],
    ...
    @classmethod
    def get_formatter(
        cls, format_name: str, stage: Optional[str] = None
    ) -> Optional[Type[ReportFormatter]]:
...
def format_report(report: QualityReport, format_name: str = "text", **kwargs) -> str:
    ...
    if not formatter_class:
        raise ValueError(
            f"No formatter found for format '{format_name}' and stage '{report.stage}'"
        )
...
def save_report(
    report: QualityReport, output_path: Path, format_name: str = "text", **kwargs
):
