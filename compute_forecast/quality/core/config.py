...
class QualityThresholds(BaseModel):
    ...
    min_completeness: float = Field(
        default=0.8, ge=0.0, le=1.0, description="Minimum completeness score"
    )
    min_coverage: float = Field(
        default=0.7, ge=0.0, le=1.0, description="Minimum coverage score"
    )
    min_consistency: float = Field(
        default=0.9, ge=0.0, le=1.0, description="Minimum consistency score"
    )
    min_accuracy: float = Field(
        default=0.85, ge=0.0, le=1.0, description="Minimum accuracy score"
    )
...
class QualityConfigModel(BaseModel):
    ...
    thresholds: QualityThresholds = Field(
        default_factory=QualityThresholds, description="Quality thresholds"
    )
    skip_checks: List[str] = Field(
        default_factory=list, description="List of checks to skip"
    )
