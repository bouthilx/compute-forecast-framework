...
class QualityFilter:
    ...
    def filter_papers(self, papers: List[QualityMetrics]) -> Dict[str, List[QualityMetrics]]:
        """
        ...
        logger.debug(f"Filtered {len(papers)} papers: {len(results['passed'])} passed, {len(results['failed'])} failed")
    ...
    def get_filter_statistics(self) -> Dict[str, Any]:
        ...
        if stats['papers_evaluated'] > 0:
            ...
            stats['rejection_rate'] = stats['papers_rejected'] / stats['papers_evaluated']
        else:
            stats['pass_rate'] = 0.0
    ...
    def get_rejection_analysis(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'most_restrictive_threshold': most_restrictive[0] if most_restrictive[0] else None,
    ...
    def simulate_threshold_impact(self, papers: List[QualityMetrics],
                                 threshold_changes: Dict[str, float]) -> Dict[str, Any]:
        """
        ...
        temp_thresholds = QualityThresholds(
            ...
            min_citation_count=threshold_changes.get('min_citation_count', self.thresholds.min_citation_count),
            min_paper_quality_score=threshold_changes.get('min_paper_quality_score', self.thresholds.min_paper_quality_score),
            min_combined_quality_score=threshold_changes.get('min_combined_quality_score', self.thresholds.min_combined_quality_score),
            min_venue_quality_score=threshold_changes.get('min_venue_quality_score', self.thresholds.min_venue_quality_score),
            min_venue_impact_factor=threshold_changes.get('min_venue_impact_factor', self.thresholds.min_venue_impact_factor)
        )
        ...
        impact = {
            ...
            'change_in_passed': len(results['passed']) - len(original_results['passed']),
            'change_in_failed': len(results['failed']) - len(original_results['failed']),
        ...
        if len(papers) > 0:
            ...
            impact['pass_rate_change'] = impact['new_pass_rate'] - impact['original_pass_rate']
        
        return impact
    
    def is_paper_borderline(self, metrics: QualityMetrics, tolerance: float = 0.1) -> Dict[str, bool]:
        """
        ...
        borderline_status['citation_count'] = citation_diff <= (self.thresholds.min_citation_count * tolerance)
        
        # Paper quality score
        paper_score_diff = abs(metrics.paper_quality_score - self.thresholds.min_paper_quality_score)
        borderline_status['paper_quality_score'] = paper_score_diff <= tolerance
        
        # Combined quality score
        combined_score_diff = abs(metrics.combined_quality_score - self.thresholds.min_combined_quality_score)
        borderline_status['combined_quality_score'] = combined_score_diff <= tolerance
        
        # Venue quality score
        venue_score_diff = abs(metrics.venue_quality_score - self.thresholds.min_venue_quality_score)
        borderline_status['venue_quality_score'] = venue_score_diff <= tolerance
        
        # Venue impact factor
        impact_diff = abs(metrics.venue_impact_factor - self.thresholds.min_venue_impact_factor)
