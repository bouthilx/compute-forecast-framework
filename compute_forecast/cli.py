...
Examples:
  ...
  compute-forecast report --output forecast.pdf
    ...
    parser.add_argument(
        "--version",
        action="version",
        version=f"compute-forecast {__version__}"
    )
    ...
    collect_parser.add_argument("--venue", help="Conference venue (e.g., NeurIPS, ICML)")
    collect_parser.add_argument("--year", type=int, help="Publication year")
    collect_parser.add_argument("--limit", type=int, default=100, help="Maximum papers to collect")
    
    # Analyze command
    analyze_parser = subparsers.add_parser("analyze", help="Analyze computational requirements")
    analyze_parser.add_argument("--input", required=True, help="Input JSON file with papers")
    ...
    report_parser.add_argument("--output", default="forecast.pdf", help="Output report file")
