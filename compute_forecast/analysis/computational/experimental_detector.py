...
class ExperimentalDetector:
    ...
    def __init__(self):
        ...
        self.section_patterns = {
            "results": re.compile(
                r"\b(?:results?|findings?|outcomes?)\b", re.IGNORECASE
            ),
            "experiments": re.compile(
                r"\b(?:experiments?|experimental)\b", re.IGNORECASE
            ),
            "evaluation": re.compile(
                r"\b(?:evaluation|evaluating|evaluated)\b", re.IGNORECASE
            ),
            "methodology": re.compile(
                r"\b(?:methodology|methods?|approach)\b", re.IGNORECASE
            ),
            "implementation": re.compile(
                r"\b(?:implementation|implementing|implemented)\b", re.IGNORECASE
            ),
        }
    ...
    def detect_experimental_content(self, text: str) -> Dict[str, Any]:
        ...
        experimental_score = self._calculate_indicator_score(
            text_lower, self.experimental_indicators
        )
        methodology_score = self._calculate_indicator_score(
            text_lower, self.methodology_indicators
        )
        implementation_score = self._calculate_indicator_score(
            text_lower, self.implementation_indicators
        )
        quantitative_score = self._calculate_indicator_score(
            text_lower, self.quantitative_indicators
        )
        ...
        return {
            ...
            "experimental_confidence": self._calculate_confidence(
                overall_score, section_presence
            ),
        }
    ...
    def _calculate_overall_experimental_score(
        self,
        ...
        experimental_patterns: Dict[str, bool],
    ) -> float:
        ...
        section_boost = (
            sum(
                1
                for key, value in section_presence.items()
                if not key.endswith("_count") and value
            )
            * 0.02
        )
    ...
    def _determine_experimental_classification(
        self,
        overall_score: float,
        section_presence: Dict[str, Any],
        experimental_patterns: Dict[str, bool],
        ...
        if section_presence.get("results", False) and section_presence.get(
            "experiments", False
        ):
            return True
        ...
        if experimental_patterns.get(
            "has_implementation_details", False
        ) and experimental_patterns.get("has_performance_metrics", False):
            return True

        return False

    def _calculate_confidence(
        self, overall_score: float, section_presence: Dict[str, Any]
    ) -> float:
