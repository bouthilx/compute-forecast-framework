...
COMPUTATIONAL_INDICATORS = {
    'gpu_hardware': [
        ...
        'GeForce', 'Quadro', 'Titan', 'multi-GPU', 'GPU cluster',
        'VRAM', 'GPU memory', 'CUDA cores', 'tensor cores'
    ],
    ...
    'training_resources': [
        ...
        'training cost', 'compute time', 'processing time', 'runtime',
        'execution time', 'computational overhead', 'resource usage',
        'compute resources', 'training resources'
    ],
    ...
    'model_scale': [
        ...
        'pre-trained model', 'fine-tuning', 'model weights',
        'neural architecture', 'deep learning model'
    ],
    ...
    'dataset_scale': [
        ...
        'training samples', 'data points', 'training examples',
        'large-scale dataset', 'massive dataset', 'big data',
        'data volume', 'training corpus'
    ],
    ...
    'optimization_compute': [
        ...
        'hyperparameter optimization', 'model selection', 'parameter search',
        'optimization algorithm', 'learning rate scheduling',
    ...
    'infrastructure': [
        ...
        'data center', 'compute cluster', 'distributed computing',
        'parallel computing', 'cloud infrastructure', 'scalable computing',
        'containerization', 'Docker', 'Kubernetes'
    ]
}

COMPUTATIONAL_PATTERNS = {
    'gpu_count': re.compile(r'(\d+)\s*(?:x\s*)?(?:GPU|V100|A100|H100|RTX|Tesla)', re.IGNORECASE),
    'training_time': re.compile(r'(\d+(?:\.\d+)?)\s*(hours?|days?|weeks?|minutes?)', re.IGNORECASE),
    'parameter_count': re.compile(r'(\d+(?:\.\d+)?)\s*(million|billion|M|B)\s*parameters?', re.IGNORECASE),
    'dataset_size': re.compile(r'(\d+(?:\.\d+)?)\s*(million|billion|M|B|K)\s*(?:training\s+)?(samples?|examples?|tokens?)', re.IGNORECASE),
    'batch_size': re.compile(r'batch\s*size\s*(?:of\s*)?(\d+)', re.IGNORECASE),
    'memory_usage': re.compile(r'(\d+(?:\.\d+)?)\s*(GB|TB|MB)\s*(?:memory|RAM|VRAM)', re.IGNORECASE),
    'learning_rate': re.compile(r'learning\s*rate\s*(?:of\s*)?(\d+(?:\.\d+)?(?:e-?\d+)?)', re.IGNORECASE),
    'epochs': re.compile(r'(\d+)\s*epochs?', re.IGNORECASE),
    'compute_cost': re.compile(r'\$(\d+(?:,\d+)*(?:\.\d+)?)\s*(?:in\s*)?compute', re.IGNORECASE),
...
def get_category_counts() -> Dict[str, int]:
    ...
    return {category: len(keywords) for category, keywords in COMPUTATIONAL_INDICATORS.items()}
    ...
    for pattern_name, test_text in test_cases.items():
        ...
        if matches:
            ...
            if actual_groups != expected:
                results[f"{pattern_name}_group_count_error"] = f"Expected {expected} groups, got {actual_groups}"
