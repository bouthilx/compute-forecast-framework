...
class PaperClassifier(BaseAnalyzer):
    ...
    def classify_paper_authorship(self, paper: Paper, confidence_threshold: float = 0.7) -> AuthorshipAnalysis:
        ...
        for author in authors:
            ...
            affiliation_classification = self.affiliation_parser.classify_affiliation(affiliation)
            ...
            author_details.append({
                ...
                'matched_organization': affiliation_classification.get('matched_organization')
            })
    ...
    def _make_final_classification(self, academic_count: int, industry_count: int, 
                                 unknown_count: int, author_details: List[Dict]) -> Dict:
        ...
        if industry_percentage < 0.25:
            ...
            confidence = self._calculate_confidence(academic_percentage, author_details, 'academic')
        elif academic_percentage < 0.25:
            category = 'industry_eligible'  
            confidence = self._calculate_confidence(industry_percentage, author_details, 'industry')
        else:
            category = 'needs_manual_review'
            confidence = 0.5
    ...
    def _calculate_confidence(self, dominant_percentage: float, author_details: List[Dict], 
                            category_type: str) -> float:
        ...
        relevant_confidences = [
            author['confidence'] for author in author_details 
            if author['type'] == category_type
        ]
        
        if relevant_confidences:
            avg_individual_confidence = sum(relevant_confidences) / len(relevant_confidences)
    ...
    def get_classification_summary(self, papers: List[Paper]) -> Dict:
        ...
        return {
            ...
            'academic_percentage': academic_eligible / total_papers if total_papers > 0 else 0.0,
            'industry_percentage': industry_eligible / total_papers if total_papers > 0 else 0.0,
            'review_percentage': needs_review / total_papers if total_papers > 0 else 0.0,
