        ...
        if file_path is None:
            ...
            if file_path is None:
                raise FileNotFoundError(
                    f"No domain analysis file found in {package_dir}. Tried: {domain_files}"
                )
        ...
        for domain, count in domain_paper_counts.items():
            ...
            for venue in venues_in_domain:
                if venue in venue_stats:
                    domain_papers = [
                        p
                        for p in venue_stats[venue]["papers"]
            ...
            sorted_venues = sorted(
                domain_venue_stats.items(), key=lambda x: x[1], reverse=True
            )
