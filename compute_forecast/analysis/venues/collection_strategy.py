...
class CollectionStrategyOptimizer:
    ...
    def generate_collection_strategy(self, domain: str) -> DomainCollectionStrategy:
        ...
        for venue_name in self.general_ml_venues:
            venue_score = next(
                (v for v in venue_scores if v.venue_name == venue_name), None
            )
            if venue_score:
                target = VenueTarget(
        ...
        strategy_metadata = {
            ...
            "collection_coverage": self._assess_collection_coverage(
                domain, venue_scores
            ),
        }
    ...
    def _calculate_venue_diversity(
        self, venue_scores: List[VenueScore]
    ) -> Dict[str, Any]:
        ...
        venue_tiers = [
            score.ranking_factors.get("venue_tier", "unknown") for score in venue_scores
        ]
        tier_distribution = {tier: venue_tiers.count(tier) for tier in set(venue_tiers)}

        # Computational focus distribution
        comp_scores = [
            score.component_scores.get("computational_focus", 0)
            for score in venue_scores
        ]
    ...
    def _assess_collection_coverage(
        self, domain: str, venue_scores: List[VenueScore]
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "coverage_percentage": round(
                len(venues_in_both) / len(db_venue_names) * 100, 1
            )
            if db_venue_names
            else 0,
    ...
    def generate_all_domain_strategies(self) -> Dict[str, DomainCollectionStrategy]:
        ...
        for domain in all_domains:
            try:
                ...
                logger.info(
                    f"Generated strategy for {domain}: {strategy.total_target_papers} target papers"
                )
    ...
    def export_collection_strategies(self) -> Dict[str, Any]:
        ...
        for domain, strategy in strategies.items():
            ...
            total_venues += (
                len(strategy.primary_venues)
                + len(strategy.secondary_venues)
                + len(strategy.backup_venues)
            )
        ...
        if strategies:
            ...
            export_data["summary_statistics"] = {
                ...
                "domains_with_sufficient_venues": sum(
                    1 for s in strategies.values() if len(s.primary_venues) >= 2
                ),
                "collection_feasibility": "high"
                if avg_venues_per_domain >= 6
                else "medium",
            }
    ...
    def validate_strategies(self) -> Dict[str, Any]:
        ...
        for domain, strategy in strategies.items():
            ...
            if not domain_validation["target_papers_reasonable"]:
                issues.append(
                    f"Unreasonable target papers: {strategy.total_target_papers}"
                )
